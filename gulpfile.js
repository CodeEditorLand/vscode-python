	...
	if (args && args.buildNumber) {
		...
		const buildNumberPortion =
			versionParts.length > 2
				? versionParts[2].replace(/(\d+)/, args.buildNumber)
				: args.buildNumber;
		const newVersion =
...
gulp.task("installPythonRequirements", async () => {
	...
	await spawnAsync(
		...
		.catch((ex) => {
			console.error(
				"Failed to install Jedi LSP requirements using 'python'",
				ex
			);
			return false;
		});
...
gulp.task("installDebugpy", async () => {
	...
	await spawnAsync(
		...
		.catch((ex) => {
			console.error(
				"Failed to install dependencies need by 'install_debugpy.py' using 'python'",
				ex
			);
			return false;
		});
	...
	await spawnAsync(
		...
		.catch((ex) => {
			console.error(
				"Failed to install DEBUGPY wheels using 'python'",
				ex
			);
			return false;
		});
	...
	await spawnAsync(
		...
		.catch((ex) => {
			console.error(
				"Failed to download get-pip wheels using 'python'",
				ex
			);
			return false;
		});
...
gulp.task(
	"installPythonLibs",
	gulp.series("installPythonRequirements", "installDebugpy")
);
