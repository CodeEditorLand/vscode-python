...
suite("Result Resolver tests", () => {
	suite("Test discovery", () => {
		...
		setup(() => {
			...
			defaultErrorMessage =
				"pytest test discovery error (see Output > Python)";
			blankTestItem = {
		...
		test("resolveDiscovery calls populate test tree correctly", async () => {
			...
			const populateTestTreeStub = sinon
				.stub(util, "populateTestTree")
				.returns();

			// call resolve discovery
			const deferredTillEOT: Deferred<void> = createDeferred<void>();
			resultResolver.resolveDiscovery(
				payload,
				deferredTillEOT,
				cancelationToken
			);
		...
		test("resolveDiscovery should create error node on error with correct params and no root node with tests in payload", async () => {
			...
			const buildErrorNodeOptionsStub = sinon
				.stub(util, "buildErrorNodeOptions")
				.returns(errorTestItemOptions);
			const createErrorTestItemStub = sinon
				.stub(testItemUtilities, "createErrorTestItem")
				.returns(blankTestItem);
		...
		test("resolveDiscovery should create error and not clear test items to allow for error tolerant discovery", async () => {
			...
			if (!deleteSpy.calledOnce) {
				throw new Error(
					"The delete method was called, but it shouldn't have been."
				);
			}
			if (replaceSpy.called) {
				throw new Error(
					"The replace method was called, but it shouldn't have been."
				);
			}
		});
	});
	suite("Test execution result resolver", () => {
		...
		setup(() => {
			...
			const testItemCollectionMock =
				typemoq.Mock.ofType<TestItemCollection>();
			...
			testControllerMock
				.setup((t) => t.items)
				.returns(() => testItemCollectionMock.object);
			...
			sinon
				...
				.callsFake((testNode: TestItem) => [testNode]);
		});
		teardown(() => {
			sinon.restore();
		});
		test("resolveExecution create correct subtest item for unittest", async () => {
			// test specific constants used expected values
			sinon
				.stub(testItemUtilities, "clearAllChildren")
				.callsFake(() => undefined);
			...
			testControllerMock
				.setup((t) =>
					t.createTestItem(typemoq.It.isAny(), typemoq.It.isAny())
				)
				.callback((id: string) => {
					generatedId = id;
			...
			const deferredTillEOT: Deferred<void> = createDeferred<void>();
			resultResolver.resolveExecution(
				successPayload,
				runInstance.object,
				deferredTillEOT
			);
			...
			assert.strictEqual(
				generatedId,
				"[subTest with spaces and [brackets]]"
			);
		});
		test("resolveExecution handles failed tests correctly", async () => {
			...
			// verify that the passed function was called for the single test item
			runInstance.verify(
				(r) => r.failed(typemoq.It.isAny(), typemoq.It.isAny()),
				typemoq.Times.once()
			);
		});
		...
		test("resolveExecution handles error correctly as test outcome", async () => {
			...
			// verify that the passed function was called for the single test item
			runInstance.verify(
				(r) => r.errored(typemoq.It.isAny(), typemoq.It.isAny()),
				typemoq.Times.once()
			);
		});
