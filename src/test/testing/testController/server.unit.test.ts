...
suite("Python Test Server, DataWithPayloadChunks", () => {
	...
	setup(async () => {
		...
		execService
			.setup((x) =>
				x.execObservable(typeMoq.It.isAny(), typeMoq.It.isAny())
			)
			.returns(() => ({
				proc: mockProc,
		...
		execService
			.setup((p) => (p as unknown as any).then)
			.returns(() => undefined);
	});
		...
		test(`run correctly`, async () => {
			...
			const stubExecutionFactory2 = {
				createActivatedEnvironment: () =>
					Promise.resolve(stubExecutionService2),
...
suite("Python Test Server, Send command etc", () => {
	...
	test("sendCommand should add the port to the command being sent and add the correct extra spawn variables", async () => {
		...
		execService
			...
			.returns((_args, options2) => {
				...
				return typeMoq.Mock.ofType<ObservableExecutionResult<string>>()
					.object;
			});
		...
		execService.verify(
			(x) => x.execObservable(expectedArgs, typeMoq.It.isAny()),
			typeMoq.Times.once()
		);
		if (error) {
	...
	test("sendCommand should add right extra variables to command during debug", async () => {
		...
		debugLauncherMock
			.setup((x) =>
				x.launchDebugger(typeMoq.It.isAny(), typeMoq.It.isAny())
			)
			.returns((options, _) => {
		...
		execService
			...
			.returns(
				() =>
					typeMoq.Mock.ofType<ObservableExecutionResult<string>>()
						.object
			);
		const execFactory = typeMoq.Mock.ofType<IPythonExecutionFactory>();
		execFactory
		...
		server = new PythonTestServer(
			execFactory.object,
			debugLauncherMock.object
		);
		sinon.stub(server, "getPort").returns(12345);
		// const portServer = server.getPort();
		await server.serverReady();
		...
		debugLauncherMock.verify(
			(x) => x.launchDebugger(typeMoq.It.isAny(), typeMoq.It.isAny()),
			typeMoq.Times.once()
		);
		if (error) {
			assert(false, errorMessage);
