...
function configure(): SetupOptions {
	...
	const invert =
		(process.env.VSC_PYTHON_CI_TEST_INVERT_GREP || "").length > 0;
...
function activatePythonExtensionScript() {
	const ex = new Error(
		"Failed to initialize Python extension for tests after 3 minutes"
	);
...
export async function run(): Promise<void> {
	...
	const ignoreGlob =
		options.testFilesSuffix.toLowerCase() === "ds.test"
			? []
			: ["**/**.ds.test.js"];
