...
suite("Application Diagnostics - Checks Python Path in debugger", () => {
	...
	setup(() => {
		...
		diagnosticService =
			new (class extends InvalidPythonPathInDebuggerService {
				public _clear() {
					while (
	...
	test("InvalidPythonPathInDebuggerSettings diagnostic should display one option to with a command", async () => {
		...
		diagnostic
			...
			.verifiable(typemoq.Times.atLeastOnce());
		const interpreterSelectionCommand =
			typemoq.Mock.ofType<IDiagnosticCommand>();
		commandFactory
			.setup((f) =>
				f.createCommand(
					typemoq.It.isAny(),
					typemoq.It.isObjectWith<
						CommandOption<"executeVSCCommand", CommandsWithoutArgs>
					>({
