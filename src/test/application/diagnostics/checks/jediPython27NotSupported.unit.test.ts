...
suite("Application Diagnostics - Jedi with Python 2.7 deprecated", () => {
	suite("Diagnostics", () => {
		...
		test("Should return an empty diagnostics array if the active interpreter version is Python 3", async () => {
			...
			const { configurationService, workspaceService } =
				createConfigurationAndWorkspaceServices(
		...
		setup(() => {
			serviceContainer = {
				get: (serviceIdentifier: symbol) =>
					services[serviceIdentifier.toString()] as IWorkspaceService,
				tryGet: () => ({}),
			} as unknown as IServiceContainer;
	...
	suite("Handler", () => {
		...
		setup(() => {
			...
			handleMessageStub = sinon.stub(
				DiagnosticCommandPromptHandlerService.prototype,
				"handle"
			);
		});
		...
		test("Handling a diagnostic that should be ignored does not display a prompt", async () => {
			const diagnosticHandlerService =
				new DiagnosticCommandPromptHandlerService(serviceContainer);
			...
			await service.handle([
				new JediPython27NotSupportedDiagnostic("ignored", undefined),
			]);
		...
		test("Handling a diagnostic should show a prompt", async () => {
			...
			const diagnostic = new JediPython27NotSupportedDiagnostic(
				"diagnostic",
				undefined
			);
