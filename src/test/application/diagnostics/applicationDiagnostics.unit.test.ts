...
suite("Application Diagnostics - ApplicationDiagnostics", () => {
	...
	setup(() => {
		...
		envHealthCheck
			.setup((service) => service.runInBackground)
			.returns(() => true);
		...
		pythonInterpreterCheck
			...
			.returns(() => false);
		pythonInterpreterCheck
			.setup((service) => service.runInUntrustedWorkspace)
			.returns(() => false);
		...
		serviceContainer
			...
			.returns(() => [
				envHealthCheck.object,
				lsNotSupportedCheck.object,
				pythonInterpreterCheck.object,
			]);
		serviceContainer
	...
	test("Register should register source maps", () => {
		const sourceMapService =
			typemoq.Mock.ofType<ISourceMapSupportService>();
		sourceMapService
			.setup((s) => s.register())
	...
	test("Ensure diagnostics run in foreground and background", async () => {
		...
		when(
			svcContainer.getAll<IDiagnosticsService>(IDiagnosticsService)
		).thenReturn([
		...
		when(foreGroundService.diagnose(anything())).thenReturn(
			foreGroundDeferred.promise
		);
		when(backGroundService.diagnose(anything())).thenReturn(
			backgroundGroundDeferred.promise
		);
