...
[true, false].forEach((tbExtensionInstalled) => {
	suite(
		`Tensorboard Extension is ${
			tbExtensionInstalled ? "installed" : "not installed"
		}`,
		() => {
			suite("TensorBoard nbextension code lens provider", () => {
				...
				setup(() => {
					sinon
						.stub(
							...
							"isTensorboardExtensionInstalled"
						)
						.returns(tbExtensionInstalled);
					experiment = mock<TensorboardExperiment>();
					codeLensProvider =
						new TensorBoardNbextensionCodeLensProvider(
							[],
							instance(experiment)
						);
				...
				test("Provide code lens for Python notebook loading tensorboard nbextension", async () => {
					...
					const codeLens = codeLensProvider.provideCodeLenses(
						document,
						cancelTokenSource.token
					);
					assert.ok(
						codeLens.length > 0,
						"Failed to provide code lens for file loading tensorboard nbextension"
					);
				});
				...
				test("Fails when cancellation is signaled", () => {
					...
					assert.ok(
						codeLens.length === 0,
						"Provided codelens even after cancellation was requested"
					);
				});
