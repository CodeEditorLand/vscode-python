...
export function escapeCodicons(text: string): string {
	return text.replace(escapeCodiconsRegex, (match, escaped) =>
		escaped ? match : `\\${match}`
	);
}
	...
	export function t(
		...
		if ((_args as Array<string>).length > 0) {
			return (_message as string).replace(/{(\d+)}/g, (match, number) =>
				(_args as Array<string>)[number] === undefined
					? match
					: (_args as Array<string>)[number]
			);
		}
...
export class Hover {
	...
	constructor(
		...
		if (Array.isArray(contents)) {
			this.contents = <vscode.MarkdownString[] | vscode.MarkedString[]>(
				contents
			);
...
export class CodeActionKind {
	...
	public static readonly RefactorMove: CodeActionKind = new CodeActionKind(
		"refactor.move"
	);
	...
	public static readonly SourceOrganizeImports: CodeActionKind =
		new CodeActionKind("source.organize.imports");
...
export interface TestItemCollection
	extends Iterable<[string, vscode.TestItem]> {
	/**
