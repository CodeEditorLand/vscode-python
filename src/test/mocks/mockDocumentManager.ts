...
export class MockDocumentManager implements IMockDocumentManager {
	...
	private didChangeTextEditorSelectionEmitter =
		new EventEmitter<TextEditorSelectionChangeEvent>();

	private didChangeTextEditorOptionsEmitter =
		new EventEmitter<TextEditorOptionsChangeEvent>();

	private didChangeTextEditorViewColumnEmitter =
		new EventEmitter<TextEditorViewColumnChangeEvent>();

	private didCloseEmitter = new EventEmitter<TextDocument>();

	private didSaveEmitter = new EventEmitter<TextDocument>();

	private didChangeTextDocumentEmitter =
		new EventEmitter<TextDocumentChangeEvent>();
	...
	public openTextDocument(_options?: {
		language?: string;
		content?: string;
	...
	public addDocument(
		...
		let existing = this.textDocuments.find(
			(d) => d.uri.fsPath === file
		) as MockDocument;
		if (existing) {
			existing.setContent(code);
		} else {
			existing = new MockDocument(
				code,
				file,
				this.saveDocument,
				language
	...
	public changeDocument(
		file: string,
		changes: { range: Range; newText: string }[]
	): void {
		const doc = this.textDocuments.find(
			(d) => d.uri.fsPath === Uri.file(file).fsPath
		) as MockDocument;
		if (doc) {
	...
	private saveDocument = (doc: TextDocument): Promise<boolean> => {
		...
		this.addDocument(
			doc.getText(),
			path.join(EXTENSION_ROOT_DIR, "baz.py")
		);
