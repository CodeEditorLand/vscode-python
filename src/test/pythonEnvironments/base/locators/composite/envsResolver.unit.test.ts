...
suite("Python envs locator - Environments Resolver", () => {
	...
	suite("iterEnvs()", () => {
		...
		setup(() => {
			...
			sinon
				.stub(workspaceApis, "getWorkspaceFolderPaths")
				.returns([testVirtualHomeDir]);
		});
		...
		test("Iterator yields environments after resolving basic envs received from parent iterator", async () => {
			...
			const resolvedEnvReturnedByBasicResolver =
				createExpectedResolvedEnvInfo(
					path.join(
						testVirtualHomeDir,
						".venvs",
						"win1",
			...
			const parentLocator = new SimpleLocator<BasicEnvInfo>(
				envsReturnedByParentLocator
			);
		...
		test("Updates to environments from the incoming iterator are applied properly", async () => {
			...
			const resolvedUpdatedEnvReturnedByBasicResolver =
				createExpectedResolvedEnvInfo(
					path.join(
						testVirtualHomeDir,
						".venvs",
			...
			const iteratorUpdateCallback = () => {
				...
				}); // It is essential for the incoming iterator to fire event signifying it's done
			};
			const envs = await getEnvsWithUpdates(
				iterator,
	...
	suite("resolveEnv()", () => {
		let stubShellExec: sinon.SinonStub;
		const envsWithoutPython = path.join(
			TEST_LAYOUT_ROOT,
			"envsWithoutPython"
		);
		...
		test("Calls into basic resolver to get environment info, then calls environnment service to resolve environment further and return it", async function () {
			...
			const expected = await resolver.resolveEnv(
				path.join(testVirtualHomeDir, ".venvs", "win1", "python.exe")
			);
		...
		test("Resolver should return empty version info for envs lacking an interpreter", async function () {
			...
			sinon
				...
				.returns("conda");
			sinon
				.stub(externalDependencies, "exec")
				.callsFake(async (command: string, args: string[]) => {
					...
					throw new Error(
						`${command} is missing or is not executable`
					);
				});
			const parentLocator = new SimpleLocator([]);
			const resolver = new PythonEnvsResolver(
			...
			const expected = await resolver.resolveEnv(
				path.join(envsWithoutPython, "condaLackingPython")
			);

			assert.deepEqual(expected?.version, getEmptyVersion());
			assert.equal(expected?.display, "Python ('condaLackingPython')");
			assert.equal(
				expected?.detailedDisplayName,
				"Python ('condaLackingPython': conda)"
			);
		});
