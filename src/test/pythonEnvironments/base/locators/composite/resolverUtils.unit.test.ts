...
suite("Resolver Utils", () => {
	...
	setup(() => {
		sinon
			...
			.withArgs("condaPath")
			.returns("conda");
		getWorkspaceFolders = sinon.stub(
			workspaceApis,
	...
	suite("Pyenv", () => {
		...
		setup(() => {
			sinon
				.stub(platformApis, "getEnvironmentVariable")
				.withArgs("PYENV_ROOT")
				.returns(testPyenvRoot);
		});
	...
	suite("Microsoft store", () => {
		...
		const testStoreAppRoot = path.join(
			testLocalAppData,
			"Microsoft",
			"WindowsApps"
		);
	...
	suite("Globally-installed envs", () => {
		const testPosixKnownPathsRoot = path.join(
			TEST_LAYOUT_ROOT,
			"posixroot"
		);
	...
	suite("Windows registry", () => {
		...
		function fakeRegistryValues({
			arch,
			hive,
			key,
		}: winreg.Options): Promise<winreg.IRegistryValue[]> {
			const regArch =
				arch === "x86" ? registryData.x86 : registryData.x64;
		...
		function fakeRegistryKeys({
			arch,
			hive,
			key,
		}: winreg.Options): Promise<winreg.IRegistryKey[]> {
			const regArch =
				arch === "x86" ? registryData.x86 : registryData.x64;
			const regHive = hive === winreg.HKCU ? regArch.HKCU : regArch.HKLM;
			for (const k of regHive) {
		...
		setup(async () => {
			sinon
				.stub(winreg, "readRegistryValues")
				.callsFake(fakeRegistryValues);
