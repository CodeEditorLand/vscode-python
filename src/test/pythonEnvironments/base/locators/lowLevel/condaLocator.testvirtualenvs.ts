...
class CondaEnvs {
	...
	constructor() {
		...
		this.condaEnvironmentsTxt = path.join(
			home,
			".conda",
			"environments.txt"
		);
	...
	public async create(): Promise<void> {
		try {
			...
			throw new Error(
				`Failed to create environments.txt ${this.condaEnvironmentsTxt}, Error: ${err}`
			);
		}
	}
	...
	public async update(): Promise<void> {
		try {
			await fs.writeFile(
				this.condaEnvironmentsTxt,
				"path/to/environment"
			);
		} catch (err) {
			throw new Error(
				`Failed to update environments file ${this.condaEnvironmentsTxt}, Error: ${err}`
			);
		}
	}
