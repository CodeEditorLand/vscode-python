...
class MicrosoftStoreEnvs {
	...
	public async create(version: string): Promise<string> {
		const dirName = path.join(
			this.storeAppRoot,
			`PythonSoftwareFoundation.Python.${version}_qbz5n2kfra8p0`
		);
		...
		try {
			...
			throw new Error(
				`Failed to create Windows Apps executable ${filename}, Error: ${err}`
			);
		}
		try {
			...
			throw new Error(
				`Failed to create Windows Apps directory ${dirName}, Error: ${err}`
			);
		}
	...
	public async update(version: string): Promise<void> {
		...
		try {
			...
			throw new Error(
				`Failed to update Windows Apps directory ${dirName}, Error: ${err}`
			);
		}
	}
