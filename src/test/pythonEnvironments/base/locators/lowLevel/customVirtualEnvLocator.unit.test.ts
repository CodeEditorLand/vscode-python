...
suite("CustomVirtualEnvironment Locator", () => {
	...
	setup(async () => {
		...
		watchLocationForPatternStub = sinon.stub(
			fsWatcher,
		...
		onDidChangePythonSettingStub = sinon.stub(
			externalDependencies,
		...
		readFileStub
			...
			.returns(path.join(TEST_LAYOUT_ROOT, "pipenv", "project2"));
	...
	test("iterEnvs(): Windows with both settings set", async () => {
		getPythonSettingStub
			.withArgs("venvPath")
			.returns(testVenvPathWithTilda);
		getPythonSettingStub
			.withArgs("venvFolders")
			.returns([".venvs", ".virtualenvs", "Envs"]);
	...
	test("iterEnvs(): Non-Windows with both settings set", async () => {
		...
		getPythonSettingStub
			.withArgs("venvPath")
			.returns(path.join(testWorkspaceFolder, "posix2conda"));
		getPythonSettingStub
			.withArgs("venvFolders")
			.returns([
				".venvs",
