...
import {
	getEnvs,
	getQueryFilter,
} from "../../../client/pythonEnvironments/base/locatorUtils";
...
const env2 = createNamedEnv(
	...
	"3.8.1rc2",
	PythonEnvKind.Pyenv,
	"/pyenv/3.8.1rc2/bin/python"
);
const env3 = createNamedEnv(
	"env3",
	"3.9.1b2",
...
const envS1 = createNamedEnv(
	"env S1",
	"3.9",
	PythonEnvKind.OtherVirtual,
	`${homeDir}/some-dir/bin/python`
);
setSearchLocation(envS1, `${homeDir}/`); // Have a search location ending in '/'
const envS2 = createNamedEnv(
	"env S2",
	"3.9",
	PythonEnvKind.OtherVirtual,
	`${homeDir}/some-dir2/bin/python`
);
setSearchLocation(envS2, homeDir);
const envS3 = createNamedEnv(
	"env S2",
	"3.9",
	PythonEnvKind.OtherVirtual,
	`${workspaceRoot.fsPath}/p/python`
);
envS3.searchLocation = workspaceRoot;
const rootedEnvs = [envS1, envS2, envS3];

const envSL1 = createLocatedEnv(
	`${homeDir}/.venvs/envSL1`,
	"3.9.0",
	PythonEnvKind.Venv
);
setSearchLocation(envSL1);
const envSL2 = createLocatedEnv(
	`${workspaceRoot.fsPath}/.venv`,
	"3.8.2",
	PythonEnvKind.Pipenv
);
setSearchLocation(envSL2);
const envSL3 = createLocatedEnv(
	`${homeDir}/.conda-envs/envSL3`,
	"3.8.2",
	PythonEnvKind.Conda
);
setSearchLocation(envSL3);
const envSL4 = createLocatedEnv(
	"/opt/python3.10",
	"3.10.0a1",
	PythonEnvKind.Custom
);
setSearchLocation(envSL4);
const envSL5 = createLocatedEnv(
	`${homeDir}/.venvs/envSL5`,
	"3.9.0",
	PythonEnvKind.Venv
);
