...
suite("Conda Creation provider tests", () => {
	...
	setup(() => {
		...
		pickExistingCondaActionStub = sinon.stub(
			condaUtils,
			"pickExistingCondaAction"
		);
		pickExistingCondaActionStub.resolves(
			condaUtils.ExistingCondaAction.Create
		);

		getPrefixCondaEnvPathStub = sinon.stub(
			commonUtils,
	...
	test("Create conda environment", async () => {
		...
		_next!({
			out: `${CONDA_ENV_CREATED_MARKER}new_environment`,
			source: "stdout",
		});
		_complete!();
	...
	test("Use existing conda environment", async () => {
		...
		pickWorkspaceFolderStub.resolves(workspace1);
		pickExistingCondaActionStub.resolves(
			condaUtils.ExistingCondaAction.UseExisting
		);
