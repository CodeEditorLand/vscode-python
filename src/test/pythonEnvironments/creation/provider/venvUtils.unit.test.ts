...
suite("Venv Utils test", () => {
		...
		findFilesStub.callsFake(() => {
			if (allow) {
				...
				return Promise.resolve([
					Uri.file(
						path.join(workspace1.uri.fsPath, "requirements.txt")
					),
					Uri.file(
		...
		assert.isTrue(
			showQuickPickWithBackStub.calledWithExactly(
				[
					...
					{
						label: "dev-requirements.txt",
						buttons: [OPEN_REQUIREMENTS_BUTTON],
					},
					{
						label: "test-requirements.txt",
						buttons: [OPEN_REQUIREMENTS_BUTTON],
					},
				],
				{
	...
	test("User clicks button to open requirements.txt", async () => {
		...
		showQuickPickWithBackStub.callsFake(
			async (_items, _options, _token, callback) => {
				callback({
