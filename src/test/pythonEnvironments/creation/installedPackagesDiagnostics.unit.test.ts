...
const MISSING_PACKAGES: Diagnostic[] = [
	{
		...
		message:
			"Package `flake8-csv` is not installed in the selected environment.",
		source: "Python-InstalledPackagesChecker",
		code: {
			value: "not-installed",
			target: Uri.parse(`https://pypi.org/p/flake8-csv`),
		},
		severity: 3,
		relatedInformation: [],
	},
...
suite("Create Env content button settings tests", () => {
	...
	setup(() => {
		...
		getOpenTextDocumentsStub = sinon.stub(
			workspaceApis,
			"getOpenTextDocuments"
		);
		getOpenTextDocumentsStub.returns([]);

		onDidOpenTextDocumentStub = sinon.stub(
			workspaceApis,
			"onDidOpenTextDocument"
		);
		onDidSaveTextDocumentStub = sinon.stub(
			workspaceApis,
			"onDidSaveTextDocument"
		);
		onDidCloseTextDocumentStub = sinon.stub(
			workspaceApis,
		...
		getInstalledPackagesDiagnosticsStub = sinon.stub(
			installUtils,
			"getInstalledPackagesDiagnostics"
		);
		interpreterPathService = typemoq.Mock.ofType<IInterpreterPathService>();
		interpreterPathService
			.setup((i) =>
				i.onDidChange(typemoq.It.isAny(), undefined, undefined)
			)
	...
	[getSomeRequirementFile().object, getPyProjectTomlFile().object].forEach(
		(file) => {
			test(`Should run packages check on open of a dep file: ${file.fileName}`, () => {
				...
				getInstalledPackagesDiagnosticsStub.returns(
					Promise.resolve(MISSING_PACKAGES)
				);
			...
			test(`Should trigger a context update on active editor switch to dep file: ${file.fileName}`, () => {
				...
				diagnosticCollection
					.setup((d) => d.get(typemoq.It.isAny()))
					.returns(() => MISSING_PACKAGES);
