...
suite("Environment Identifier", () => {
	suite("Conda", () => {
		test("Conda layout with conda-meta and python binary in the same directory", async () => {
			...
			const envType: PythonEnvKind =
				await identifyEnvironment(interpreterPath);
	...
	suite("Pipenv", () => {
		...
		test("Path to a global pipenv environment", async () => {
			...
			const expectedProjectFile = path.join(
				TEST_LAYOUT_ROOT,
				"pipenv",
				"project2"
			);
			readFile
				.withArgs(expectedDotProjectFile)
				.resolves(expectedProjectFile);
			const interpreterPath: string = path.join(
				TEST_LAYOUT_ROOT,
				"pipenv",
				"globalEnvironments",
	...
	suite("Microsoft Store", () => {
		...
		executable.forEach((exe) => {
			...
			test(`Program files app data not set (${exe})`, async () => {
				...
				pathExists
					.withArgs(
						path.join(path.dirname(interpreterPath), "idle.exe")
					)
					.resolves(true);
			...
			test(`Path using long path style slashes (${exe})`, async () => {
				...
				const envType: PythonEnvKind = await identifyEnvironment(
					`\\\\?\\${interpreterPath}`
				);
				assert.deepEqual(envType, PythonEnvKind.MicrosoftStore);
			});
	...
	suite("Pyenv", () => {
		...
		test("PYENV_ROOT is set to a custom value on non-Windows", async function () {
			...
			getEnvVarStub
				.withArgs("PYENV_ROOT")
				.returns(path.join(TEST_LAYOUT_ROOT, "pyenv3"));
	...
	suite("Virtualenvwrapper", () => {
		...
		suiteSetup(() => {
			...
			getUserHomeDirStub.returns(
				path.join(TEST_LAYOUT_ROOT, "virtualenvwrapper1")
			);
		});
