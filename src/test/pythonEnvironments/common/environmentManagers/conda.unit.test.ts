...
suite("Conda and its environments are located correctly", () => {
				...
				if (child === undefined) {
					if (throwIfMissing) {
						const err: NodeJS.ErrnoException = new Error(
							`${currentPath} does not contain ${fileName}`
						);
						err.code = "ENOENT";
						throw err;
					} else {
	...
	setup(() => {
		...
		sinon
			.stub(windowsUtils, "getRegistryInterpreters")
			.callsFake(async () => registryInterpreters);
		...
		sinon.stub(fsapi, "lstat").callsFake(async (filePath: fs.PathLike) => {
			if (typeof filePath !== "string") {
				throw new Error(
					`expected filePath to be string, got ${typeof filePath}`
				);
			}
		...
		sinon
			.stub(fsapi, "pathExists")
			.callsFake(async (filePath: string | Buffer) => {
				if (typeof filePath !== "string") {
					throw new Error(
						`expected filePath to be string, got ${typeof filePath}`
					);
		...
		sinon
			.stub(fsapi, "readdir")
			.callsFake(async (filePath: fs.PathLike) => {
				if (typeof filePath !== "string") {
					throw new Error(
						`expected filePath to be string, got ${typeof filePath}`
					);
		...
		sinon
			...
			.callsFake(
				async (
					...
					if (typeof filePath !== "string") {
						throw new Error(
							`expected path to be string, got ${typeof path}`
						);
					}
			...
			.callsFake(async (command: string, args: string[]) => {
				for (const prefix of ["", ...execPath]) {
					...
					if (args[0] === "info" && args[1] === "--json") {
						...
						throw new Error(
							`Invalid arguments: ${util.inspect(args)}`
						);
					}
				}
	...
	suite("Conda binary is located correctly", () => {
		...
		suite("Must find conda in well-known locations", () => {
			...
			condaDirNames.forEach((condaDirName) => {
				suite(
					`Must find conda in well-known locations on Linux with ${condaDirName} directory name`,
					() => {
						setup(() => {
							osType = platform.OSType.Linux;
							homeDir = "/home/user";
				...
				suite(
					`Must find conda in well-known locations on Windows with ${condaDirName} directory name`,
					() => {
						setup(() => {
							osType = platform.OSType.Windows;
						...
						[
							"D:\\ProgramData",
							"E:\\Users\\user",
							"F:\\Users\\user\\AppData\\Local\\Continuum",
						].forEach((prefix) => {
							const condaPath = `${prefix}\\${condaDirName}`;
	...
	test("Conda run args returns appropriate args for conda version starting with 4.9.0", async () => {
		...
		const conda = await Conda.getConda();
		let args = await conda?.getRunPythonArgs({
			name: "envName",
			prefix: "envPrefix",
		});
