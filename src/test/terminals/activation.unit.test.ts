...
suite("Terminal", () => {
	suite("Terminal Auto Activation", () => {
		...
		setup(() => {
			...
			when(manager.onDidOpenTerminal).thenReturn(
				onDidOpenTerminalEventEmitter.event
			);
			when(
				activator.activateEnvironmentInTerminal(anything(), anything())
			).thenResolve();
		...
		test("Should activate terminal", async () => {
			...
			await (onDidOpenTerminalEventEmitter.fire(
				terminal
			) as unknown as Promise<void>);

			// The terminal should get activated.
			verify(
				activator.activateEnvironmentInTerminal(terminal, anything())
			).once();
		});
