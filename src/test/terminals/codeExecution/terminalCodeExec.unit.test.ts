		...
		(testSuiteName) => {
			...
			setup(() => {
				...
				pythonExecutionFactory =
					TypeMoq.Mock.ofType<IPythonExecutionFactory>();
			...
			suite(`${testSuiteName} (validation of title)`, () => {
						...
						.setup((f) =>
							f.getTerminalService(
								TypeMoq.It.is<TerminalCreationOptions>(
									(a) => a.title === expectedTerminalTitle
								)
							)
						)
				...
				async function ensureTerminalIsCreatedUponInvokingInitializeRepl(
					...
					interpreterService
						.setup((s) =>
							s.getActiveInterpreter(TypeMoq.It.isAny())
						)
						.returns(() =>
							Promise.resolve({
								path: PYTHON_PATH,
			...
			suite(testSuiteName, async function () {
				...
				async function ensureWeSetCurrentDriveBeforeChangingDirectory(
					_isWindows: boolean
				): Promise<void> {
					const file = Uri.file(
						path.join("d:", "path", "to", "file", "one.py")
					);
					terminalSettings
					...
					terminalService.verify(
						async (t) => t.sendText(TypeMoq.It.isValue("d:")),
						TypeMoq.Times.once()
					);
				}
				...
				test("Ensure once set current drive before, we always send command to change the drive letter for subsequent executions", async () => {
					...
					await executor.executeFile(file);
					terminalService.verify(
						async (t) => t.sendText(TypeMoq.It.isValue("c:")),
						TypeMoq.Times.once()
					);
				});
				...
				test("Ensure we do not change drive if current drive letter is same as the file drive letter on windows", async () => {
					await ensureWeDoNotChangeDriveIfDriveLetterSameAsFileDriveLetter(
						true
					);
				});
				...
				async function ensureWeWetCurrentDirectoryAndQuoteBeforeExecutingFile(
					...
					terminalService.verify(
						async (t) =>
							t.sendText(TypeMoq.It.isValue(`cd ${dir}`)),
						TypeMoq.Times.once()
					);
				}
				...
				test("Ensure we set current directory before executing file if in the same directory as the current workspace (non windows)", async () => {
					await ensureWeSetCurrentDirectoryBeforeExecutingFileInWorkspaceDirectory(
						false
					);
				});
				...
				test("Ensure we set current directory before executing file if file is not in a workspace (non windows)", async () => {
					await ensureWeSetCurrentDirectoryBeforeExecutingFileNotInSameDirectory(
						false
					);
				});
				...
				async function testCondaFileExecution(
					...
					sinon
						.stub(Conda, "getConda")
						.resolves(new Conda(condaFile));
					sinon
						.stub(Conda.prototype, "getCondaVersion")
						.resolves(new SemVer(CONDA_RUN_VERSION));
					sinon
						.stub(
							Conda.prototype,
					...
					if (!env) {
						assert(
							false,
							"Should not be undefined for conda version 4.9.0"
						);
						return;
					}
					const procs = createPythonProcessService(
						procService.object,
						env
					);
					const condaExecutionService = {
						getInterpreterInformation:
							env.getInterpreterInformation,
				...
				async function testReplCommandArguments(
					...
					const replCommandArgs = await (
						executor as TerminalCodeExecutionProvider
					).getExecutableInfo();
				...
				async function testReplCondaCommandArguments(
					...
					const djangoArgs = isDjangoRepl
						? ["manage.py", "shell"]
						: [];
					const expectedTerminalArgs = [
						...terminalArgs,
						...djangoArgs,
					];
					...
					expect(replCommandArgs).not.to.be.an(
						"undefined",
						"Conda command args are undefined"
					);
					expect(replCommandArgs.command).to.be.equal(
						pythonPath,
						"Repl needs to use python, not conda"
					);
					expect(replCommandArgs.args).to.be.deep.equal(
						expectedTerminalArgs,
						"Incorrect terminal arguments"
					);
				}

				test("Ensure conda args with env name are returned when building repl args with a conda env with a name", async () => {
					await testReplCondaCommandArguments(
						PYTHON_PATH,
						["-a", "b", "c"],
						{
							name: "foo-env",
