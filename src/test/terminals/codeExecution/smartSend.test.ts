...
suite("REPL - Smart Send", () => {
	...
	test("Cursor is not moved when explicit selection is present", async () => {
		experimentService
			.setup((exp) =>
				exp.inExperimentSync(
					TypeMoq.It.isValue(EnableREPLSmartSend.experiment)
				)
			)
			.returns(() => true);
		...
		const wholeFileContent = await fs.readFile(
			path.join(TEST_FILES_PATH, `sample_smart_selection.py`),
			"utf8"
		);
		...
		processService
			...
			.returns((file, args, options) =>
				execObservable.apply(actualProcessService, [
					file,
					args,
					options,
				])
		...
		await codeExecutionHelper.normalizeLines(
			"my_dict = {",
			wholeFileContent
		);
