...
suite("Module Installer", () => {
	[undefined, Uri.file(__filename)].forEach((resource) => {
		...
		async function initializeDI() {
			...
			ioc.serviceManager.addSingleton<IProcessLogger>(
				IProcessLogger,
				ProcessLogger
			);
			...
			mockTerminalFactory =
				TypeMoq.Mock.ofType<ITerminalServiceFactory>();
			...
			mockTerminalFactory
				...
				.callback((passedInResource) =>
					expect(passedInResource).to.be.deep.equal({ resource })
				)
			...
			const activatedEnvironmentLaunch =
				mock<IActivatedEnvironmentLaunch>();
			when(
				activatedEnvironmentLaunch.selectIfLaunchedViaActivatedEnv()
			...
			ioc.serviceManager.rebindInstance<ICondaService>(
				ICondaService,
				condaService.object
			);
			interpreterService = TypeMoq.Mock.ofType<IInterpreterService>();
			ioc.serviceManager.rebindInstance<IInterpreterService>(
				IInterpreterService,
			...
			ioc.serviceManager.addSingleton<IInterpreterPathService>(
				IInterpreterPathService,
				InterpreterPathService
			);
			ioc.serviceManager.addSingleton<IExtensions>(
				IExtensions,
				Extensions
			);
			ioc.serviceManager.addSingleton<IRandom>(IRandom, Random);
			ioc.serviceManager.addSingleton<IApplicationShell>(
				IApplicationShell,
				ApplicationShell
			);
			ioc.serviceManager.addSingleton<IClipboard>(
				IClipboard,
				ClipboardService
			);
			ioc.serviceManager.addSingleton<ICommandManager>(
				ICommandManager,
				CommandManager
			);
			ioc.serviceManager.addSingleton<IDocumentManager>(
				IDocumentManager,
				DocumentManager
			);
			ioc.serviceManager.addSingleton<IDebugService>(
				IDebugService,
				DebugService
			);
			ioc.serviceManager.addSingleton<IApplicationEnvironment>(
				IApplicationEnvironment,
			...
			ioc.serviceManager.addSingleton<IBrowserService>(
				IBrowserService,
				BrowserService
			);
			ioc.serviceManager.addSingleton<ITerminalActivator>(
				ITerminalActivator,
				TerminalActivator
			);
			...
			ioc.serviceManager.addSingleton<IExperimentService>(
				IExperimentService,
				ExperimentService
			);
			...
			ioc.serviceManager.addSingleton<IMultiStepInputFactory>(
				IMultiStepInputFactory,
				MultiStepInputFactory
			);
			ioc.serviceManager.addSingleton<IImportTracker>(
				IImportTracker,
				ImportTracker
			);
			ioc.serviceManager.addBinding(
				IImportTracker,
				IExtensionSingleActivationService
			);
			ioc.serviceManager.addSingleton<IShellDetector>(
				IShellDetector,
		...
		test("Ensure pip is supported and conda is not", async () => {
			...
			const moduleInstallers =
				ioc.serviceContainer.getAll<IModuleInstaller>(IModuleInstaller);
		...
		test(`Validate pip install arguments ${resourceTestNameSuffix}`, async () => {
			...
			interpreterService
				...
				.returns(() =>
					Promise.resolve({ envType: EnvironmentType.Unknown } as any)
				);
