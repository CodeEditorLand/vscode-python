...
suite("ProcessService Observable", () => {
	...
	test("variables can be changed after the fact", async () => {
		...
		let result = await procService.exec(
			pythonPath,
			["-c", `import os;print(os.environ.get("MY_TEST_VARIABLE"))`],
			{
		...
		result = await procService.exec(pythonPath, [
			"-c",
			`import os;print(os.environ.get("MY_TEST_VARIABLE"))`,
		]);
		expect(result).not.to.be.an("undefined", "result is undefined");
