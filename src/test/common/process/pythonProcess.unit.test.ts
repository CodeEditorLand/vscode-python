...
suite("PythonProcessService", () => {
	...
	test("execObservable should call processService.execObservable", () => {
		...
		processService
			.setup((p) => p.execObservable(pythonPath, args, options))
			.returns(() => observable);
		...
		processService.verify(
			(p) => p.execObservable(pythonPath, args, options),
			TypeMoq.Times.once()
		);
	...
	test("execModuleObservable should call processService.execObservable with the -m argument", () => {
		...
		processService
			.setup((p) => p.execObservable(pythonPath, expectedArgs, options))
			.returns(() => observable);
		const env = createPythonEnv(
			pythonPath,
			processService.object,
		...
		processService.verify(
			(p) => p.execObservable(pythonPath, expectedArgs, options),
			TypeMoq.Times.once()
		);
		expect(result).to.be.equal(
			observable,
	...
	test("exec should call processService.exec", async () => {
		...
		const stdout = "foo";
		processService
			.setup((p) => p.exec(pythonPath, args, options))
		...
		processService.verify(
			(p) => p.exec(pythonPath, args, options),
			TypeMoq.Times.once()
		);
	...
	test("execModule should call processService.exec with the -m argument", async () => {
		...
		processService.verify(
			(p) => p.exec(pythonPath, expectedArgs, options),
			TypeMoq.Times.once()
		);
		expect(result.stdout).to.be.equal(
			stdout,
	...
	test("execModule should throw an error if the module is not installed", async () => {
		...
		processService
			.setup((p) =>
				p.exec(pythonPath, ["-c", `import ${moduleName}`], {
					throwOnStdErr: true,
				})
			)
