...
suite("InstallationChannelManager - getInstallationChannel()", () => {
	...
	test("If multiple channels are returned by the resource, show quick pick of the channel names and return the selected channel installer", async () => {
		...
		appShell
			.setup((a) =>
				a.showQuickPick<typeof selection>(
					TypeMoq.It.isAny(),
					TypeMoq.It.isAny()
				)
			)
...
suite("InstallationChannelManager - getInstallationChannels()", () => {
	...
	test("If no installers are returned by serviceContainer, return an empty list", async () => {
		...
		const channel =
			await installChannelManager.getInstallationChannels(resource);
...
suite("InstallationChannelManager - showNoInstallersMessage()", () => {
	...
	[EnvironmentType.Conda, EnvironmentType.Pipenv].forEach(
		(interpreterType) => {
			[
				{
					osName: "Windows",
				...
				const expectedURL = `https://www.bing.com/search?q=Install Pip ${
					testParams.osName
				...
				test(`If \'Search for help\' is selected in error prompt, open correct URL for ${
					...
					const platformService =
						TypeMoq.Mock.ofType<IPlatformService>();
					serviceContainer
						.setup((s) =>
							s.get<IInterpreterService>(IInterpreterService)
						)
						.returns(() => interpreterService.object);
					serviceContainer
						.setup((s) =>
							s.get<IApplicationShell>(IApplicationShell)
						)
						.returns(() => appShell.object)
						.verifiable(TypeMoq.Times.once());
					serviceContainer
					...
					interpreterService
						...
						.returns(() =>
							Promise.resolve(activeInterpreter as any)
						);
					platformService
						.setup((p) => p.isWindows)
