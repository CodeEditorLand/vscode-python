...
suite("Common - Conda Installer", () => {
	...
	setup(() => {
		...
		when(serviceContainer.get<ICondaService>(ICondaService)).thenReturn(
			instance(condaService)
		);
		when(
			serviceContainer.get<IComponentAdapter>(IComponentAdapter)
		).thenReturn(instance(condaLocatorService));
		when(
	...
	test("Installer is not supported when current env is not a conda env", async () => {
		...
		when(condaLocatorService.isCondaEnvironment(pythonPath)).thenResolve(
			false
		);
	...
	test("Include path of environment", async () => {
		...
		assert.deepEqual(execInfo, {
			args: [
				"install",
				"--prefix",
				condaEnv.path.fileToCommandArgumentForPythonExt(),
				"abc",
				"-y",
			],
			execPath: condaPath,
