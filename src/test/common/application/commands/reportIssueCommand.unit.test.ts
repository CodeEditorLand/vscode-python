...
suite("Report Issue Command", () => {
	...
	test("Test if issue body is filled correctly when including all the settings", async () => {
		...
		const args: [string, { extensionId: string; issueBody: string }] =
			capture<AllCommands, { extensionId: string; issueBody: string }>(
				cmdManager.executeCommand
			).last();
	...
	test("Test if issue body is filled when only including settings which are explicitly set", async () => {
		...
		when(appEnvironment.packageJson).thenReturn(
			require(path.join(EXTENSION_ROOT_DIR, "package.json"))
		);
	...
	test("Should send telemetry event when run Report Issue Command", async () => {
		...
		sinon.assert.calledWith(
			sendTelemetryStub,
			EventName.USE_REPORT_ISSUE_COMMAND
		);
		sinon.restore();
	});
});
