...
suite("Python Settings", async () => {
	...
	setup(() => {
		...
		const persistentStateFactory = new PersistentStateFactory(
			globalMemento,
			workspaceMemento
		);
	...
	function testIfValueIsUpdated(settingName: string, value: any) {
		test(`${settingName} updated`, async () => {
			...
			expect((settings as any)[settingName]).to.be.equal(
				(expected as any)[settingName]
			);
			config.verifyAll();
		});
	}
		...
		const values = [
			{
				...
				expected: LanguageServerType.Jedi,
				default: false,
			},
			{
				ls: LanguageServerType.JediLSP,
				expected: LanguageServerType.Jedi,
				default: false,
			},
			{
				ls: LanguageServerType.Microsoft,
	...
	function testExperiments(enabled: boolean) {
		...
		for (const key of Object.keys(expected.experiments)) {
			expect((settings.experiments as any)[key]).to.be.deep.equal(
				(expected.experiments as any)[key]
			);
		}
		config.verifyAll();
	}
