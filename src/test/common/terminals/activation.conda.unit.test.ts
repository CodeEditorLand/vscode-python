...
suite("Terminal Environment Activation conda", () => {
	...
	test('Conda activation on bash uses "conda" after 4.4.0', async () => {
		...
		const expected = [
			`source ${path
				...
				.fileToCommandArgumentForPythonExt()} EnvA`,
		];
	...
	const testsForActivationUsingInterpreterPath: {
		...
		},
		{
			...
			expectedResult: [
				"path/to/activate",
				'conda activate "Env with spaces"',
			],
			isWindows: true,
		},
		{
	...
	testsForActivationUsingInterpreterPath.forEach((testParams) => {
		test(testParams.testName, async () => {
			...
			condaService
				.setup((c) =>
					c.getActivationScriptFromInterpreter(
						TypeMoq.It.isAny(),
						TypeMoq.It.isAny()
					)
				)
				.returns(() =>
					Promise.resolve({
						path: path
							.join(path.dirname(interpreterPath), "activate")
			...
			condaService
				...
				.returns(() => Promise.resolve(testParams.condaInfo));
			...
			case TerminalShellType.fish: {
				if (envName !== "") {
					expectedActivationCommand = [
						`conda activate ${envName.toCommandArgumentForPythonExt()}`,
					];
				} else {
					expectedActivationCommand = [
						`conda activate ${expectEnvActivatePath}`,
					];
				}
				break;
			}
	...
	getNamesAndValues<TerminalShellType>(TerminalShellType).forEach(
		(shellType) => {
			test(`Conda activation command for shell ${shellType.name} on (windows)`, async () => {
				...
				await testCondaActivationCommands(
					true,
					false,
					false,
					pythonPath,
	...
	async function expectCondaActivationCommand(
		...
		const activationCommands =
			await terminalHelper.getEnvironmentActivationCommands(
				TerminalShellType.bash,
				undefined
			);
		expect(activationCommands).to.deep.equal(
	...
	test("Get activation script command if environment is not a conda environment", async () => {
		...
		const mockProvider =
			TypeMoq.Mock.ofType<ITerminalActivationCommandProvider>();
		serviceContainer
			.setup((c) =>
				c.getAll(
		...
		(testParams: WindowsActivationTestParams) => {
			test(testParams.testName, async () => {
				...
				condaSrv
					.setup((c) => c.getCondaFile())
					.returns(async () =>
						path.join(testParams.basePath, "conda.exe")
					);
