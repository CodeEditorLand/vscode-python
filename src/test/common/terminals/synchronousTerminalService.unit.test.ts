...
suite("Terminal Service (synchronous)", () => {
	...
	setup(() => {
		...
		service = new SynchronousTerminalService(
			instance(fs),
	...
	suite(
		"Show, sendText and dispose should invoke corresponding methods in wrapped TerminalService",
		() => {
			...
			test("sendText should invokeSendText in wrapped TerminalService (errors should be bubbled up)", async () => {
				when(terminalService.sendText("Blah")).thenReject(
					new Error("kaboom")
				);
	...
	suite("sendCommand", () => {
		...
		test("run sendCommand in terminalService if there is no cancellation token", async () => {
			when(
				terminalService.sendCommand("cmd", deepEqual(["1", "2"]))
			).thenResolve();
		...
		test("run sendCommand in terminalService should be cancelled", async () => {
			...
			when(
				terminalService.sendCommand(anything(), anything())
			).thenResolve();
			when(
			...
			const promise = service
				.sendCommand("cmd", ["1", "2"], cancel.token)
			...
			verify(
				terminalService.sendCommand(
					...
					deepEqual([
						...
						"1",
						"2",
						tmpFile.filePath.fileToCommandArgumentForPythonExt(),
					])
				)
			).once();
		}).timeout(1_000);
