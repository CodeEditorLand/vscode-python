...
suite("Shell Detectors", () => {
	...
	test("Test Priority of detectors", async () => {
		...
		expect(
			new VSCEnvironmentShellDetector(instance(appEnv)).priority
		).to.equal(3);
		expect(
			new SettingsShellDetector(
				instance(workspaceService),
				instance(platformService)
			).priority
		).to.equal(2);
		expect(
			new UserEnvironmentShellDetector(
	...
	test("Test identification of Terminal Shells (base class method)", async () => {
		...
		shellPathsAndIdentification.forEach((shellType, shellPath) => {
			expect(
				shellDetector.identifyShellFromShellPath(shellPath)
			).to.equal(
				shellType,
	...
	test("Identify shell based on VSC Settings", async () => {
		const shellDetector = new SettingsShellDetector(
			instance(workspaceService),
			instance(platformService)
		);
		shellPathsAndIdentification.forEach((shellType, shellPath) => {
			...
			expect(
				shellDetector.identify(telemetryProperties, {} as any)
			).to.equal(
				shellType,
				`Incorrect Shell Type for path '${shellPath}'`
			);
	...
	getNamesAndValues<OSType>(OSType).forEach((os) => {
		test(`Get shell path from settings (OS ${os.name})`, async () => {
			...
			expect(shellPath).to.equal(
				os.value === OSType.Unknown ? "" : shellPathInSettings
			);
			expect(getStub.callCount).to.equal(
				os.value === OSType.Unknown ? 0 : 1
			);
	...
	test("Identify shell based on user environment variables", async () => {
		const shellDetector = new UserEnvironmentShellDetector(
			instance(currentProcess),
			instance(platformService)
		);
		shellPathsAndIdentification.forEach((shellType, shellPath) => {
