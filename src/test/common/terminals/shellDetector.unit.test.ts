...
suite("Shell Detector", () => {
	...
	getNamesAndValues<OSType>(OSType).forEach((os) => {
		...
		test(`Test identification of Terminal Shells in order of priority ${testSuffix}`, async () => {
			...
			const nameDetectorIdentify = sandbox.stub(
				TerminalNameShellDetector.prototype,
				"identify"
			);
			...
			const vscEnvDetectorIdentify = sandbox.stub(
				VSCEnvironmentShellDetector.prototype,
				"identify"
			);
			...
			const userEnvDetectorIdentify = sandbox.stub(
				UserEnvironmentShellDetector.prototype,
				"identify"
			);
			...
			const settingsDetectorIdentify = sandbox.stub(
				SettingsShellDetector.prototype,
				"identify"
			);
			...
			const vscEnvDetector = new VSCEnvironmentShellDetector(
				instance(mock(ApplicationEnvironment))
			);
			const userEnvDetector = new UserEnvironmentShellDetector(
				mock(MockProcess),
		...
		test(`Use shell provided by detector ${testSuffix}`, () => {
			...
			when(detector.identify(anything(), anything())).thenReturn(
				detectedShell
			);
		...
		test(`Use shell provided by detector with highest priority ${testSuffix}`, () => {
			...
			when(detector1.identify(anything(), anything())).thenReturn(
				TerminalShellType.tcshell
			);
			when(detector2.identify(anything(), anything())).thenReturn(
				detectedShell
			);
			when(detector3.identify(anything(), anything())).thenReturn(
		...
		test(`Fall back to detectors that can identify a shell ${testSuffix}`, () => {
			...
			when(detector4.identify(anything(), anything())).thenReturn(
				undefined
			);
			const shellDetector = new ShellDetector(instance(platformService), [
				instance(detector1),
