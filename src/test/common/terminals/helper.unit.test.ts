...
suite("Terminal Service helpers", () => {
	...
	function doSetup() {
		...
		).thenReturn(instance(condaService));
		...
		shellDetectorIdentifyTerminalShell = sinon.stub(
			ShellDetector.prototype,
		...
		[undefined, Uri.parse("a")].forEach((resource) => {
			suite(title(resource), () => {
				...
				test("Activation command must return undefined if none of the proivders support the shell", async () => {
					...
					when(
						nushellActivationProvider.isShellSupported(anything())
					).thenReturn(false);
					when(
				...
				test("Activation command must return command from Command Prompt if that is supported, and so is bash and nushell but no commands are returned", async () => {
					...
					when(
						nushellActivationProvider.getActivationCommands(
							resource,
							anything()
						)
					).thenResolve([]);
				...
				[undefined, pythonInterpreter].forEach((interpreter) => {
					test("Activation command for Shell must be empty for unknown os", async () => {
						...
						for (const item of getNamesAndValues<TerminalShellType>(
							...
							const cmd =
								await helper.getEnvironmentActivationShellCommands(
									resource,
									item.value,
									interpreter
								);
				...
				[undefined, pythonInterpreter].forEach((interpreter) => {
					[OSType.Linux, OSType.OSX, OSType.Windows].forEach(
						(osType) => {
							test(`Activation command for Shell must never use pipenv nor pyenv (${osType})`, async () => {
								...
								shellDetectorIdentifyTerminalShell.returns(
									shellToExpect
								);
								...
								when(
									nushellActivationProvider.isShellSupported(
										shellToExpect
									)
								).thenReturn(false);
								...
								verify(pythonSettings.pythonPath).times(
									interpreter ? 0 : 1
								);
								verify(
									condaService.isCondaEnvironment(pythonPath)
								).times(interpreter ? 0 : 1);
								verify(
									bashActivationProvider.isShellSupported(
										shellToExpect
									)
								...
								verify(
									pyenvActivationProvider.isShellSupported(
										anything()
									)
								).never();
								verify(
	...
	suite("Identify Terminal Shell", () => {
		...
		test("Use shell detector to identify terminal shells", () => {
			...
			expect(shellDetectorIdentifyTerminalShell.args[0]).deep.equal([
				terminal,
			]);
		});
		test("Detector passed throught constructor is used by shell detector class", () => {
			...
			when(mockDetector.identify(anything(), terminal)).thenReturn(
				expectedShell
			);
