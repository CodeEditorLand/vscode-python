...
import {
	ExperimentService,
	TargetPopulation,
} from "../../../client/common/experiments/service";
...
suite("Experimentation service", () => {
	...
	setup(() => {
		...
		when(
			...
			new PersistentState(globalMemento, dummyExperimentKey, {
				features: [],
			})
		);
	...
	function configureApplicationEnvironment(
		channel: Channel,
		version: string,
		contributes?: Record<string, unknown>
	) {
		when(appEnvironment.extensionChannel).thenReturn(channel);
		when(appEnvironment.extensionName).thenReturn(
	...
	suite("Initialization", () => {
		test("Users with a release version of the extension should be in the Public target population", () => {
			const getExperimentationServiceStub = sinon.stub(
				tasClient,
	...
	suite("Experiment value retrieval", () => {
		...
		setup(() => {
			getTreatmentVariableStub = sinon
				.stub()
