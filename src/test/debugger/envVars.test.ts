...
suite("Resolving Environment Variables when Debugging", () => {
	...
	setup(async () => {
		...
		const envParser =
			ioc.serviceContainer.get<IEnvironmentVariablesService>(
				IEnvironmentVariablesService
			);
		const pathUtils = ioc.serviceContainer.get<IPathUtils>(IPathUtils);
		mockProcess =
			ioc.serviceContainer.get<ICurrentProcess>(ICurrentProcess);
		debugEnvParser = new DebugEnvironmentVariablesHelper(
			envParser,
			mockProcess
		);
	...
	test("Confirm base environment variables are merged without overwriting when provided", async () => {
		...
		const envVars = await debugEnvParser.getEnvironmentVariables(
			args,
			baseEnvVars
		);
		expect(envVars).not.be.undefined;
