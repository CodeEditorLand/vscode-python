...
import {
	...
	debugStateKeys,
} from "../../../../client/debugger/extension/adapter/factory";
...
suite("Debugging - Adapter Factory", () => {
	...
	setup(() => {
		...
		when(interpreterService.getInterpreterDetails(pythonPath)).thenResolve(
			interpreter
		);
	...
	test("Display a message if no python interpreter is set", async () => {
		...
		await expect(promise).to.eventually.be.rejectedWith(
			"Debug Adapter Executable not provided"
		);
	...
	test('Use "debugAdapterPython" when specified', async () => {
		...
		when(
			interpreterService.getInterpreterDetails("/bin/custompy")
		).thenResolve(customInterpreter);
