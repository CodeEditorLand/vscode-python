...
suite("Smoke Test: Datascience", () => {
	...
	test("Run Cell in interactive window", async () => {
		...
		await vscode.commands
			.executeCommand<void>("jupyter.runallcells", textDocument.uri)
			.then(undefined, (err) => {
				assert.fail(
					`Something went wrong running all cells in the interactive window: ${err}`
				);
			});
	...
	test("Run Cell in native editor", async () => {
		...
		await vscode.commands
			.executeCommand<void>("jupyter.notebookeditor.runallcells")
			.then(undefined, (err) => {
				assert.fail(
					`Something went wrong running all cells in the native editor: ${err}`
				);
			});
		const checkIfFileHasBeenCreated = () => fs.pathExists(outputFile);
		await waitForCondition(
