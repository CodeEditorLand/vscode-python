...
suite("Interpreters service", () => {
	...
	setup(() => {
		...
		serviceManager.addSingletonInstance<IExperimentService>(
			IExperimentService,
			experiments.object
		);
		...
		installer
			.setup((i) => i.isInstalled(TypeMoq.It.isAny()))
		...
		reportActiveInterpreterChangedStub = sinon.stub(
			proposedApi,
	...
	test("Register the correct handler", async () => {
		...
		workspace
			.setup((w) => w.workspaceFolders)
			.returns(() => [{ uri: "" }] as any);
		let interpreterPathServiceHandler: (
			e: InterpreterConfigurationScope
		) => any | undefined = () => 0;
		documentManager
			.setup((d) =>
				d.onDidChangeActiveTextEditor(
		...
		service.initialize();
		expect(interpreterPathServiceHandler).to.not.equal(
			undefined,
			"Handler not set"
		);
	...
	test("If stored setting is an empty string, refresh the interpreter display", async () => {
		...
		configService
			...
			.returns(() => ({ pythonPath: "current path" }) as any);
		interpreterDisplay
			.setup((i) => i.refresh())
			.returns(() => Promise.resolve())
			.verifiable(TypeMoq.Times.once());
	...
	test("If stored setting is equal to current interpreter path setting, do not refresh the interpreter display", async () => {
		...
		configService
			.setup((c) => c.getSettings(resource))
			.returns(() => ({ pythonPath: "setting" }) as any);
		interpreterDisplay
			.setup((i) => i.refresh())
			.returns(() => Promise.resolve())
