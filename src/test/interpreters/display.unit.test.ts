...
suite("Interpreters Display", () => {
	...
	async function createInterpreterDisplay(
		filters: IInterpreterStatusbarVisibilityFilter[] = []
	) {
	...
	async function setupMocks(useLanguageStatus: boolean) {
		...
		if (!useLanguageStatus) {
			...
			applicationShell
				.setup((a) =>
					a.createLanguageStatusItem(
						TypeMoq.It.isAny(),
						TypeMoq.It.isValue({ language: PYTHON_LANGUAGE })
					)
				)
	...
	function setupWorkspaceFolder(resource: Uri, workspaceFolder?: Uri) {
		if (workspaceFolder) {
			...
			workspaceService
				.setup((w) =>
					w.getWorkspaceFolder(TypeMoq.It.isValue(resource))
				)
	...
	[false].forEach((useLanguageStatus) => {
		suite(
			`When ${
				useLanguageStatus ? `using language status` : "using status bar"
			}`,
			() => {
				setup(async () => {
						...
						languageStatusItem.verify(
							(s) =>
								(s.command = TypeMoq.It.isValue({
									title: InterpreterQuickPickList.browsePath
										.openButtonLabel,
									command: Commands.Set_Interpreter,
								})),
							TypeMoq.Times.once()
						);
						expect(disposableRegistry).contain(
							languageStatusItem.object
						);
					} else {
						statusBar.verify(
							(s) => (s.command = TypeMoq.It.isAny()),
							TypeMoq.Times.once()
						);
						...
						languageStatusItem.verify(
							(s) =>
								(s.text = TypeMoq.It.isValue(
									activeInterpreter.detailedDisplayName
								)!),
							TypeMoq.Times.once()
						);
						languageStatusItem.verify(
							(s) =>
								(s.detail = TypeMoq.It.isValue(
				...
				test("If interpreter is not identified then tooltip should point to python Path", async () => {
					...
					if (useLanguageStatus) {
						...
						languageStatusItem.verify(
							(s) =>
								(s.text =
									TypeMoq.It.isValue(expectedDisplayName)),
							TypeMoq.Times.once()
						);
					} else {
						statusBar.verify(
				...
				test("If interpreter file does not exist then update status bar accordingly", async () => {
					...
					fileSystem
						.setup((f) =>
							f.fileExists(TypeMoq.It.isValue(pythonPath))
						)
						.returns(() => Promise.resolve(false));
					interpreterHelper
						.setup((v) =>
							v.getInterpreterInformation(
					...
					if (useLanguageStatus) {
						...
						statusBar.verify(
							(s) =>
								(s.backgroundColor = TypeMoq.It.isValue(
									new ThemeColor(
				...
				test("Ensure we try to identify the active workspace when a resource is not provided ", async () => {
					...
					interpreterService
						.setup((i) =>
							i.getActiveInterpreter(TypeMoq.It.isValue(resource))
						)
						.returns(() => Promise.resolve(activeInterpreter))
					...
					if (useLanguageStatus) {
						...
						statusBar.verify(
							(s) =>
								(s.tooltip = TypeMoq.It.isValue(pythonPath)!),
							TypeMoq.Times.atLeastOnce()
						);
					}
				});
				suite("Visibility", () => {
					...
					test("Status bar must not be displayed if a filter is registered that needs it to be hidden", async () => {
						const filter1: IInterpreterStatusbarVisibilityFilter = {
							hidden: true,
						};
						const filter2: IInterpreterStatusbarVisibilityFilter = {
							hidden: false,
						};
