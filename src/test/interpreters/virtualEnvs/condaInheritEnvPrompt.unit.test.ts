...
suite("Conda Inherit Env Prompt", async () => {
	...
	suite("Method shouldShowPrompt()", () => {
		setup(() => {
			...
			applicationEnvironment =
				TypeMoq.Mock.ofType<IApplicationEnvironment>();
			applicationEnvironment
				.setup((a) => a.remoteName)
				.returns(() => undefined);
		...
		test("Returns false if prompt has already been shown in the current session", async () => {
			...
			workspaceService
				...
				.verifiable(TypeMoq.Times.never());
			const result =
				await condaInheritEnvPrompt.shouldShowPrompt(resource);
			expect(result).to.equal(false, "Prompt should not be shown");
			expect(
				condaInheritEnvPrompt.hasPromptBeenShownInCurrentSession
	...
	suite("Method activate()", () => {
		...
		test("Invokes initializeInBackground() in the background", async () => {
			...
			initializeInBackground = sinon.stub(
				CondaInheritEnvPrompt.prototype,
				"initializeInBackground"
			);
			initializeInBackground.callsFake(
				() => initializeInBackgroundDeferred.promise
			);
			condaInheritEnvPrompt = new CondaInheritEnvPrompt(
				interpreterService.object,
				workspaceService.object,
	...
	suite("Method initializeInBackground()", () => {
		...
		test("Show prompt if shouldShowPrompt() returns true", async () => {
			shouldShowPrompt = sinon.stub(
				CondaInheritEnvPrompt.prototype,
				"shouldShowPrompt"
			);
			shouldShowPrompt.callsFake(() => Promise.resolve(true));
			promptAndUpdate = sinon.stub(
				CondaInheritEnvPrompt.prototype,
				"promptAndUpdate"
			);
	...
	suite("Method promptAndUpdate()", () => {
		...
		setup(() => {
			...
			notificationPromptEnabled =
				TypeMoq.Mock.ofType<IPersistentState<any>>();
			platformService = TypeMoq.Mock.ofType<IPlatformService>();
			applicationEnvironment =
				TypeMoq.Mock.ofType<IApplicationEnvironment>();
			applicationEnvironment
			...
			).thenReturn(notificationPromptEnabled.object);
			condaInheritEnvPrompt = new CondaInheritEnvPrompt(
				interpreterService.object,
				workspaceService.object,
				appShell.object,
