...
import {
	...
	Interpreters,
} from "../../../../client/common/utils/localize";
import {
	IMultiStepInput,
...
suite("Set Interpreter Command", () => {
	...
	setup(() => {
		...
		pythonPathUpdater =
			TypeMoq.Mock.ofType<IPythonPathUpdaterServiceManager>();
		...
		when(
			interpreterService.triggerRefresh(anything(), anything())
		).thenResolve();
	...
	suite("Test method _pickInterpreter()", async () => {
		...
		setup(() => {
			...
			interpreterSelector
				.setup((i) =>
					i.getSuggestions(TypeMoq.It.isAny(), TypeMoq.It.isAny())
				)
				.returns(() => [item]);
			interpreterSelector
				.setup((i) =>
					i.getRecommendedSuggestion(
						TypeMoq.It.isAny(),
						TypeMoq.It.isAny()
					)
				)
			...
			pythonSettings
				...
				.returns(() => currentPythonPath);
			pythonSettings
				.setup((p) => p.defaultInterpreterPath)
				.returns(() => defaultInterpreterPath);
		...
		test("Existing state path must be removed before displaying picker", async () => {
			const state: InterpreterStateArgs = {
				path: "some path",
				workspace: undefined,
			};
			const multiStepInput =
				TypeMoq.Mock.ofType<IMultiStepInput<InterpreterStateArgs>>();
			multiStepInput
		...
		test("Picker should be displayed with expected items", async () => {
			...
			multiStepInput
				...
				.returns(() =>
					Promise.resolve(undefined as unknown as QuickPickItem)
				);
			...
			expect(actualParameters).to.not.equal(
				undefined,
				"Parameters not set"
			);
		...
		test("Picker should install python if corresponding item is selected", async () => {
			...
			multiStepInput
				...
				.returns(() =>
					Promise.resolve(
						noPythonInstalled as unknown as QuickPickItem
					)
				);
			interpreterSelector.reset();
			interpreterSelector
		...
		test("Picker should reload window if corresponding item is selected", async () => {
			...
			multiStepInput
				...
				.returns(() =>
					Promise.resolve(
						tipToReloadWindow as unknown as QuickPickItem
					)
				);
			interpreterSelector.reset();
			interpreterSelector
		...
		test("Items displayed should be grouped if no refresh is going on", async () => {
			...
			const suggestions = [
				...
				recommended,
				{
					label: EnvGroups.Workspace,
					kind: QuickPickItemKind.Separator,
				},
				interpreterItems[0],
				{
					label: EnvGroups.VirtualEnvWrapper,
					kind: QuickPickItemKind.Separator,
				},
				interpreterItems[1],
				interpreterItems[2],
			...
			delete actualParameters!.onChangeItem;
			assert.deepStrictEqual(
				actualParameters?.items,
				expectedParameters.items,
				"Params not equal"
			);
		});
		...
		test("Items displayed should be filtered out if a filter is provided", async () => {
			...
			await setInterpreterCommand._pickInterpreter(
				...
				state,
				(e) =>
					e.envType === EnvironmentType.VirtualEnvWrapper ||
					e.envType === EnvironmentType.Global
			);
		...
		test("If system variables are used in the default interpreter path, make sure they are resolved when the path is displayed", async () => {
			...
			const workspaceDefaultInterpreterPath =
				"${workspaceFolder}/defaultInterpreterPath";
			...
			const expandedPath = systemVariables.resolveAny(
				workspaceDefaultInterpreterPath
			);
			...
			pythonSettings
				.setup((p) => p.defaultInterpreterPath)
				.returns(() => workspaceDefaultInterpreterPath);
			configurationService
				.setup((x) => x.getSettings(TypeMoq.It.isAny()))
				.returns(() => pythonSettings.object);
			workspace.setup((w) => w.rootPath).returns(() => "rootPath");
		...
		test("Events to update quickpick updates the quickpick accordingly", async () => {
			...
			when(interpreterService.refreshPromise).thenReturn(
				refreshPromiseDeferred.promise
			);
			...
			expect(onChangedCallback).to.not.equal(
				undefined,
				"Callback not set"
			);
			...
			await onChangedCallback!(
				changeEvent,
				quickPick as unknown as QuickPick<QuickPickItem>
			); // Invoke callback, meaning that the items are supposed to change.
			...
			interpreterSelector
				.setup((i) =>
					i.suggestionToQuickPickItem(
						TypeMoq.It.isValue(refreshedItem.interpreter),
						undefined,
						false
					)
				)
			...
			interpreterSelector
				.setup((i) =>
					i.suggestionToQuickPickItem(
						TypeMoq.It.isValue(newItem.interpreter),
						undefined,
						false
					)
				)
		...
		test("If `Enter or browse...` option is selected, call the corresponding method with correct arguments", async () => {
			...
			multiStepInput
				.setup((i) => i.showQuickPick(TypeMoq.It.isAny()))
				.returns(() =>
					Promise.resolve(expectedEnterInterpreterPathSuggestion)
				);

			const step = await setInterpreterCommand._pickInterpreter(
				multiStepInput.object,
				state
			);
			...
			assert(
				_enterOrBrowseInterpreterPath.calledOnceWith(
					multiStepInput.object,
					{
						path: undefined,
						workspace: undefined,
	...
	suite("Test method _enterOrBrowseInterpreterPath()", async () => {
		...
		setup(() => {
			getItemsStub = sinon
				.stub(SetInterpreterCommand.prototype, "_getItems")
				.returns([]);
		});
		...
		test("Picker should be displayed with expected items", async () => {
			...
			await setInterpreterCommand._enterOrBrowseInterpreterPath(
				multiStepInput.object,
				state
			);
		...
		test("If `Browse...` option is selected on Windows, file browser is opened using expected parameters", async () => {
			...
			appShell
				.setup((a) =>
					a.showOpenDialog(expectedParams as OpenDialogOptions)
				)
				.verifiable(TypeMoq.Times.once());
			platformService.setup((p) => p.isWindows).returns(() => true);

			await setInterpreterCommand
				._enterOrBrowseInterpreterPath(multiStepInput.object, state)
				.ignoreErrors();
		...
		suite("SELECT_INTERPRETER_ENTERED_EXISTS telemetry", async () => {
			...
			setup(() => {
				sendTelemetryStub = sinon
					.stub(Telemetry, "sendTelemetryEvent")
					.callsFake(
						(eventName: EventName, _, properties: unknown) => {
			...
			test("A telemetry event should be sent after manual entry of an intepreter path", async () => {
				...
				const multiStepInput =
					TypeMoq.Mock.ofType<
						IMultiStepInput<InterpreterStateArgs>
					>();
				multiStepInput
					.setup((i) => i.showQuickPick(TypeMoq.It.isAny()))
					.returns(() => Promise.resolve("enteredPath"));
				...
				expect(existsTelemetry.eventName).to.equal(
			...
			test("A telemetry event should be sent after browsing for an interpreter", async () => {
				...
				appShell
					.setup((a) => a.showOpenDialog(expectedParams))
					.returns(() =>
						Promise.resolve([{ fsPath: "browsedPath" } as Uri])
					);
				platformService.setup((p) => p.isWindows).returns(() => false);
			...
			type DiscoveredPropertyTestValues = {
				discovered: boolean;
			...
			const discoveredPropertyTestMatrix: DiscoveredPropertyTestValues[] =
				[
					...
					{
						discovered: true,
						pathType: SelectionPathType.HomeRelative,
					},
					{
						discovered: true,
						pathType: SelectionPathType.WorkspaceRelative,
					},
			...
			for (const testValue of discoveredPropertyTestMatrix) {
				...
				} been discovered, with an interpreter path path that is ${
					testValue.pathType
				})`, async () => {
					const telemetryResult = await testDiscovered(
						testValue.discovered,
						testValue.pathType
					);

					expect(telemetryResult.properties).to.deep.equal({
						discovered: testValue.discovered,
					});
				});
			}
		});
	...
	suite("Test method setInterpreter()", async () => {
		test("Update Global settings when there are no workspaces", async () => {
			...
			multiStepInputFactory
				.setup((f) => f.create())
				.returns(() => multiStepInput as IMultiStepInput<unknown>);
			pythonPathUpdater
				.setup((p) =>
					p.updatePythonPath(
		...
		test("Update selected workspace folder settings when there is more than one workspace folder", async () => {
			...
			appShell
				.setup((s) =>
					s.showQuickPick(
						TypeMoq.It.isValue(expectedItems),
						TypeMoq.It.isAny()
					)
				)
				.returns(() =>
		...
		test("Make sure multiStepInput.run is called with the correct arguments", async () => {
			const pickInterpreter = sinon.stub(
				SetInterpreterCommand.prototype,
				"_pickInterpreter"
			);
			setInterpreterCommand = new SetInterpreterCommand(
