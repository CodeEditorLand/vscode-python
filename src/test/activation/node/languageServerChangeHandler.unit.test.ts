...
suite("Language Server - Change Handler", () => {
	...
	[
		...
		test(`Handler should do nothing if language server is ${t} and did not change`, async () => {
			...
			verify(
				appShell.showInformationMessage(anyString(), anyString())
			).never();
			verify(
				appShell.showWarningMessage(anyString(), anyString())
			).never();
	...
	test("Handler should open Pylance store page when language server changes to Pylance, Pylance is not installed and user clicks Yes", async () => {
		...
		verify(
			commands.executeCommand("extension.open", PYLANCE_EXTENSION_ID)
		).once();
		verify(
	...
	[ConfigurationTarget.Global, ConfigurationTarget.Workspace].forEach(
		(target) => {
			const targetName =
			...
			test(`Revert to Jedi with setting in ${targetName} config`, async () => {
				...
				when(workspace.getConfiguration("python")).thenReturn(
				...
				when(
					configuration.inspect<string>("languageServer")
				).thenReturn(inspection);

				handler = makeHandler(undefined);
				await handler.handleLanguageServerChange(
					LanguageServerType.Node
				);
	...
	[ConfigurationTarget.WorkspaceFolder, undefined].forEach((target) => {
		...
		test(`Revert to Jedi with ${targetName} setting does nothing`, async () => {
			...
			when(configuration.inspect<string>("languageServer")).thenReturn(
				inspection
			);
	...
	function makeHandler(
		initialLSType: LanguageServerType | undefined
	): LanguageServerChangeHandler {
