...
suite("Pylance Language Server - Interactive Window LSP Notebooks", () => {
	...
	setup(() => {
		...
		jupyterApi.registerGetNotebookUriForTextDocumentUriFunction(
			getNotebookUriFunction
		);
	});
	teardown(() => {
		middleware?.dispose();
	...
	test("Notebook-related document should be added at end of cells in notebookDocument/didOpen", async () => {
		...
		const cellCount = 2;
		const [notebookDocument, cells] = createNotebookDocument(
			getNotebookUriFunction(uri)!,
			cellCount
		);
		await middleware.notebooks.didOpen(
			notebookDocument,
			cells,
			async (_, nextCells) => {
	...
	test("Notebook-related document opened after notebook causes notebookDocument/didChange", async () => {
		...
		verify(
			languageClientMock.sendNotification(anything(), anything())
		).once();
		const message = capture(languageClientMock.sendNotification).last()[1];

		expect(message.notebookDocument.uri).to.equal(
		...
		expect(message.change.cells.structure).to.deep.equal({
			array: {
				...
				cells: [
					{
						kind: NotebookCellKind.Code,
						document: textDocument.uri.toString(),
					},
				],
			},
			didOpen: [
		...
		for (let i = 0; i < cellCount; i = i + 1) {
			cells.push({
				...
				document: createTextDocument(
					Uri.from({ scheme: "test-cell", path: `cell${i}` })
				),
				metadata: {},
				outputs: [],
				executionSummary: undefined,
