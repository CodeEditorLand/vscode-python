...
suite("Language Server Output Channel", () => {
	...
	setup(() => {
		...
		languageServerOutputChannel = new LanguageServerOutputChannel(
			appShell.object,
	...
	test("Create output channel if one does not exist before and return it", async () => {
		appShell
			.setup((a) =>
				a.createOutputChannel(OutputChannelNames.languageServer)
			)
			.returns(() => output.object)
			.verifiable(TypeMoq.Times.once());
		const { channel } = languageServerOutputChannel;
		appShell.verifyAll();
		expect(channel).to.not.equal(
			undefined,
	...
	test("Register Command to display output panel", async () => {
		...
		commandManager
			.setup((c) =>
				c.registerCommand(
					TypeMoq.It.isValue("python.viewLanguageServerOutput"),
					TypeMoq.It.isAny()
				)
			)
			.verifiable(TypeMoq.Times.once());
	...
	test("Display panel when invoking command python.viewLanguageServerOutput", async () => {
		...
		expect(cmdCallback).to.not.equal(
			undefined,
			"Command handler should not be undefined"
		);
