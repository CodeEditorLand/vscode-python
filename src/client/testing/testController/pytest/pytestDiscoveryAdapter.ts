...
export class PytestTestDiscoveryAdapter implements ITestDiscoveryAdapter {
	...
	async discoverTests(
		...
		const dataReceivedDisposable = this.testServer.onDiscoveryDataReceived(
			async (e: DataReceivedEvent) => {
		...
		const disposeDataReceiver = function (testServer: ITestServer) {
			traceInfo(
				`Disposing data receiver for ${uri.fsPath} and deleting UUID; pytest discovery.`
			);
