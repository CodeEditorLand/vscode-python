...
export class PytestTestExecutionAdapter implements ITestExecutionAdapter {
	...
	async runTests(
		...
		const dataReceivedDisposable = this.testServer.onRunDataReceived(
			(e: DataReceivedEvent) => {
		...
		const disposeDataReceiver = function (testServer: ITestServer) {
			traceInfo(
				`Disposing data receiver for ${uri.fsPath} and deleting UUID; pytest execution.`
			);
	...
	private async runTestsNew(
		...
		const fullPluginPath = path.join(
			EXTENSION_ROOT_DIR,
			relativePathToPytest
		);
		...
		try {
			...
			if (
				testArgs.filter((a) => a.startsWith("--rootdir")).length === 0
			) {
			...
			if (
				debugBool &&
				!testArgs.some((a) => a.startsWith("--capture") || a === "-s")
			) {
			...
			if (debugBool) {
				...
				const deferredTillExecClose: Deferred<void> =
