...
export class UnittestTestExecutionAdapter implements ITestExecutionAdapter {
	...
	public async runTests(
		...
		const disposedDataReceived = this.testServer.onRunDataReceived(
			(e: DataReceivedEvent) => {
				if (runInstance) {
		...
		const disposeDataReceiver = function (testServer: ITestServer) {
			traceInfo(
				`Disposing data receiver for ${uri.fsPath} and deleting UUID; unittest execution.`
			);
	...
	private async runTestsNew(
		...
		await this.testServer.sendCommand(
			options,
			env,
			runTestIdsPort.toString(),
			runInstance,
			testIds,
			() => {
				deferredTillEOT?.resolve();
