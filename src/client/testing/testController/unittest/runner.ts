...
export class UnittestRunner implements ITestsRunner {
	...
	public async runTests(
		...
		try {
			...
			testRun.runInstance.appendOutput(
	...
	private async runTest(
		...
		runInstance.appendOutput(
			`Running tests (unittest): ${testNodes
				.map((t) => t.id)
				.join(" ; ")}\r\n`
		);
		...
		this.server.on("result", (data: ITestData) => {
			...
			if (testCase && rawTestCase) {
				...
				if (
					...
					data.outcome === "failed" ||
					data.outcome === "passed-unexpected"
				) {
					const traceback = data.traceback
						? getTracebackForOutput(data.traceback)
						: "";
					const text = `${rawTestCase.rawId} Failed: ${
						data.message ?? data.outcome
					}\r\n${traceback}\r\n`;
					...
					if (sub) {
						...
						runInstance.appendOutput(
							fixLogLines(`${rawTestCase.rawId} [subtests]:\r\n`)
						);
					...
					if (data.subtest) {
						runInstance.appendOutput(
							fixLogLines(`${data.subtest} Passed\r\n`)
						);
						...
						const subtest = testController?.createTestItem(
							data.subtest,
							data.subtest
						);
						if (subtest) {
		...
		runInstance.appendOutput(
			`Total number of tests run: ${counts.total}\r\n`
		);
		runInstance.appendOutput(
			`Total number of tests passed: ${counts.passed}\r\n`
		);
		runInstance.appendOutput(
			`Total number of tests failed: ${counts.failed}\r\n`
		);
		runInstance.appendOutput(
			`Total number of tests failed with errors: ${counts.errored}\r\n`
		);
		runInstance.appendOutput(
			`Total number of tests skipped: ${counts.skipped}\r\n\r\n`
		);
		...
		subTestStats.forEach((v, k) => {
			runInstance.appendOutput(
				`Sub-tests for [${k}]: Total=${v.passed + v.failed} Passed=${
					v.passed
				} Failed=${v.failed}\r\n\r\n`
			);
		});
		...
		if (failFast) {
			runInstance.appendOutput(
				`Total number of tests skipped due to fail fast: ${
					counts.total - tested.length
				}\r\n`
			);
		}
	}
