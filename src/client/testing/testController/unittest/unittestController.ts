...
import {
	...
	unittestGetTestPattern,
	unittestGetTopLevelDirectory,
} from "./arguments";
import {
	createErrorTestItem,
			...
			try {
				...
				if (exceptions.length === 0) {
					...
					if (errorNode === undefined) {
						errorNode = createErrorTestItem(testController, {
							...
							label: `Unittest Discovery Error [${path.basename(
								workspace.uri.fsPath
							)}]`,
							error: message,
						});
				...
				testController.items.add(
					createErrorTestItem(testController, {
						...
						error: util.format(
							`${cancel} discovering unittest tests (see Output > Python):\r\n`,
							ex
						),
					})
				);
