...
export class PythonTestServer implements ITestServer, Disposable {
	private _onDataReceived: EventEmitter<DataReceivedEvent> =
		new EventEmitter<DataReceivedEvent>();
	...
	private _onRunDataReceived: EventEmitter<DataReceivedEvent> =
		new EventEmitter<DataReceivedEvent>();

	private _onDiscoveryDataReceived: EventEmitter<DataReceivedEvent> =
		new EventEmitter<DataReceivedEvent>();
			...
			socket.on("data", (data: Buffer) => {
				...
				while (buffer.length > 0) {
					try {
						...
						traceError(
							`Error reading data from buffer: ${ex} observed.`
						);
	...
	public _resolveData(buffer: Buffer): Buffer {
		try {
			const extractedJsonPayload = extractJsonPayload(
				buffer.toString(),
				this.uuids
			);
			...
			if (
				...
				traceVerbose(
					`Firing data received event,  ${extractedJsonPayload.cleanedJsonData}`
				);
				this._fireDataReceived(
					extractedJsonPayload.uuid,
					extractedJsonPayload.cleanedJsonData
				);
			} else {
				traceVerbose(
	...
	private _fireDataReceived(uuid: string, extractedJSON: string): void {
		if (
			extractedJSON.includes(`"tests":`) ||
			extractedJSON.includes(`"command_type": "discovery"`)
		) {
			...
			extractedJSON.includes(`"result":`) ||
			extractedJSON.includes(`"command_type": "execution"`)
		) {
	...
	async sendCommand(
		...
		const { uuid } = options;
		const isDiscovery =
			(testIds === undefined || testIds.length === 0) &&
			runTestIdPort === undefined;
		...
		const pythonPathCommand = [options.cwd, ...pythonPathParts].join(
			path.delimiter
		);
		...
		const execService =
			await this.executionFactory.createActivatedEnvironment(
				creationOptions
			);
		...
		try {
			if (options.debugBool) {
				...
				if (isRun) {
					...
					traceInfo(
						`Running unittests for workspace ${options.cwd} with arguments: ${args}\r\n`
					);
				} else {
					// This means it is running discovery
					traceLog(
						`Discovering unittest tests for workspace ${options.cwd} with arguments: ${args}\r\n`
					);
				}
				const deferredTillExecClose =
					createDeferred<ExecutionResult<string>>();

				let resultProc: ChildProcess | undefined;

				runInstance?.token.onCancellationRequested(() => {
					traceInfo(
						`Test run cancelled, killing unittest subprocess for workspace ${options.cwd}.`
					);
			...
			traceError(
				`Error while server attempting to run unittest command for workspace ${options.cwd}: ${ex}`
			);
			this.uuids = this.uuids.filter((u) => u !== uuid);
