...
export async function getInterpreterDataFromRegistry(
	...
	const allData = await Promise.all(
		subKeys.map((subKey) =>
			getInterpreterDataFromKey(subKey, distroOrgName)
		)
	);
...
async function getRegistryInterpretersImpl(): Promise<
	IRegistryInterpreterData[]
> {
	...
	for (const arch of ["x64", "x86"]) {
		for (const hive of [HKLM, HKCU]) {
			...
			let keys: string[] = [];
			try {
				keys = (await readRegistryKeys({ arch, hive, key: root })).map(
					(k) => k.key
				);
			} catch (ex) {
				traceError(
			...
			for (const key of keys) {
				registryData = registryData.concat(
					await getInterpreterDataFromRegistry(arch, hive, key)
				);
			}
		}
	}
