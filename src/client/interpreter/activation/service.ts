...
export class EnvironmentActivationServiceCache {
	...
	private static staticMap = new Map<
		string,
		InMemoryCache<NodeJS.ProcessEnv | undefined>
	>();

	private normalMap = new Map<
		string,
		InMemoryCache<NodeJS.ProcessEnv | undefined>
	>();
	...
	implements IEnvironmentActivationService, IDisposable
{
	...
	private readonly activatedEnvVariablesCache =
	...
	public async getActivatedEnvironmentVariablesImpl(
		...
		try {
			...
			if (!command) {
				...
				traceVerbose(
					`Activation Commands received ${activationCommands} for shell ${shellInfo.shell}`
				);
				if (
					!activationCommands ||
				...
				const activationCommand =
					fixActivationCommands(activationCommands).join(" && ");
				// In order to make sure we know where the environment output is,
				// put in a dummy echo we can look for
				const commandSeparator = [
					TerminalShellType.powershell,
					TerminalShellType.powershellCore,
				].includes(shellInfo.shellType)
					? ";"
					: "&&";
