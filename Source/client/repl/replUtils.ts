...
import {
	getMultiLineSelectionText,
	getSingleLineSelectionText,
} from "../terminals/codeExecution/helper";
 ...
 */
export async function getSelectedTextToExecute(
...
export function insertNewLineToREPLInput(
	...
	if (activeEditor) {
		...
		const newPosition = position.with(
			position.line,
			activeEditor.document.lineAt(position.line).text.length,
		);
 ...
 */
export function getExistingReplViewColumn(
