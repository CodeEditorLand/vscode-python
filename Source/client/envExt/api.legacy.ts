...
import {
	EnvironmentType,
	PythonEnvironment as PythonEnvironmentLegacy,
...
export async function ensureEnvironmentContainsPythonLegacy(
	...
	if (envType === EnvironmentType.Conda) {
		...
		if (
			packages &&
			packages.length > 0 &&
			packages.some((pkg) => pkg.name.toLowerCase() === "python")
		) {
			return;
		}
		traceInfo(
			`EnvExt: Python not found in ${envType} environment ${pythonPath}`,
		);
		traceInfo(
			`EnvExt: Installing Python in ${envType} environment ${pythonPath}`,
		);
		await api.installPackages(pythonEnv, ["python"]);
	}
}

export async function setInterpreterLegacy(
	pythonPath: string,
	uri: Uri | undefined,
): Promise<void> {
	const api = await getEnvExtApi();
...
export async function resetInterpreterLegacy(
	uri: Uri | undefined,
): Promise<void> {
	const api = await getEnvExtApi();
