...
import {
	...
	PythonEnvType,
	PythonVersion,
} from "../pythonEnvironments/base/info";
...
function getExecutable(pythonEnv: PythonEnvironment): string {
	...
	const basename = path
		.basename(pythonEnv.environmentPath.fsPath)
		.toLowerCase();
	if (
		isWindows() &&
...
class EnvExtApis implements IDiscoveryAPI, Disposable {
	...
	constructor(private envExtApi: PythonEnvironmentApi) {
		...
		this._disposables.push(
			this._onProgress,
			this._onChanged,
			this.envExtApi.onDidChangeEnvironments((e) =>
	...
	async resolveEnv(envPath?: string): Promise<PythonEnvInfo | undefined> {
		...
		const pythonEnv = await this.envExtApi.resolveEnvironment(
			Uri.file(envPath),
		);
		if (pythonEnv) {
...
export async function createEnvExtApi(
