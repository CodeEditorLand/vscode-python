...
const cachedExecutablePath: Map<string, Promise<string | undefined>> = new Map<
	string,
	Promise<string | undefined>
>();

class PythonEnvironment implements IPythonEnvironment {
	private cachedInterpreterInformation:
	...
	private async getInterpreterInformationImpl(): Promise<
		InterpreterInformation | undefined
	> {
		try {
			const python = this.getExecutionInfo();
...
function createDeps(
	...
	return {
		...
		exec: async (cmd: string, args: string[]) =>
			exec(cmd, args, { throwOnStdErr: true }),
		shellExec,
	};
}
...
export async function createPixiEnv(
	...
	const pythonArgv = pixiEnv.pixi.getRunPythonArgs(
