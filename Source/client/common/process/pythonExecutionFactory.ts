...
export class PythonExecutionFactory implements IPythonExecutionFactory {
	...
	public async create(
		...
		if (!pythonPath || pythonPath === "python") {
			...
			if (!interpreterPath || interpreterPath === "python") {
				...
				const success = await Promise.race([
					this.autoSelection
						.autoSelectInterpreter(options.resource)
						.then(() => true),
		...
		const pixiExecutionService = await this.createPixiExecutionService(
			pythonPath,
			processService,
		);
		...
		const windowsStoreInterpreterCheck =
			this.pyenvs.isMicrosoftStoreInterpreter.bind(this.pyenvs);
	...
	public async createCondaExecutionService(
		...
		const [condaEnvironment] = await Promise.all([
	...
	public async createPixiExecutionService(
		...
		const pixiEnvironment =
			await getPixiEnvironmentFromInterpreter(pythonPath);
		if (!pixiEnvironment) {
			return undefined;
		}

		const env = await createPixiEnv(
			pixiEnvironment,
			processService,
			this.fileSystem,
		);
