...
export class TerminalService implements ITerminalService, Disposable {
	...
	public dispose() {
		...
		if (
			this.executeCommandListeners &&
			this.executeCommandListeners.size > 0
		) {
	...
	public async executeCommand(
		commandLine: string,
	): Promise<ITerminalExecutedCommand | undefined> {
		...
		if (!terminal.shellIntegration) {
			const promise = new Promise<boolean>((resolve) => {
				const shellIntegrationChangeEventListener =
					this.terminalManager.onDidChangeTerminalShellIntegration(
						() => {
							this.executeCommandListeners.delete(
								shellIntegrationChangeEventListener,
							);
							resolve(true);
						},
					);
				...
				setTimeout(() => {
					this.executeCommandListeners.add(
						shellIntegrationChangeEventListener,
					);
					resolve(true);
				}, TIMEOUT_DURATION);
		...
		if (terminal.shellIntegration) {
			...
			return await new Promise((resolve) => {
				const listener =
					this.terminalManager.onDidEndTerminalShellExecution((e) => {
