...
export function createNamedPipeServer(
	...
	onConnectionCallback: (
		value: [rpc.MessageReader, rpc.MessageWriter],
	) => void,
	...
	return new Promise((resolve, reject) => {
		...
		const server = net.createServer((socket) => {
			...
			connectionCount += 1;
			traceVerbose(
				"new client is connected to the socket, connectionCount: ",
				connectionCount,
				pipeName,
			);
			socket.on("close", () => {
				...
				connectionCount -= 1;
				traceVerbose(
					"client emitted close event, connectionCount: ",
					connectionCount,
				);
				if (connectionCount <= 0) {
					// if all clients are closed, close the server
					traceVerbose(
						"connection count is <= 0, closing the server: ",
						pipeName,
					);
					server.close();
				}
		...
		const closedServerPromise = new Promise<void>(
...
export function namedPipeClient(
	name: string,
): [rpc.MessageReader, rpc.MessageWriter] {
