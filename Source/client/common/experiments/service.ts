...
export class ExperimentService implements IExperimentService {
	...
	public inExperimentSync(experiment: string): boolean {
		...
		if (
			...
			this._optOutFrom.includes(experiment)
		) {
			return false;
		}

		if (
			this._optInto.includes("All") ||
			this._optInto.includes(experiment)
		) {
	...
	private logExperiments() {
		const telemetrySettings =
			this.workspaceService.getConfiguration("telemetry");
