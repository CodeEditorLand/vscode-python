 ...
 */
export class LspInteractiveWindowMiddlewareAddon
	implements Middleware, Disposable
{
	constructor(
	...
	private notebookDocumentMap: Map<string, NotebookDocument> = new Map<
		string,
		NotebookDocument
	>();
	...
	private unlinkedTextDocumentMap: Map<string, TextDocument> = new Map<
		string,
		TextDocument
	>();
	...
	public async didOpen(
		...
		try {
			...
			await this.getClient()?.sendNotification(
