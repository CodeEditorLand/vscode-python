...
import {
	ensureEnvironmentContainsPythonLegacy,
	getActiveInterpreterLegacy,
} from "../envExt/api.legacy";
...
export class InterpreterService implements Disposable, IInterpreterService {
	...
	private readonly didChangeInterpreterEmitter = new EventEmitter<
		Uri | undefined
	>();

	private readonly didChangeInterpreterInformation =
		new EventEmitter<PythonEnvironment>();
	...
	public initialize(): void {
			...
			implements IInterpreterStatusbarVisibilityFilter
		{
			...
			public readonly interpreterVisibilityEmitter =
			...
			this.onDidChangeInterpreters((e): void => {
				...
				if (interpreter) {
					...
					for (const {
						path,
						workspaceFolder,
					} of this.activeInterpreterPaths.values()) {
