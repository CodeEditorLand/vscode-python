...
function isInterpreterQuickPickItem(
	item: QuickPickType,
): item is IInterpreterQuickPickItem {
	return "interpreter" in item;
}

function isSpecialQuickPickItem(
	item: QuickPickType,
): item is ISpecialQuickPickItem {
...
export class SetInterpreterCommand
	extends BaseInterpreterSelectorCommand
	implements IInterpreterQuickPick
{
	...
	public async _pickInterpreter(
		...
		const preserveOrderWhenFiltering =
		...
		const title =
			params?.title === null
				? undefined
				: (params?.title ??
					InterpreterQuickPickList.browsePath.openButtonLabel);

		const buttons: QuickInputButtonSetup[] = [
			{
				button: this.refreshButton,
				callback: (quickpickInput) => {
					this.refreshCallback(quickpickInput, {
						isButton: true,
		...
		>({
			...
			activeItem: (quickPick) =>
				this.getActiveItem(state.workspace, quickPick), // Use a promise here to ensure quickpick is initialized synchronously.
			matchOnDetail: true,
		...
		if (selection === undefined) {
			...
			this.commandManager
				.executeCommand(Commands.InstallPython)
				.then(noop, noop);
	...
	public _getItems(
		...
		if (params?.showCreateEnvironment) {
			suggestions.push(this.createEnvironmentSuggestion, {
				label: "",
				kind: QuickPickItemKind.Separator,
			});
		}

		suggestions.push(this.manualEntrySuggestion, {
			label: "",
			kind: QuickPickItemKind.Separator,
		});

		const defaultInterpreterPathSuggestion =
			this.getDefaultInterpreterPathSuggestion(resource);

		if (defaultInterpreterPathSuggestion) {
			suggestions.push(defaultInterpreterPathSuggestion);
		}

		const interpreterSuggestions = this.getSuggestions(
			resource,
			filter,
			params,
		);
	...
	private getSuggestions(
		...
		if (
			recommended &&
			items[0].interpreter.id === recommended.interpreter.id
		) {
			items.shift();
		}

		return getGroupedQuickPickItems(
			items,
			recommended,
	...
	private async getActiveItem(
		...
		const activeInterpreterItem = suggestions.find(
			(i) =>
				isInterpreterQuickPickItem(i) &&
				i.interpreter.id === interpreter?.id,
		);
		...
		const firstInterpreterSuggestion = suggestions.find((s) =>
			isInterpreterQuickPickItem(s),
		);
		...
		const noPythonInstalledItem = suggestions.find(
			(i) =>
				isSpecialQuickPickItem(i) &&
				i.label === this.noPythonInstalled.label,
		);

		return noPythonInstalledItem ?? suggestions[0];
	}

	private getDefaultInterpreterPathSuggestion(
		resource: Resource,
	): ISpecialQuickPickItem | undefined {
		const config = this.workspaceService.getConfiguration(
			"python",
			resource,
		);
		...
		const defaultInterpreterPathValue = systemVariables.resolveAny(
			config.get<string>("defaultInterpreterPath"),
		);

		if (
			defaultInterpreterPathValue &&
			defaultInterpreterPathValue !== "python"
		) {
			return {
	...
	private updateQuickPickItems(
		...
		quickPick.items = this.getUpdatedItems(
			quickPick.items,
			event,
			resource,
			filter,
		...
		const activeItem = activeItemBeforeUpdate
			? quickPick.items.find((item) => {
					if (
						...
						isInterpreterQuickPickItem(activeItemBeforeUpdate)
					) {
						return (
							item.interpreter.id ===
							activeItemBeforeUpdate.interpreter.id
						);
					}

					if (
						isSpecialQuickPickItem(item) &&
						isSpecialQuickPickItem(activeItemBeforeUpdate)
					) {
	...
	private getUpdatedItems(
		...
		if (env) {
			envIndex = updatedItems.findIndex(
				(item) =>
					isInterpreterQuickPickItem(item) &&
					item.interpreter.id === env.id,
			);
		}

		if (event.new) {
			const newSuggestion =
				this.interpreterSelector.suggestionToQuickPickItem(
					event.new,
					resource,
					!areItemsGrouped,
				);
			...
			if (envIndex === -1) {
				const noPyIndex = updatedItems.findIndex(
					(item) =>
						isSpecialQuickPickItem(item) &&
						item.label === this.noPythonInstalled.label,
				);
				...
				const tryReloadIndex = updatedItems.findIndex(
					(item) =>
						isSpecialQuickPickItem(item) &&
						item.label === this.tipToReloadWindow.label,
				);
	...
	private finalizeItems(
		...
		const interpreterSuggestions = this.interpreterSelector.getSuggestions(
			resource,
			true,
		);
		...
		if (!r) {
			if (interpreterSuggestions.length) {
				...
				items.forEach((item, i) => {
					if (
						isInterpreterQuickPickItem(item) &&
						isProblematicCondaEnvironment(item.interpreter)
					) {
						if (!items[i].label.includes(Octicons.Warning)) {
							items[i].label =
								`${Octicons.Warning} ${items[i].label}`;

							items[i].tooltip =
								InterpreterQuickPickList.condaEnvWithoutPythonTooltip;
						}
					}
				});
			} else {
				...
				if (
					...
					!items.some(
						(i) =>
							isSpecialQuickPickItem(i) &&
							i.label === this.tipToReloadWindow.label,
					)
				) {
	...
	private setRecommendedItem(
		...
		const areItemsGrouped = items.find(
			(item) =>
				isSeparatorItem(item) && item.label === EnvGroups.Recommended,
		);
		...
		const index = items.findIndex(
			(item) =>
				isInterpreterQuickPickItem(item) &&
				item.interpreter.id === recommended.interpreter.id,
		);
	...
	public async _enterOrBrowseInterpreterPath(
		...
		if (typeof selection === "string") {
			...
			selection &&
			selection.label === InterpreterQuickPickList.browsePath.label
		) {
			sendTelemetryEvent(
				EventName.SELECT_INTERPRETER_ENTER_CHOICE,
				undefined,
			...
			const uris = await this.applicationShell.showOpenDialog({
				filters: this.platformService.isWindows
					? filtersObject
					: undefined,
			...
			if (uris && uris.length > 0) {
				...
				this.sendInterpreterEntryTelemetry(
					state.path!,
					state.workspace,
				);
			} else {
	...
	public async setInterpreter(): Promise<void> {
		...
		const interpreterState: InterpreterStateArgs = {
			path: undefined,
			workspace: wkspace,
		};
	...
	public async getInterpreterViaQuickPick(
		...
		await multiStep.run(
			(input, s) => this._pickInterpreter(input, s, filter, params),
			interpreterState,
		);
	...
	private sendInterpreterEntryTelemetry(
		selection: string,
		workspace: Resource,
	): void {
		...
		const expandedPaths = suggestions.map((s) => {
			const suggestionPath = isInterpreterQuickPickItem(s)
				? s.interpreter.path
				: "";
...
function getGroupedQuickPickItems(
	...
	if (recommended) {
		updatedItems.push(
			{ label: EnvGroups.Recommended, kind: QuickPickItemKind.Separator },
			recommended,
		);
	}
	...
	for (const item of items) {
		previousGroup = addSeparatorIfApplicable(
			updatedItems,
			item,
			workspacePath,
			previousGroup,
...
function addSeparatorIfApplicable(
	...
	if (!previousGroup) {
		...
		previousGroup =
			lastItem && isInterpreterQuickPickItem(lastItem)
