...
@injectable()
export class InterpreterLocatorProgressStatusBarHandler
	implements IExtensionSingleActivationService
{
		...
		this.pyenvs.onProgress(
			(event) => {
				if (event.stage === ProgressReportStage.discoveryStarted) {
					...
					event.stage === ProgressReportStage.discoveryFinished
				) {
					this.hideProgress();
				}
			},
	...
	private createProgress() {
		const progressOptions: ProgressOptions = {
			...
			title: `[${
				this.isFirstTimeLoadingInterpreters
					? Interpreters.discovering
