...
} from "./base/locators/common/nativePythonUtils";
import {
	createPythonWatcher,
...
function validEnv(nativeEnv: NativeEnvInfo): boolean {
	if (nativeEnv.prefix === undefined && nativeEnv.executable === undefined) {
		traceError(
			`Invalid environment [native]: ${JSON.stringify(nativeEnv)}`,
		);
		return false;
	}
	return true;
...
function isSubDir(pathToCheck: string | undefined, parents: string[]): boolean {
	return parents.some((prefix) => {
		if (pathToCheck) {
			return path
				.normalize(pathToCheck)
				.startsWith(path.normalize(prefix));
		}
		return false;
	});
}
...
function getName(
	...
	if (nativeEnv.prefix && envType === PythonEnvType.Conda) {
		...
		const workspaces = (getWorkspaceFolders() ?? []).map(
			(wf) => wf.uri.fsPath,
		);
		...
		if (
			condaEnvDirs.length > 0 &&
			isSubDir(nativeEnv.prefix, condaEnvDirs)
		) {
...
function toPythonEnvInfo(
	...
	const executable =
		nativeEnv.executable ?? makeExecutablePath(nativeEnv.prefix);
	return {
		name,
...
class NativePythonEnvironments implements IDiscoveryAPI, Disposable {
	...
	getRefreshPromise(
		_options?: GetRefreshEnvironmentsOptions,
	...
	private processNative(
		native: NativeEnvInfo | NativeEnvManagerInfo,
	): string | undefined {
	...
	private processEnv(native: NativeEnvInfo): string | undefined {
		...
		try {
			const version = native.version
				? parseVersion(native.version)
				: undefined;

			if (
				categoryToKind(native.kind) === PythonEnvKind.Conda &&
				!native.executable
			) {
	...
	private processEnvManager(native: NativeEnvManagerInfo) {
		...
		switch (tool) {
			case "conda":
				traceLog(
					`Conda environment manager found at: ${native.executable}`,
				);
			...
			case "poetry":
				traceLog(
					`Poetry environment manager found at: ${native.executable}`,
				);
				break;
			default:
	...
	async resolveEnv(envPath?: string): Promise<PythonEnvInfo | undefined> {
		...
		if (native) {
			if (
				native.kind === NativePythonEnvironmentKind.Conda &&
				this._condaEnvDirs.length === 0
			) {
	...
	private initializeWatcher(): void {
		...
		if (home) {
			watcher.watchPath(
				Uri.file(path.join(home, ".conda", "environments.txt")),
			);
		}
	}
	...
	private async workspaceEventHandler(
		e: PythonWorkspaceEnvEvent,
	): Promise<void> {
		if (
			e.type === FileChangeType.Created ||
			e.type === FileChangeType.Changed
...
export function createNativeEnvironmentsApi(
