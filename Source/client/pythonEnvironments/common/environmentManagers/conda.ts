...
export class Conda {
	 ...
	 */
	private static condaPromise = new Map<
		string | undefined,
		Promise<Conda | undefined>
	>();

	private condaInfoCached = new Map<
		string | undefined,
	...
	public static setConda(condaPath: string): void {
		Conda.condaPromise.set(
			undefined,
			Promise.resolve(new Conda(condaPath)),
		);
	}
	...
	private static async locate(
		...
		for await (const condaPath of getCandidates()) {
			...
			try {
				...
				if (
					getOSType() === OSType.Windows &&
					(isTestExecution() || condaPath !== customCondaPath)
				) {
