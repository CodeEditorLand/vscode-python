...
import {
	executeCommand,
	registerCommand,
} from "../../common/vscodeApis/commandApis";
import { IInterpreterQuickPick } from "../../interpreter/configuration/types";
import {
	getCreationEvents,
	handleCreateEnvironmentCommand,
...
class CreateEnvironmentProviders {
	...
	public add(provider: CreateEnvironmentProvider) {
		if (
			this._createEnvProviders.filter((p) => p.id === provider.id)
				.length > 0
		) {
			throw new Error(
...
const _createEnvironmentProviders: CreateEnvironmentProviders =
