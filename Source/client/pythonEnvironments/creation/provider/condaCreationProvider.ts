...
function getCondaEnvFromOutput(output: string): string | undefined {
	try {
		const envPath = output
			...
			.filter(
				(s) =>
					s.startsWith(CONDA_ENV_CREATED_MARKER) ||
					s.startsWith(CONDA_ENV_EXISTING_MARKER),
			)[0];
...
async function createEnvironment(
	...
	const workspaceStep = new MultiStepNode(
		...
		async (context?: MultiStepAction) => {
			try {
				workspace = (await pickWorkspaceFolder(undefined, context)) as
					| WorkspaceFolder
					| undefined;
			} catch (ex) {
				if (
					ex === MultiStepAction.Back ||
					ex === MultiStepAction.Cancel
				) {
					return ex;
				}
				throw ex;
			...
			if (workspace === undefined) {
				traceError(
					"Workspace was not selected or found for creating conda environment.",
				);
	...
	const existingEnvStep = new MultiStepNode(
		...
		async (context?: MultiStepAction) => {
			if (workspace && context === MultiStepAction.Continue) {
				try {
					existingCondaAction =
						await pickExistingCondaAction(workspace);
					return MultiStepAction.Continue;
				} catch (ex) {
					if (
						ex === MultiStepAction.Back ||
	...
	const versionStep = new MultiStepNode(
		...
		async (context) => {
			if (
				...
				existingCondaAction === ExistingCondaAction.UseExisting
			) {
	...
	if (workspace) {
		if (existingCondaAction === ExistingCondaAction.Recreate) {
			...
			if (
				await deleteEnvironment(workspace, getExecutableCommand(conda))
			) {
				sendTelemetryEvent(EventName.ENVIRONMENT_DELETE, undefined, {
					environmentType: "conda",
					status: "deleted",
			...
			return {
				path: getPrefixCondaEnvPath(workspace),
		...
		async (
			...
			try {
				...
				if (workspace) {
					...
					throw new Error(
						"Failed to create conda environment. See Output > Python for more info.",
					);
				} else {
					throw new Error(
						"A workspace is needed to create conda environment",
					);
				}
			} catch (ex) {
				traceError(ex);
