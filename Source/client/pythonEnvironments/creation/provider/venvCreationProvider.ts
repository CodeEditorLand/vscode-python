...
function generateCommandArgs(
	...
	if (installInfo) {
		...
		const requirements = installInfo
			.filter((i) => i.installType === "requirements")
			.map((i) => i.installItem);
			...
			async (context?: MultiStepAction) => {
				if (workspace) {
					if (
							...
							interpreter =
								await this.interpreterQuickPick.getInterpreterViaQuickPick(
									...
									(i: PythonEnvironment) =>
										...
										i.type === undefined, // only global intepreters
									{
						...
						existingVenvAction === ExistingVenvAction.UseExisting
					) {
			...
			async (context?: MultiStepAction) => {
				if (workspace && installPackages) {
					if (existingVenvAction !== ExistingVenvAction.UseExisting) {
						try {
							installInfo =
								await pickPackagesToInstall(workspace);
						} catch (ex) {
							if (
								ex === MultiStepAction.Back ||
								ex === MultiStepAction.Cancel
