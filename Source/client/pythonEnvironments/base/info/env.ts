...
function buildEnvDisplayString(
	...
	const shouldDisplayKind =
		getAllDetails ||
		env.searchLocation ||
		globallyInstalledEnvKinds.includes(env.kind);
...
export function areSameEnv(
	...
	if (
		getEnvID(leftFilename, leftInfo.location) ===
		getEnvID(rightFilename, rightInfo.location)
	) {
		...
		if (isSameDirectory) {
			...
			if (leftVersion && rightVersion) {
				if (
					...
					areSimilarVersions(leftVersion, rightVersion)
				) {
					return true;
				}
			}
