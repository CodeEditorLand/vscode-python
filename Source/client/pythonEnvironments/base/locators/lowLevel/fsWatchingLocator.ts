...
export enum FSWatcherKind {
	Global = 0, // Watcher observes a global location such as ~/.envs, %LOCALAPPDATA%/Microsoft/WindowsApps.
	Workspace = 1, // Watchers observes directory in the user's currently open workspace.
}

type DirUnwatchableReason =
...
export abstract class FSWatchingLocator extends LazyResourceBasedLocator {
	...
	protected async initWatchers(): Promise<void> {
		...
		if (
			this.watcherKind === FSWatcherKind.Global &&
			!isWatchingAFile(this.creationOptions)
		) {
