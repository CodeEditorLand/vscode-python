...
} from "../lowLevel/customVirtualEnvLocator";
import {
	NativePythonTelemetry,
	sendNativeTelemetry,
} from "./nativePythonTelemetry";
...
export function isNativeEnvInfo(
	info: NativeEnvInfo | NativeEnvManagerInfo,
): info is NativeEnvInfo {
...
export interface NativePythonFinder extends Disposable {
	...
	): AsyncIterable<NativeEnvInfo | NativeEnvManagerInfo>;
	/**
...
class NativePythonFinderImpl
	extends DisposableBase
	implements NativePythonFinder
{
	private readonly connection: rpc.MessageConnection;

	private firstRefreshResults:
		| undefined
		| (() => AsyncGenerator<NativeEnvInfo, void, unknown>);

	private readonly outputChannel = this._register(
		createLogOutputChannel("Python Locator", { log: true }),
	);
	...
	public async resolve(executable: string): Promise<NativeEnvInfo> {
		...
		const environment = await this.connection.sendRequest<NativeEnvInfo>(
			"resolve",
			{
				executable,
			},
		...
		this.outputChannel.info(
			`Resolved Python Environment ${environment.executable}`,
		);
	...
	private start(): rpc.MessageConnection {
		this.outputChannel.info(
			`Starting Python Locator ${PYTHON_ENV_TOOLS_PATH} server`,
		);
		...
		try {
			...
			this.outputChannel.error(
				`Error starting Python Finder ${PYTHON_ENV_TOOLS_PATH} server`,
				ex,
			);
		}
	...
	private doRefresh(options?: NativePythonEnvironmentKind | Uri[]): {
		...
		const discovered = disposable.add(
			new EventEmitter<NativeEnvInfo | NativeEnvManagerInfo>(),
		);
		...
		disposable.add(
			this.connection.onNotification(
				...
				(data: NativeEnvInfo) => {
					this.outputChannel.info(
						`Discovered env: ${data.executable || data.prefix}`,
					);
					...
					if (data.executable && (!data.version || !data.prefix)) {
						...
						const promise = this.connection
							...
							.catch((ex) =>
								this.outputChannel.error(
									`Error in Resolving ${JSON.stringify(data)}`,
									ex,
								),
							);
...
function getPythonSettingAndUntildify<T>(
	name: string,
	scope?: Uri,
): T | undefined {
...
export function getNativePythonFinder(
	...
	if (!isTrusted()) {
		return {
			async *refresh() {
				traceError(
					"Python discovery not supported in untrusted workspace",
				);
