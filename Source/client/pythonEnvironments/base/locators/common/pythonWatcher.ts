...
class PythonWatcherImpl implements PythonWatcher {
	...
	private readonly _onDidWorkspaceEnvChanged =
		new EventEmitter<PythonWorkspaceEnvEvent>();

	private readonly _onDidGlobalEnvChanged =
		new EventEmitter<PythonGlobalEnvEvent>();

	private readonly _disposeMap: Map<string, Disposable> = new Map<
		string,
		Disposable
	>();
	...
	constructor() {
		this.disposables.push(
			this._onDidWorkspaceEnvChanged,
			this._onDidGlobalEnvChanged,
		);
	}

	onDidGlobalEnvChanged: Event<PythonGlobalEnvEvent> =
		this._onDidGlobalEnvChanged.event;

	onDidWorkspaceEnvChanged: Event<PythonWorkspaceEnvEvent> =
		this._onDidWorkspaceEnvChanged.event;
	...
	private fireWorkspaceEvent(
		...
		if (
			...
			this._onDidWorkspaceEnvChanged.fire({
				type,
				workspaceFolder: wf,
	...
	watchPath(uri: Uri, pattern?: string): void {
		...
		disposables.push(
			watcher,
			watcher.onDidChange(() => {
				this._onDidGlobalEnvChanged.fire({
