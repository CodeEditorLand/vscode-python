...
async function updateEnvUsingRegistry(env: PythonEnvInfo): Promise<void> {
	...
	if (data) {
		const versionStr =
			data.versionStr ?? data.sysVersionStr ?? data.interpreterPath;
...
async function resolveGloballyInstalledEnv(
	...
	try {
		version = env.identifiedUsingNativeLocator
			? env.version
			: parseVersionFromExecutable(executablePath);
	} catch {
		version = UNKNOWN_PYTHON_VERSION;
	}
...
async function resolveSimpleEnv(env: BasicEnvInfo): Promise<PythonEnvInfo> {
	...
	const envInfo = buildEnvInfo({
		...
		version: env.identifiedUsingNativeLocator
			? env.version
			: await getPythonVersionFromPath(executablePath),
		executable: executablePath,
		sysPrefix: env.envPath,
		location: env.envPath,
		display: env.displayName,
...
async function resolveCondaEnv(env: BasicEnvInfo): Promise<PythonEnvInfo> {
	if (env.identifiedUsingNativeLocator) {
		...
		const version =
			env.version ??
			(executable
				? await getPythonVersionFromPath(executable)
				: undefined);
	...
	const envPath =
		env.envPath ?? getEnvironmentDirFromPath(env.executablePath);
	...
	if (env.executablePath.length > 0) {
		...
		executable = await conda.getInterpreterPathForEnvironment({
			prefix: envPath,
		});
	}
...
async function resolvePyenvEnv(env: BasicEnvInfo): Promise<PythonEnvInfo> {
	...
	const envInfo = buildEnvInfo({
		...
		kind:
			env.identifiedUsingNativeLocator && env.kind
				? env.kind
				: PythonEnvKind.Pyenv,
		executable: executablePath,
		source: [],
		...
		version:
			env.version ??
			(await getPythonVersionFromPath(
				executablePath,
