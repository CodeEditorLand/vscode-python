	...
	implements IDiscoveryAPI
{
	...
	private refreshesPerQuery = new Map<
		PythonLocatorQuery | undefined,
		Deferred<void>
	>();

	/** Keeps track of scheduled refreshes other than the ongoing one for various queries. */
	private scheduledRefreshesPerQuery = new Map<
		PythonLocatorQuery | undefined,
		Promise<void>
	>();
	...
	private hasRefreshFinishedForQuery = new Map<
		PythonLocatorQuery | undefined,
		boolean
	>();
	...
	public triggerRefresh(
		...
		if (!refreshPromise) {
			if (
				options?.ifNotTriggerredAlready &&
				this.hasRefreshFinished(query)
			) {
			...
			refreshPromise = this.startRefresh(query).then(() => {
				...
				traceInfo(
					`Environment refresh took ${stopWatch.elapsedTime} milliseconds`,
				);
			});
		}
	...
	private async addEnvsToCacheForQuery(
				...
				if (isProgressEvent(event)) {
					switch (event.stage) {
						case ProgressReportStage.discoveryFinished:
							...
							listener.dispose();
							traceInfo(
								`Environments refresh finished (event): ${stopWatch.elapsedTime} milliseconds`,
							);
							break;
		...
		traceInfo(
			`Environments refresh paths discovered: ${stopWatch.elapsedTime} milliseconds`,
		);
	...
	private getRefreshPromiseForQuery(query?: PythonLocatorQuery) {
		...
		return (
			this.refreshesPerQuery.get(query)?.promise ??
			this.refreshesPerQuery.get(undefined)?.promise
		);
	}

	private hasRefreshFinished(query?: PythonLocatorQuery) {
		return (
			this.hasRefreshFinishedForQuery.get(query) ??
			this.hasRefreshFinishedForQuery.get(undefined)
		);
	}
	...
	private resolveProgressStates(query: PythonLocatorQuery | undefined) {
		...
		if (isRefreshComplete) {
			this.progress.fire({
				stage: ProgressReportStage.discoveryFinished,
			});
		}
	}
	...
	private async sendTelemetryImpl(stopWatch: StopWatch) {
		...
		for await (const data of this.nativeFinder.refresh()) {
			if (isNativeEnvInfo(data)) {
				...
				if (data.executable) {
					...
					executablesFoundByNativeLocator.add(
						data.executable.toLowerCase(),
					);
				} else if (data.prefix) {
					// Lowercase for purposes of comparison (safe).
					executablesFoundByNativeLocator.add(
				...
				// Lowercase for purposes of comparison (safe).
				(data.symlinks || []).forEach((exe) =>
					executablesFoundByNativeLocator.add(exe.toLowerCase()),
				);
			}
		}
		...
		const nativeCondaEnvs = nativeEnvs.filter(
			(e) => categoryToKind(e.kind) === PythonEnvKind.Conda,
		);
		const condaTelemetry = await getCondaTelemetry(
			this.nativeFinder,
			nativeCondaEnvs,
			nativeEnvs,
		);
		...
		await Promise.all(
			envs.map(async (env) => {
				try {
					...
					if (!exe || !(await pathExists(exe))) {
						exe = (await pathExists(env.executable.sysPrefix))
							? env.executable.sysPrefix
							: "";
					}
					if (
						env.executable.sysPrefix &&
						prefixesSeenAlready.has(env.executable.sysPrefix)
					) {
					...
					if (!executablesFoundByNativeLocator.has(exe)) {
						...
						if (
							...
							(exe.toLowerCase().includes("/envs/") ||
								exe.toLowerCase().includes("\\envs\\"))
						) {
							return;
						}
						traceError(
							`Environment ${exe} is missing from native locator`,
						);
						switch (env.kind) {
			...
			traceError("Failed to send telemetry for missing environments", ex),
		);

		const environmentsWithoutPython = envs.filter(
			(e) =>
				getEnvPath(e.executable.filename, e.location).pathType ===
				"envFolderPath",
		).length;
		const activeStateEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.ActiveState,
		).length;
		const condaEnvs = envs.filter((e) => e.kind === PythonEnvKind.Conda);
		const customEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.Custom,
		).length;
		const hatchEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.Hatch,
		).length;
		const microsoftStoreEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.MicrosoftStore,
		).length;
		const otherGlobalEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.OtherGlobal,
		).length;
		const otherVirtualEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.OtherVirtual,
		).length;
		const pipEnvEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.Pipenv,
		).length;
		const poetryEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.Poetry,
		).length;
		const pyenvEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.Pyenv,
		).length;
		const systemEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.System,
		).length;
		const unknownEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.Unknown,
		).length;
		const venvEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.Venv,
		).length;
		const virtualEnvEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.VirtualEnv,
		).length;
		const virtualEnvWrapperEnvs = envs.filter(
			(e) => e.kind === PythonEnvKind.VirtualEnvWrapper,
		).length;
		...
		condaTelemetry.condaEnvsWithoutPrefix = condaEnvs.filter(
			(e) => !e.executable.sysPrefix,
		).length;

		await Promise.all(
			condaEnvs.map(async (e) => {
				if (
					e.executable.sysPrefix &&
					!(await pathExists(e.executable.sysPrefix))
				) {
					condaTelemetry.prefixNotExistsCondaEnvs += 1;
				}
				if (
					e.executable.filename &&
					!(await isCondaEnvironment(e.executable.filename))
				) {
		...
		const nativeEnvironmentsWithoutPython = nativeEnvs.filter(
			(e) => e.executable === undefined,
		).length;
		const nativeCustomEnvs = nativeEnvs.filter(
			(e) => categoryToKind(e.kind) === PythonEnvKind.Custom,
		).length;
		...
		const nativePipEnvEnvs = nativeEnvs.filter(
			(e) => categoryToKind(e.kind) === PythonEnvKind.Pipenv,
		).length;
		const nativePoetryEnvs = nativeEnvs.filter(
			(e) => categoryToKind(e.kind) === PythonEnvKind.Poetry,
		).length;
		const nativePyenvEnvs = nativeEnvs.filter(
			(e) => categoryToKind(e.kind) === PythonEnvKind.Pyenv,
		).length;
		const nativeSystemEnvs = nativeEnvs.filter(
			(e) => categoryToKind(e.kind) === PythonEnvKind.System,
		).length;
		const nativeUnknownEnvs = nativeEnvs.filter(
			(e) => categoryToKind(e.kind) === PythonEnvKind.Unknown,
		).length;
		const nativeVenvEnvs = nativeEnvs.filter(
			(e) => categoryToKind(e.kind) === PythonEnvKind.Venv,
		).length;
	...
	private async sendNativeLocatorTelemetry(nativeEnvs: NativeEnvInfo[]) {
		...
		await Promise.all(
			nativeEnvs.map(async (e) => {
				...
				const resolvedEnv = await this.resolveEnv(e.executable).catch(
					noop,
				);
				if (!resolvedEnv) {
					return;
				...
				if (
					e.prefix &&
					resolvedEnv.executable.sysPrefix.toLowerCase() !==
						e.prefix.trim().toLowerCase()
				) {
					traceError(
...
async function getCondaTelemetry(
	...
	const userProvidedCondaExe = fsPath.normalize(
		(
			getConfiguration("python").get<string>(CONDAPATH_SETTING_KEY) || ""
		).trim(),
	);
	...
	const [info, nativeCondaInfo, condaEnvsInEnvironmentsTxt, environmentsTxt] =
		await Promise.all([
			Conda.getConda()
			...
			nativeFinder
				...
				.catch((ex) =>
					traceError(
						`Failed to get conda info from native locator`,
						ex,
					),
				),
			getCondaEnvironmentsTxt()
				.then(async (txtFile) => {
					...
					await Promise.all(
						lines.map(async (line) => {
							if (
								(await pathExists(line)) &&
								(await isCondaEnvironment(line))
							) {
								envs.push(line);
							}
						}),
				...
				.catch((ex) =>
					traceError(
						`Failed to get conda envs from environments.txt`,
						ex,
					),
				)
				.then((items) => items || []),
	...
	if (nativeCondaInfo) {
		...
		condaTelemetry.nativeCondaInfoEnvsDirs = new Set(
			nativeCondaInfo.envDirs,
		).size;
		condaTelemetry.nativeCondaRcs = new Set(nativeCondaInfo.condaRcs).size;
		condaTelemetry.userProvidedEnvFound =
			nativeCondaInfo.userProvidedEnvFound;

		const nativeEnvTxt = fsPath.normalize(
			nativeCondaInfo.environmentsTxt || "",
		);
		condaTelemetry.nativeCondaEnvTxtExists =
			nativeCondaInfo.environmentsTxtExists === true;
		condaTelemetry.nativeCondaEnvsFromTxt = (
			nativeCondaInfo.environmentsFromTxt || []
		).length;
	...
	const nativeCondaRcs = (nativeCondaInfo?.condaRcs || []).map((rc) =>
		fsPath.normalize(rc),
	);
	condaTelemetry.nativeCondaRcsNotFound = condaRcs.filter(
		(rc) => !nativeCondaRcs.includes(rc),
	).length;
	...
	const nativeCondaEnvDirs = (nativeCondaInfo?.envDirs || []).map((envDir) =>
		fsPath.normalize(envDir),
	);
	const nativeCondaEnvPrefix = nativeCondaEnvs
		.filter((e) => e.prefix)
		.map((e) => fsPath.normalize(e.prefix || ""));
	...
	condaTelemetry.missingNativeCondaEnvsFromTxt =
		condaEnvsInEnvironmentsTxt.filter(
	...
	if (
		rootPrefix &&
		(await pathExists(rootPrefix)) &&
		(await isCondaEnvironment(rootPrefix))
	) {
		...
		condaTelemetry.condaRootPrefixFoundInInfoNotInNative =
			condaInfoEnvs.some(
				(env) => env.toLowerCase() === rootPrefix.toLowerCase(),
			) &&
			!nativeCondaEnvs.some(
				(e) =>
					fsPath.normalize(e.prefix || "").toLowerCase() ===
					rootPrefix.toLowerCase(),
			);
		condaTelemetry.condaRootPrefixFoundInTxt =
			condaEnvsInEnvironmentsTxt.some(
		...
		if (condaTelemetry.condaRootPrefixFoundInInfoNotInNative) {
			...
			const rootPrefixEnvs = await flattenIterable(
				nativeFinder.refresh([Uri.file(rootPrefix)]),
			);
			...
			condaTelemetry.condaRootPrefixFoundInInfoAfterFind =
				!!rootPrefixEnv;
			condaTelemetry.condaRootPrefixFoundInInfoAfterFindKind =
				rootPrefixEnv?.kind;
			...
			condaTelemetry.condaRootPrefixFoundAsPrefixOfAnother =
				nativeEnvs.find((e) =>
					fsPath
						.normalize(e.prefix || "")
						.toLowerCase()
	...
	if (
		defaultPrefix &&
		(await pathExists(defaultPrefix)) &&
		(await isCondaEnvironment(defaultPrefix))
	) {
		...
		condaTelemetry.condaDefaultPrefixFoundInInfoNotInNative =
			condaInfoEnvs.some(
				(env) => env.toLowerCase() === defaultPrefix.toLowerCase(),
			) &&
			!nativeCondaEnvs.some(
				(e) =>
					fsPath.normalize(e.prefix || "").toLowerCase() ===
		...
		condaTelemetry.condaDefaultPrefixFoundInTxt =
			condaEnvsInEnvironmentsTxt.some(
				(e) => e.toLowerCase() === rootPrefix.toLowerCase(),
			);
		...
		if (condaTelemetry.condaDefaultPrefixFoundInInfoNotInNative) {
			// Verify we are able to discover this environment as a conda env using native finder.
			const defaultPrefixEnvs = await flattenIterable(
				nativeFinder.refresh([Uri.file(defaultPrefix)]),
			);
			// Did we find an env with the same prefix?
			...
			condaTelemetry.condaDefaultPrefixEnvsAfterFind =
				defaultPrefixEnvs.length;
			condaTelemetry.condaDefaultPrefixFoundInInfoAfterFind =
				!!defaultPrefixEnv;
			condaTelemetry.condaDefaultPrefixFoundInInfoAfterFindKind =
				defaultPrefixEnv?.kind;
			condaTelemetry.condaDefaultPrefixFoundAsAnotherKind =
				nativeEnvs.find(
					(e) =>
						fsPath.normalize(e.prefix || "").toLowerCase() ===
						defaultPrefix.toLowerCase(),
				)?.kind;
			condaTelemetry.condaDefaultPrefixFoundAsPrefixOfAnother =
				nativeEnvs.find((e) =>
					fsPath
						.normalize(e.prefix || "")
						.toLowerCase()
