...
const onDidInterpretersChangedEvent = new EventEmitter<
	EnvironmentsChangedParams[]
>();
/**
...
const onDidActiveInterpreterChangedEvent =
	new EventEmitter<ActiveEnvironmentChangedParams>();
/**
 * @deprecated Will be removed soon.
 */
...
function isEnvSame(path: string, env: PythonEnvInfo) {
	return (
		arePathsSame(path, env.location) ||
		arePathsSame(path, env.executable.filename)
	);
}
