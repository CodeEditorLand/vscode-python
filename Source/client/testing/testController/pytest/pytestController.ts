...
import {
	preparePytestArgumentsForDiscovery,
	pytestGetTestFilesAndFolders,
} from "./arguments";
...
export class PytestController implements ITestFrameworkController {
	...
	constructor(
		@inject(ITestDiscoveryHelper)
		private readonly discoveryHelper: ITestDiscoveryHelper,
		@inject(ITestsRunner)
		@named(PYTEST_PROVIDER)
		private readonly runner: ITestsRunner,
	...
	public async refreshTestData(
		...
		if (workspace) {
			...
			const testFilesAndDirectories = pytestGetTestFilesAndFolders(
				options.args
			);

			// Set arguments to use with pytest discovery script.
			const args = runAdapter([
				"discover",
				"pytest",
				"--",
				...preparePytestArgumentsForDiscovery(options),
			]);
			...
			if (testFilesAndDirectories.length === 0) {
				...
				discoveryRunOptions = testFilesAndDirectories.map(
					(testDir) => ({
						...options,
						args: [...args, testDir],
					})
			...
			try {
				...
				rawTestData = flatten(
					await Promise.all(
						discoveryRunOptions.map((o) =>
							this.discoveryHelper.runTestDiscovery(o)
						)
					)
				);
				...
				testController.items.add(
					createErrorTestItem(testController, {
						...
						label: `pytest Discovery Error [${path.basename(
							workspace.uri.fsPath
						)}]`,
						error: util.format(
			...
			if (workspaceNode) {
				...
				const newItem = createWorkspaceRootTestItem(
					testController,
					this.idToRawData,
					{
						id: root,
