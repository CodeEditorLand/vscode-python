...
export class PythonResultResolver implements ITestResultResolver {
	...
	public subTestStats: Map<string, { passed: number; failed: number }> =
		new Map();
	...
	public _resolveCoverage(
		...
		for (const [key, value] of Object.entries(payload.result)) {
			...
			const linesMissed = fileCoverageMetrics.lines_missed
				? fileCoverageMetrics.lines_missed
				: []; // undefined if no lines missed
				...
				if (testItem.outcome === "error") {
					...
					if (parentTestItem) {
						...
						if (subTestItem) {
							...
							throw new Error(
								"Unable to create new child node for subtest",
							);
						}
					} else {
