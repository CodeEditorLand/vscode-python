...
async function updateTestItemFromRawDataInternal(
	...
	if (rawId === nodeRawData[0].root || rawId === nodeRawData[0].rootid) {
		...
		await asyncForEach(
			testItemCollectionToArray(item.children),
			async (c) => {
	...
	if (rawData.length === 1) {
		...
		rawTestCaseNodes
			...
			.forEach((r) => {
				const childItem = createTestCaseItem(
					testController,
					idToRawData,
					testRoot,
					r,
	...
	if (rawCaseData.length > 1) {
		// Something is wrong, there can only be one test node with that id
		traceError(
			`Multiple (${rawCaseData.length}) raw data nodes had the same id: ${rawId}`,
		);
	}
}
...
function updateTestResultMapForSnapshot(
...
export function checkForFailedTests(
	resultMap: Map<string, TestResultState>,
): boolean {
	return (
