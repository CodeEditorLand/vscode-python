...
export function extractJsonPayload(
	...
	const rpcHeaders: ParsedRPCHeadersAndData =
		parseJsonRPCHeadersAndData(rawData);
	...
	const rpcContent: IJSONRPCData = ExtractJsonRPCData(
		payloadLength,
		rpcHeaders.remainingRawData,
	);
...
export function parseJsonRPCHeadersAndData(
	rawData: string,
): ParsedRPCHeadersAndData {
	/**
...
export function ExtractJsonRPCData(
	payloadLength: string | undefined,
	rawData: string,
): IJSONRPCData {
	/**
...
export function pythonTestAdapterRewriteEnabled(
...
export async function startTestIdsNamedPipe(
	testIds: string[],
): Promise<string> {
...
export async function writeTestIdsFile(testIds: string[]): Promise<string> {
	...
	try {
		...
		traceError(
			...
			error,
			" Attempt to use extension root dir instead",
		);
...
export async function startRunResultNamedPipe(
	...
	const server = await createNamedPipeServer(
		pipeName,
		([reader, _writer]) => {
			...
			perConnectionDisposables.push(
				...
				cancellationToken?.onCancellationRequested(() => {
					console.log(
						`Test Result named pipe ${pipeName}  cancelled`,
					);
				...
				reader.listen((data: Message) => {
					traceVerbose(
						`Test Result named pipe ${pipeName} received data`,
					);
					dataReceivedCallback(
						(data as ExecutionResultMessage)
			...
			server.serverOnClosePromise().then(() => {
				...
				traceVerbose(
					`Test Result named pipe ${pipeName} closed. Disposing of listener/s.`,
				);
				// dispose of all data listeners and cancelation listeners
				disposeOfServer();
			});
...
export async function startDiscoveryNamedPipe(
	...
	await createNamedPipeServer(pipeName, ([reader, _writer]) => {
		...
		disposables.push(
			...
			reader.listen((data: Message) => {
				traceVerbose(
					`Test Discovery named pipe ${pipeName} received data`,
				);
				callback(
...
export async function startTestIdServer(testIds: string[]): Promise<number> {
	...
	try {
		await startServer()
			.then((assignedPort) => {
				traceVerbose(
					`Server started for pytest test ids server and listening on port ${assignedPort}`,
				);
				returnPort = assignedPort;
			})
			.catch((error) => {
				traceError(
					"Error starting server for pytest test ids server:",
					error,
				);
				return 0;
			})
		...
		traceError(
			"Error starting server for pytest test ids server, cannot get port.",
		);
		return returnPort;
	}
}
...
export function populateTestTree(
	...
	if (!testRoot) {
		testRoot = testController.createTestItem(
			testTreeData.path,
			testTreeData.name,
			Uri.file(testTreeData.path),
		);
	...
	testTreeData.children.forEach((child) => {
		if (!token?.isCancellationRequested) {
			if (isTestItem(child)) {
				...
				if (!node) {
					node = testController.createTestItem(
						child.id_,
						child.name,
						Uri.file(child.path),
					);
...
function isTestItem(
	test: DiscoveredTestNode | DiscoveredTestItem,
): test is DiscoveredTestItem {
 ...
 */
export function addValueIfKeyNotExist(
	args: string[],
	key: string,
	value: string | null,
): string[] {
