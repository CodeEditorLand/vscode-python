...
export class WorkspaceTestAdapter {
	...
	public async executeTests(
		...
		if (this.executing) {
			traceError(
				"Test execution already in progress, not starting a new one.",
			);
		...
		try {
			...
			const message = util.format(
				`${cancel} ${Testing.seePythonOutput}\r\n`,
				ex,
			);
	...
	public async discoverTests(
		...
		if (this.discovering) {
			traceError(
				"Test discovery already in progress, not starting a new one.",
			);
		...
		try {
			// ** execution factory only defined for new rewrite way
			if (executionFactory !== undefined) {
				await this.discoveryAdapter.discoverTests(
					this.workspaceUri,
					executionFactory,
				);
			} else {
