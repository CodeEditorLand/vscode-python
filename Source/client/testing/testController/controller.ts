...
import {
	DelayedTrigger,
	IDelayedTrigger,
} from "../../common/utils/delayTrigger";
...
import {
	buildErrorNodeOptions,
	pythonTestAdapterRewriteEnabled,
} from "./common/utils";
...
type EventPropertyType =
	IEventNamePropertyMapping[EventName.UNITTEST_DISCOVERY_TRIGGER];
...
export class PythonTestController
	implements ITestController, IExtensionSingleActivationService
{
	...
	private readonly refreshingCompletedEvent: EventEmitter<void> =
		new EventEmitter<void>();

	private readonly refreshingStartedEvent: EventEmitter<void> =
		new EventEmitter<void>();

	private readonly runWithoutConfigurationEvent: EventEmitter<
		WorkspaceFolder[]
	...
	public readonly onRunWithoutConfiguration =
		this.runWithoutConfigurationEvent.event;
	...
	constructor(
		...
		private readonly pytest: ITestFrameworkController,
		@inject(ITestFrameworkController)
		@named(UNITTEST_PROVIDER)
		private readonly unittest: ITestFrameworkController,
		...
		@inject(ITestDebugLauncher)
		private readonly debugLauncher: ITestDebugLauncher,
		@inject(ITestOutputChannel)
		private readonly testOutputChannel: ITestOutputChannel,
			...
			if (settings.testing.pytestEnabled) {
				if (pythonTestAdapterRewriteEnabled(this.serviceContainer)) {
					...
					if (workspace && workspace.uri) {
						const testAdapter = this.testAdapters.get(
							workspace.uri,
						);

						if (testAdapter) {
							const testProviderInAdapter =
								testAdapter.getTestProvider();
							...
							traceError(
								"Unable to find test adapter for workspace.",
							);
						}
					} else {
					...
					await this.pytest.refreshTestData(
						this.testController,
						uri,
						this.refreshCancellation.token,
					);
				...
				if (pythonTestAdapterRewriteEnabled(this.serviceContainer)) {
					...
					// else use OLD test discovery mechanism
					await this.unittest.refreshTestData(
						this.testController,
						uri,
						this.refreshCancellation.token,
					);
	...
	private async resolveChildren(item: TestItem | undefined): Promise<void> {
		if (item) {
			...
			if (settings.testing.pytestEnabled) {
				return this.pytest.resolveChildren(
					this.testController,
					item,
					this.refreshCancellation.token,
				);
			...
			await Promise.all(
				workspaces.map(async (workspace) => {
					if (
						...
						traceError(
							"Cannot trigger test discovery as a valid interpreter is not selected",
						);
	...
	private async runTests(
		...
		if (request.include) {
			uniq(
				request.include.map((r) =>
					this.workspaceService.getWorkspaceFolder(r.uri),
				),
			).forEach((w) => {
				if (w) {
					workspaces.push(w);
			...
			(this.workspaceService.workspaceFolders || []).forEach((w) =>
				workspaces.push(w),
			);
		}
		...
		try {
			await Promise.all(
				workspaces.map(async (workspace) => {
					...
					(request.include ?? this.testController.items).forEach(
						(i: TestItem) => {
							const w = this.workspaceService.getWorkspaceFolder(
								i.uri,
							);
					...
					const settings = this.configSettings.getSettings(
						workspace.uri,
					);
					...
					if (testItems.length > 0) {
						const testAdapter =
							...
							(this.testAdapters.values().next()
								.value as WorkspaceTestAdapter);
						...
						if (
							...
							request.profile.loadDetailedCoverage = (
								...
								const details =
									testAdapter.resultResolver.detailedCoverageMap.get(
	...
	private watchForSettingsChanges(workspace: WorkspaceFolder): void {
		const pattern = new RelativePattern(
			workspace,
			"**/{settings.json,pytest.ini,pyproject.toml,setup.cfg}",
		);
	...
	private surfaceErrorNode(
		workspaceUri: Uri,
		message: string,
		testProvider: TestProvider,
	): void {
		let errorNode = this.testController.items.get(
			`DiscoveryError:${workspaceUri.fsPath}`,
		);

		if (errorNode === undefined) {
			const options = buildErrorNodeOptions(
				workspaceUri,
				message,
				testProvider,
			);
