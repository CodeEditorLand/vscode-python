...
export class UnittestTestExecutionAdapter implements ITestExecutionAdapter {
	...
	public async runTests(
		...
		const deferredTillServerClose: Deferred<void> =
			utils.createTestingDeferred();
	...
	private async runTestsNew(
		...
		let mutableEnv: EnvironmentVariables | undefined =
		...
		const execService =
			await executionFactory?.createActivatedEnvironment(creationOptions);
		...
		try {
			if (
				options.profileKind &&
				options.profileKind === TestRunProfileKind.Debug
			) {
				...
				if (pythonEnv) {
					traceInfo(
						`Running unittest with arguments: ${args.join(" ")} for workspace ${uri.fsPath} \r\n`,
					);
					...
					runInstance?.token.onCancellationRequested(() => {
						traceInfo(
							`Test run cancelled, killing unittest subprocess for workspace ${uri.fsPath}`,
						);
						proc.kill();
					...
					proc.stdout.on("data", (data) => {
						const out = utils.fixLogLinesNoTrailing(
							data.toString(),
						);
					...
					traceError(
						`Python Environment not found for: ${uri.fsPath}`,
					);
				}
			} else {
				// This means it is running the test
				traceInfo(
					`Running unittests for workspace ${cwd} with arguments: ${args}\r\n`,
				);
				...
				runInstance?.token.onCancellationRequested(() => {
					traceInfo(
						`Test run cancelled, killing unittest subprocess for workspace ${cwd}.`,
					);
					...
					if (code !== 0 && testIds) {
						...
						if (runInstance) {
							this.resultResolver?.resolveExecution(
								utils.createExecutionErrorPayload(
									code,
									signal,
									testIds,
									cwd,
...
function buildExecutionCommand(args: string[]): TestExecutionCommand {
	const executionScript = path.join(
		EXTENSION_ROOT_DIR,
		"python_files",
		"unittestadapter",
		"execution.py",
