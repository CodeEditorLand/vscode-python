	...
	implements ITestConfigurationManager
{
	...
	public async enable(): Promise<void> {
		...
		await Promise.all(
			UNIT_TEST_PRODUCTS.filter((prod) => prod !== this.product).map(
				(prod) =>
					this.testConfigSettingsService.disable(
						this.workspace,
						prod,
					),
			),
	...
	protected selectTestFilePattern(): Promise<string> {
		...
		const items: QuickPickItem[] = [
			...
			{
				label: "test*.py",
				description: "Python files beginning with 'test'",
			},
			{
				label: "test_*.py",
				description: "Python files beginning with 'test_'",
			},
			{
				label: "*test*.py",
