...
import {
	DebuggerTypeName,
	PythonDebuggerTypeName,
} from "../../debugger/constants";
...
export class DebugLauncher implements ITestDebugLauncher {
	...
	public async launchDebugger(
		...
		const workspaceFolder = DebugLauncher.resolveWorkspaceFolder(
			options.cwd,
		);
	...
	private static async readDebugConfig(
		...
		try {
			const configs =
				await getConfigurationsForWorkspace(workspaceFolder);

			for (const cfg of configs) {
				if (
					cfg.name &&
					(cfg.type === DebuggerTypeName ||
						cfg.type === PythonDebuggerTypeName) &&
					(cfg.request === "test" ||
						(cfg as LaunchRequestArguments).purpose?.includes(
							DebugPurpose.DebugTest,
						))
				) {
	...
	private async convertConfigToArgs(
		...
		const pythonTestAdapterRewriteExperiment =
			pythonTestAdapterRewriteEnabled(this.serviceContainer);

		const configArgs = debugConfig as LaunchRequestArguments;

		const testArgs =
			options.testProvider === "unittest"
				? options.args.filter((item) => item !== "--debug")
				: options.args;

		const script = DebugLauncher.getTestLauncherScript(
	...
	private static getTestLauncherScript(
