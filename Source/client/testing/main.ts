...
import {
	checkForFailedTests,
	updateTestResultMap,
} from "./testController/common/testItemUtilities";
...
export class UnitTestManagementService implements IExtensionActivationService {
	...
	constructor(
		...
		if (tests && !!tests.createTestController) {
			this.testController =
				serviceContainer.get<ITestController>(ITestController);
		}
	...
	private async configurationChangeHandler(
		...
		await Promise.all(
			changedWorkspaces.map((u) =>
				this.testController?.refreshTestData(u),
			),
		);
	}
	...
	private registerCommands(): void {
		...
		this.disposableRegistry.push(
			commandManager.registerCommand(
				...
				(
					_,
					_cmdSource: constants.CommandSource = constants
			...
			commandManager.registerCommand(
				...
				(
					resource?: Uri,
				): { message: string; command: Command } | undefined => {
					const wkspaceFolder =
						this.workspaceService.getWorkspaceFolder(resource) ||
