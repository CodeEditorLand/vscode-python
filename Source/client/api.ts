...
export function buildApi(
	...
	const api: PythonExtension & {
		...
		jupyter: {
			registerHooks: () =>
				jupyterIntegration.integrateWithJupyterExtension(),
		},
		tensorboard: {
			registerHooks: () =>
				tensorboardIntegration.integrateWithTensorboardExtension(),
		},
		debug: {
		...
		settings: {
			onDidChangeExecutionDetails:
				interpreterService.onDidChangeInterpreterConfiguration,
			getExecutionDetails(resource?: Resource) {
				const { pythonPath } =
					configurationService.getSettings(resource);
				// If pythonPath equals an empty string, no interpreter is set.
				return {
					execCommand: pythonPath === "" ? undefined : [pythonPath],
				};
			},
		},
		pylance: {
