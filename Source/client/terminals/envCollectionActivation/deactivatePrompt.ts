...
@injectable()
export class TerminalDeactivateLimitationPrompt
	implements IExtensionSingleActivationService
{
		...
		this.disposableRegistry.push(
			this.appShell.onDidWriteTerminalData(async (e) => {
				...
				if (
					...
					shellType === TerminalShellType.bash
				) {
					...
					const shell = await shellExec("echo $SHELL", {
						shell: this.appEnvironment.shell,
	...
	public async _notifyUsers(
		...
		const telemetrySelections: ["Edit script", "Don't show again"] = [
			"Edit script",
			"Don't show again",
		];
		const { initScript, source, destination } = scriptInfo;
		const prompts = [
			Common.editSomething.format(initScript.displayName),
