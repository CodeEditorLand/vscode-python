...
export class ShellIntegrationService implements IShellIntegrationService {
	...
	public async _isWorking(shell: string): Promise<boolean> {
		...
		const isSupposedToWork =
			isEnabled && ShellIntegrationShells.includes(shellType);
		if (!isSupposedToWork) {
			return false;
		}
		...
		const onDidExecuteTerminalCommand =
			this.appShell.onDidExecuteTerminalCommand?.bind(this.appShell);
