...
import { normCase, pathExists } from "../../common/platform/fs-paths";
...
@injectable()
export class TerminalEnvVarCollectionService
	implements IExtensionActivationService, ITerminalEnvVarCollectionService
{
	...
	private async _applyCollectionImpl(
		...
		if (useEnvExtension()) {
			...
			traceVerbose(
				"Do not activate terminal env vars as env extension is being used",
			);
			return;
		}
	...
	private async handleMicroVenv(resource: Resource) {
		try {
			...
			if (useEnvExtension()) {
				...
				traceVerbose(
					"Do not activate microvenv as env extension is being used",
				);
				return;
			}
			if (!settings.terminal.activateEnvironment) {
