...
export function registerTriggerForTerminalREPL(
	disposables: Disposable[],
): void {
	disposables.push(
		onDidStartTerminalShellExecution(
			async (e: TerminalShellExecutionStartEvent) => {
				if (
					e.execution.commandLine.isTrusted &&
					checkREPLCommand(e.execution.commandLine.value)
				) {
					sendTelemetryEvent(EventName.REPL, undefined, {
						replType: "manualTerminal",
					});
				}
			},
		),
