...
export class CodeExecutionManager implements ICodeExecutionManager {
		...
		[
			...
			this.disposableRegistry.push(
				this.commandManager.registerCommand(
					...
					async (file: Resource) => {
						traceVerbose(
							`Attempting to run Python file`,
							file?.fsPath,
						);

						if (useEnvExtension()) {
							try {
								await this.executeUsingExtension(
									file,
									cmd === Commands.Exec_In_Separate_Terminal,
								);
							} catch (ex) {
								traceError(
									"Failed to execute file in terminal",
									ex,
								);
							}
							return;
						...
						const interpreter =
							await interpreterService.getActiveInterpreter(file);
						if (!interpreter) {
							this.commandManager
								.executeCommand(
	...
	private async executeUsingExtension(
		file: Resource,
		dedicated: boolean,
	): Promise<void> {
	...
	private async executeFileInTerminal(
		...
		const executionService =
			this.serviceContainer.get<ICodeExecutionService>(
				ICodeExecutionService,
				"standard",
			);
