...
export class LanguageServerWatcher
	implements IExtensionActivationService, ILanguageServerWatcher
{
	...
	private workspaceLanguageServers: Map<
		string,
		ILanguageServerExtensionManager | undefined
	>;
	...
	private async startAndGetLanguageServer(
		...
		if (
			languageServerExtensionManager.canStartLanguageServer(interpreter)
		) {
	...
	private async onDidChangeInterpreterInformation(
		...
		while (!done) {
			...
			if (
				info.envPath === interpreter?.envPath &&
	...
	private async onDidChangeWorkspaceFolders(
		...
		if (
			...
			this.languageServerType === LanguageServerType.Jedi
		) {
