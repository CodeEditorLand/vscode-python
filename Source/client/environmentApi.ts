...
const onEnvironmentVariablesChanged =
	new EventEmitter<EnvironmentVariablesChangeEvent>();
			...
			if (e.old) {
				if (e.new) {
					...
					onEnvironmentsChanged.fire({
						type: "update",
						env: convertEnvInfoAndGetReference(e.new),
					});
					reportInterpretersChanged([
						{
							path: getEnvPath(
					...
					onEnvironmentsChanged.fire({
						type: "remove",
						env: convertEnvInfoAndGetReference(e.old),
					});
					reportInterpretersChanged([
						{
							path: getEnvPath(
...
async function resolveEnvironment(
	...
	if (
		...
		resolvedEnv.version?.minor === -1 ||
		resolvedEnv.version?.micro === -1
	) {
...
export function convertCompleteEnvInfo(
	...
	const resolvedEnv: ResolvedEnvironment = {
		...
		executable: {
			uri:
				env.executable.filename === "python"
					? undefined
					: Uri.file(env.executable.filename),
		...
		version:
			env.executable.filename === "python"
				? undefined
				: (version as ResolvedEnvironment["version"]),
		tools: tool ? [tool] : [],
	};
	return resolvedEnv;
}
