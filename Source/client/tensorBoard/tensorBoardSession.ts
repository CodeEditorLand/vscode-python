...
export class TensorBoardSession {
	...
	public async ensurePrerequisitesAreInstalled(
		...
		let [tensorboardInstallStatus, profilerPluginInstallStatus] =
			await Promise.all([
		...
		if (
			...
			(isTorchUser &&
				!needsTensorBoardInstall &&
				!needsProfilerPluginInstall) ||
		...
		[tensorboardInstallStatus, profilerPluginInstallStatus] =
			await Promise.all([
				this.installer.isProductVersionCompatible(
					Product.tensorboard,
					TensorBoardSemVerRequirement,
				...
				switch (selection?.label) {
					case TensorBoard.selectMissingSourceFile: {
						const filePickerSelection =
							await this.applicationShell.showOpenDialog({
								canSelectFiles: true,
