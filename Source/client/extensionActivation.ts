...
import { debug, DebugConfigurationProvider, languages, window } from "vscode";
...
import {
	...
	registerStartNativeReplCommand,
...
export function activateFeatures(
	...
	const interpreterService: IInterpreterService =
		ext.legacyIOC.serviceContainer.get<IInterpreterService>(
			IInterpreterService,
		);
	...
	const executionHelper =
		ext.legacyIOC.serviceContainer.get<ICodeExecutionHelper>(
			ICodeExecutionHelper,
		);
...
async function activateLegacy(
	...
	const disposables =
		serviceManager.get<IDisposableRegistry>(IDisposableRegistry);
