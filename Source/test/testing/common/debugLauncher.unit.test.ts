...
suite("Unit Tests - Debug Launcher", () => {
	...
	setup(async () => {
		environmentActivationService =
			TypeMoq.Mock.ofType<IEnvironmentActivationService>();
		environmentActivationService
			.setup((e) =>
				e.getActivatedEnvironmentVariables(TypeMoq.It.isAny())
			)
		...
		pythonTestAdapterRewriteEnabledStub = sinon.stub(
			util,
		...
		serviceContainer
			.setup((c) =>
				c.get(TypeMoq.It.isValue(IDebugEnvironmentVariablesService))
			)
	...
	function getNewResolver(configService: IConfigurationService) {
		const validator =
			TypeMoq.Mock.ofType<IInvalidPythonPathInDebuggerService>(
				undefined,
				TypeMoq.MockBehavior.Strict
			);
		validator
			.setup((v) =>
				v.validatePythonPath(
					TypeMoq.It.isAny(),
					TypeMoq.It.isAny(),
					TypeMoq.It.isAny()
				)
	...
	function setupDebugManager(
		...
		const debugArgs =
			testProvider === "unittest"
				? args.filter((item: string) => item !== "--debug")
				: args;
		...
		debugService
			...
			.returns(
				(
					_wspc: WorkspaceFolder,
					_expectedParam: DebugConfiguration
				) => {
	...
	function getTestLauncherScript(
		testProvider: TestProvider,
		pythonTestAdapterRewriteExperiment?: boolean
	) {
	...
	test("Ensure trailing commands in JSON are handled", async () => {
		...
		const configs =
			await debugLauncher.readAllDebugConfigs(workspaceFolder);
