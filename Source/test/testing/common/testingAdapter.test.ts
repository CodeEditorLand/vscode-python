...
suite("End to End Tests: test adapters", () => {
	...
	setup(async () => {
		...
		debugLauncher =
			serviceContainer.get<ITestDebugLauncher>(ITestDebugLauncher);
		testController = serviceContainer.get<TestController>(ITestController);
		envVarsService = serviceContainer.get<IEnvironmentVariablesProvider>(
			IEnvironmentVariablesProvider
		);

		// create objects that were not injected
		pythonTestServer = new PythonTestServer(
			pythonExecFactory,
			debugLauncher
		);
	...
	test("unittest discovery adapter small workspace", async () => {
		...
		await discoveryAdapter.discoverTests(workspaceUri).finally(() => {
			...
			assert.strictEqual(
				actualData.error,
				undefined,
				"Expected no errors in 'error' field"
			);
			...
			assert.strictEqual(
				callCount,
				1,
				"Expected _resolveDiscovery to be called once"
			);
		});
	});
	...
	test("pytest discovery adapter small workspace", async () => {
		...
		await discoveryAdapter
			.discoverTests(workspaceUri, pythonExecFactory)
			.finally(() => {
				// verification after discovery is complete
		...
		await executionAdapter
			...
			.finally(() => {
				...
				assert.strictEqual(
					callCount,
					1,
					"Expected _resolveExecution to be called once"
				);
			...
			try {
				...
				assert.ok(
					...
					`Expected status to be one of ${validStatuses.join(
						", "
					)}, but instead status is ${payload.status}`
				);
				assert.ok(payload.result, "Expected results to be present");
			} catch (err) {
				failureMsg = err ? (err as Error).toString() : "";
		...
		await executionAdapter
			...
			.then(() => {
				...
				for (searchString of searchStrings) {
					const count: number = (
						collectedOutput.match(new RegExp(searchString, "g")) ||
						[]
					).length;
					assert.strictEqual(
						count,
	...
	test("unittest discovery adapter seg fault error handling", async () => {
		...
		resultResolver._resolveDiscovery = async (data, _token?) => {
			...
			callCount = callCount + 1;
			traceLog(
				`unittest discovery adapter seg fault error handling \n  ${JSON.stringify(
					data
				)}`
			);
			try {
				if (data.status === "error") {
					if (data.error === undefined) {
						...
						assert.notDeepEqual(
							indexOfTest,
							-1,
							"Expected test to have a null pointer"
						);
					} else {
						assert.ok(
							data.error,
							"Expected errors in 'error' field"
						);
					}
				} else {
					const indexOfTest = JSON.stringify(data.tests).search(
						"error"
					);
					assert.notDeepEqual(
						indexOfTest,
	...
	test("pytest discovery seg fault error handling", async () => {
		...
		resultResolver._resolveDiscovery = async (data, _token?) => {
			...
			traceLog(
				`pytest discovery adapter seg fault error handling \n  ${JSON.stringify(
					data
				)}`
			);
			try {
	...
	test("unittest execution adapter seg fault error handling", async () => {
		...
		resultResolver._resolveExecution = async (data, _token?) => {
			...
			callCount = callCount + 1;
			traceLog(
				`unittest execution adapter seg fault error handling \n  ${JSON.stringify(
					data
				)}`
			);
			try {
				if (data.status === "error") {
					...
					const indexOfTest = JSON.stringify(data.result).search(
						"error"
					);
					if (indexOfTest === -1) {
						failureOccurred = true;
				...
				const indexOfTest = JSON.stringify(data).search(
					"test_seg_fault.TestSegmentationFault.test_segfault"
				);
				if (indexOfTest === -1) {
					failureOccurred = true;
		...
		await executionAdapter
			.runTests(workspaceUri, testIds, false, testRun.object)
			.finally(() => {
				assert.strictEqual(
					callCount,
					1,
	...
	test("pytest execution adapter seg fault error handling", async () => {
		...
		resultResolver._resolveExecution = async (data, _token?) => {
			// do the following asserts for each time resolveExecution is called, should be called once per test.
			console.log(
				`pytest execution adapter seg fault error handling \n  ${JSON.stringify(
					data
				)}`
			);
			callCount = callCount + 1;
