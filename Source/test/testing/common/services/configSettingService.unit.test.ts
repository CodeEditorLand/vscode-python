...
const updateMethods: (keyof Omit<
	ITestConfigSettingsService,
	"getTestEnablingSetting"
>)[] = ["updateTestArgs", "disable", "enable"];
			...
			suite(
				...
				() => {
					...
					test("Expect an exception when updating Test Arguments with workspace Uri with more than one workspace and uri does not belong to a workspace", async () => {
						...
						const promise =
							testConfigSettingsService.updateTestArgs(
								workspaceUri,
								product,
								configValue
							);
...
suite("Unit Tests - BufferedTestConfigSettingsService", () => {
	test("config changes are pushed when apply() is called", async () => {
		...
		const cfg = typeMoq.Mock.ofType<ITestConfigSettingsService>(
			undefined,
		...
		cfg.setup((c) =>
			c.disable(
				typeMoq.It.isValue(testDir),
				typeMoq.It.isValue(Product.unittest)
			)
		)
			.returns(() => Promise.resolve())
			.verifiable(typeMoq.Times.once());
