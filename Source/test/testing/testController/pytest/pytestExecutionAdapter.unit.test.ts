...
suite("pytest test execution adapter", () => {
	...
	setup(() => {
		...
		testServer
			.setup((t) =>
				t.onRunDataReceived(typeMoq.It.isAny(), typeMoq.It.isAny())
			)
			.returns(() => ({
				dispose: () => {
					/* no-body */
	...
	test("startTestIdServer called with correct testIds", async () => {
		...
		testRun
			.setup((t) => t.token)
			.returns(
				() => ({ onCancellationRequested: () => undefined }) as any
			);
		...
		testServer
			.setup((t) => t.createUUID(typeMoq.It.isAny()))
			.returns(() => uuid);
				...
				x.execObservable(
					...
					typeMoq.It.is<SpawnOptions>((options) => {
						assert.equal(
							...
							expectedExtraVariables.PYTHONPATH
						);
						assert.equal(
							options.env?.TEST_UUID,
							expectedExtraVariables.TEST_UUID
						);
						assert.equal(
							options.env?.TEST_PORT,
							expectedExtraVariables.TEST_PORT
						);
	...
	test("Debug launched correctly for pytest", async () => {
		...
		debugLauncher
			.setup((dl) =>
				dl.launchDebugger(typeMoq.It.isAny(), typeMoq.It.isAny())
			)
			.returns(async () => {
				traceInfo("stubs launch debugger");
				deferredEOT.resolve();
			});
		const utilsCreateEOTStub: sinon.SinonStub = sinon.stub(
			util,
		...
		testServer.verify(
			(x) => x.deleteUUID(typeMoq.It.isAny()),
			typeMoq.Times.once()
		);
	});
});
