...
suite("Workspace test adapter", () => {
	suite("Test discovery", () => {
		...
		setup(() => {
			...
			discoverTestsStub = sinon.stub(
				UnittestTestDiscoveryAdapter.prototype,
				"discoverTests"
			);
			sendTelemetryStub = sinon
				.stub(Telemetry, "sendTelemetryEvent")
				.callsFake(mockSendTelemetryEvent);
	...
	suite("Test execution workspace test adapter", () => {
		...
		setup(() => {
			...
			executionTestsStub = sandbox.stub(
				UnittestTestExecutionAdapter.prototype,
				"runTests"
			);
			sendTelemetryStub = sandbox
				.stub(Telemetry, "sendTelemetryEvent")
				.callsFake(mockSendTelemetryEvent);
			outputChannel = typemoq.Mock.ofType<ITestOutputChannel>();
		...
		test("If execution is already running, do not call executionAdapter.runTests again", async () => {
			...
			// Try running discovery twice
			const one = workspaceTestAdapter.executeTests(
				testController,
				runInstance.object,
				[]
			);
			const two = workspaceTestAdapter.executeTests(
				testController,
				runInstance.object,
				[]
			);
