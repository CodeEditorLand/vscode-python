...
} from "../../client/tensorBoard/constants";
...
suite("TensorBoard session creation", async () => {
	...
	setup(async () => {
		...
		experimentService =
			serviceManager.get<IExperimentService>(IExperimentService);
		const interpreterService =
			serviceManager.get<IInterpreterService>(IInterpreterService);
		sandbox
			.stub(interpreterService, "getActiveInterpreter")
			.resolves(interpreter);

		applicationShell =
			serviceManager.get<IApplicationShell>(IApplicationShell);
		commandManager = serviceManager.get<ICommandManager>(ICommandManager);
		installer = serviceManager.get<IInstaller>(IInstaller);
		workspaceConfiguration =
			workspace.getConfiguration("python.tensorBoard");
	...
	function configureStubs(
		...
		sandbox
			.stub(ImportTracker, "hasModuleImport")
			.withArgs("torch")
			.returns(hasTorchImports);
		const isProductVersionCompatible = sandbox.stub(
			installer,
	...
	async function createSession() {
		...
		assert.ok(
			session.panel?.viewColumn === ViewColumn.One,
			"Panel opened in wrong group"
		);
		assert.ok(
			session.panel?.visible,
			"Webview panel not shown on session creation golden path"
		);
		assert.ok(
			errorMessageStub.notCalled,
			"Error message shown on session creation golden path"
		);
		return session;
	}
	suite("Core functionality", async () => {
		...
		test("When webview is closed, session is killed", async () => {
			...
			assert.ok(
				daemon?.killed,
				"TensorBoard session process not killed after webview closed"
			);
		});
		test("When user selects file picker, display file picker", async () => {
			...
			sandbox
				.stub(applicationShell, "showQuickPick")
				.resolves({ label: TensorBoard.selectAnotherFolder });
			const filePickerStub = sandbox.stub(
				applicationShell,
				"showOpenDialog"
			);
			...
			assert.ok(
				filePickerStub.called,
				"User requests to select another folder and file picker was not shown"
			);
		});
		test("When user selects remote URL, display input box", async () => {
			sandbox
				.stub(applicationShell, "showQuickPick")
				.resolves({ label: TensorBoard.enterRemoteUrl });
	...
	suite("Installation prompt message", async () => {
		...
		suite("Install profiler package + upgrade tensorboard", async () => {
			async function runTest(expectTensorBoardUpgrade: boolean) {
				const installStub = sandbox
					...
					.resolves(InstallerResponse.Installed);
				await createSessionAndVerifyMessage(
					TensorBoard.installTensorBoardAndProfilerPluginPrompt
				);
				assert.ok(
					installStub.calledTwice,
					`Expected 2 installs but got ${installStub.callCount} calls`
				);
				assert.ok(
					installStub.calledWith(Product.torchProfilerInstallName)
				);
				assert.ok(
		...
		suite("Install profiler only", async () => {
			test("Has torch imports: true, is profiler package installed: false, TensorBoard installed", async () => {
				...
				assert.ok(
					session.panel?.visible,
					"Webview panel not shown, expected successful session creation"
				);
				assert.ok(
					errorMessageStub.calledOnceWith(
			...
			[false, true].forEach(async (hasTorchImports) => {
				[
					...
					const isTorchProfilerPackageInstalled =
						...
						ProductInstallStatus.Installed;
					if (
						!(hasTorchImports && !isTorchProfilerPackageInstalled)
					) {
		...
		suite("Upgrade tensorboard only", async () => {
			async function runTest() {
				...
				assert.ok(
					installStub.calledOnce,
					`Expected 1 install but got ${installStub.callCount} installs`
				);
				assert.ok(
					installStub.args[0][0] === Product.tensorboard,
					"Did not install tensorboard"
				);
				assert.ok(
					installStub.args.filter(
						(argsList) =>
							argsList[0] === Product.torchProfilerInstallName
					).length === 0,
	...
	suite("Error messages", async () => {
		test("If user cancels starting TensorBoard session, do not show error", async () => {
			...
			assert.ok(
				errorMessageStub.notCalled,
				"User canceled session start and error was shown"
			);
		});
		test("If existing install of TensorBoard is outdated and user cancels installation, do not show error", async () => {
			...
			sandbox
				...
				.resolves(ProductInstallStatus.NeedsUpgrade);
			sandbox
				.stub(installer, "install")
				.resolves(InstallerResponse.Ignore);
			...
			assert.ok(
				quickPickStub.notCalled,
				"User opted not to upgrade and we proceeded to create session"
			);
		});
		...
		test("If user does not select a logdir, do not show error", async () => {
			...
			sandbox
				.stub(applicationShell, "showQuickPick")
				.resolves({ label: TensorBoard.selectAFolder });
			sandbox
				.stub(applicationShell, "showOpenDialog")
				.resolves(undefined);
			...
			assert.ok(
				errorMessageStub.notCalled,
				"User opted not to select a logdir and error was shown"
			);
		});
		test("If starting TensorBoard times out, show error", async () => {
			...
			assert.ok(
				errorMessageStub.called,
				"TensorBoard timed out but no error was shown"
			);
		});
	...
	test("If python.tensorBoard.logDirectory is provided, do not prompt user to pick a log directory", async () => {
		...
		assert.ok(
			session.panel?.visible,
			"Expected successful session creation but webpanel not shown"
		);
		assert.ok(
			errorMessageStub.notCalled,
			"Expected successful session creation but error message was shown"
		);
		assert.ok(
	...
	suite("Jump to source", async () => {
		...
		function setupStubsForMultiStepInput() {
			...
			const multiStepFactory = serviceManager.get<IMultiStepInputFactory>(
				IMultiStepInputFactory
			);
			...
			const showQuickPickStub = sandbox
				.stub(inputInstance, "showQuickPick")
				.resolves({
			...
			const filePickerStub = sandbox
				.stub(applicationShell, "showOpenDialog")
				.resolves([Uri.file(fsPath)]);
			return [showQuickPickStub, createInputStub, filePickerStub];
		}
		test("Resolves filepaths without displaying prompt", async () => {
			const session =
				(await createSession()) as unknown as ITensorBoardSessionTestAPI;
			...
			assert.ok(
				window.activeTextEditor?.document.uri.fsPath === fsPath,
				"Wrong source file opened"
			);
			assert.ok(
				stubs.reduce(
					(prev, current) => current.notCalled && prev,
					true
				),
		...
		test("Display quickpick to user if filepath is not on disk", async () => {
			...
			assert.ok(
				stubs.reduce(
					(prev, current) => current.calledOnce && prev,
					true
				),
