...
suite("Python Environment API", () => {
	...
	setup(() => {
		...
		serviceContainer
			.setup((s) => s.get(IExtensions))
			.returns(() => extensions.object);
		serviceContainer
			.setup((s) => s.get(IInterpreterPathService))
			.returns(() => interpreterPathService.object);
		serviceContainer
			.setup((s) => s.get(IConfigurationService))
			.returns(() => configService.object);
		serviceContainer
			.setup((s) => s.get(IWorkspaceService))
			.returns(() => workspaceService.object);
		serviceContainer
			.setup((s) => s.get(IEnvironmentVariablesProvider))
			.returns(() => envVarsProvider.object);
		envVarsProvider
			.setup((e) => e.onDidEnvironmentVariablesChange)
			.returns(() => onDidChangeEnvironmentVariables.event);
		serviceContainer
			.setup((s) => s.get(IDisposableRegistry))
			.returns(() => []);

		discoverAPI
			.setup((d) => d.onProgress)
			.returns(() => onDidChangeRefreshState.event);
		discoverAPI
			.setup((d) => d.onChanged)
			.returns(() => onDidChangeEnvironments.event);
	...
	test("Provide an event to track when environment variables change", async () => {
		...
		envVarsProvider
			.setup((e) => e.getEnvironmentVariablesSync(resource))
			.returns(() => envVars);
	...
	test("Provide an event to track when active environment details change", async () => {
		...
		assert.deepEqual(events, [
			{
				id: normCasePath("path/to/environment"),
				path: "path/to/environment",
				resource: undefined,
			},
		]);
	...
	test("resolveEnvironment: valid environment (when passed as string)", async () => {
		...
		const actual = await environmentApi.resolveEnvironment(pythonPath);
		assert.deepEqual(
			(actual as EnvironmentReference).internal,
	...
	test("resolveEnvironment: valid environment (when passed as environment)", async () => {
		...
		const actual = await environmentApi.resolveEnvironment(
			convertCompleteEnvInfo(partialEnv)
		);
		assert.deepEqual(
			(actual as EnvironmentReference).internal,
			convertCompleteEnvInfo(env)
	...
	test("environments: python found", async () => {
		...
		const actual = environmentApi.known;
		const actualEnvs = actual?.map(
			(a) => (a as EnvironmentReference).internal
		);
		assert.deepEqual(
			actualEnvs?.sort((a, b) => a.id.localeCompare(b.id)),
			expectedEnvs
				.map((e) => convertEnvInfo(e))
	...
	test("Provide an event to track when list of environments change", async () => {
		...
		eventValues = events.map((e) => ({
			env: (e.env as EnvironmentReference).internal,
			type: e.type,
		}));
	...
	test("refreshInterpreters: default", async () => {
		discoverAPI
			.setup((d) =>
				d.triggerRefresh(
					undefined,
					typemoq.It.isValue({ ifNotTriggerredAlready: true })
				)
			)
			.returns(() => Promise.resolve())
			.verifiable(typemoq.Times.once());
	...
	test("refreshInterpreters: when forcing a refresh", async () => {
		discoverAPI
			.setup((d) =>
				d.triggerRefresh(
					undefined,
					typemoq.It.isValue({ ifNotTriggerredAlready: false })
				)
			)
			.returns(() => Promise.resolve())
			.verifiable(typemoq.Times.once());
