...
suite("Env file telemetry", () => {
	...
	let telemetryEvent:
		| { eventName: EventName; hasCustomEnvPath: boolean }
		| undefined;
	...
	test("Setting telemetry should be sent with hasCustomEnvPath at true if the python.envFile setting is different from the default value", () => {
		...
		assert.deepEqual(telemetryEvent, {
			eventName: EventName.ENVFILE_WORKSPACE,
			hasCustomEnvPath: true,
		});
	});
