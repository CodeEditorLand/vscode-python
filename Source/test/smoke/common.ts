...
export async function openNotebook(
	...
	const notebook = (
		vscode.window.activeTextEditor!.document as any | undefined
	)?.notebook as vscode.NotebookDocument;
	assert(notebook, "Notebook did not open");
	return notebook;
}

export async function openNotebookAndWaitForLS(
	file: string
): Promise<vscode.NotebookDocument> {
...
export async function openFileAndWaitForLS(
	...
	const textDocument = await vscode.workspace.openTextDocument(file).then(
		...
		(err) => {
			assert.fail(
				`Something went wrong opening the text document: ${err}`
			);
		}
	);
	await vscode.window
		...
		.then(undefined, (err) => {
			assert.fail(
				`Something went wrong showing the text document: ${err}`
			);
		});
...
export async function verifyExtensionIsAvailable(
	extensionId: string
): Promise<void> {
	const extension = vscode.extensions.all.find((e) => e.id === extensionId);
	assert.ok(
		extension,
		`Extension ${extensionId} not installed. ${JSON.stringify(
			vscode.extensions.all.map((e) => e.id)
		)}`
	);
