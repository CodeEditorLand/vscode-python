...
suite("Terminal - Code Execution Helper", () => {
	...
	async function ensureCodeIsNormalized(
		source: string,
		...
		processService
			...
			.returns((file, args, options) =>
				actualProcessService.execObservable.apply(
					actualProcessService,
					[file, args, options]
				)
			);
	...
	["", "1", "2", "3", "4", "5", "6", "7", "8"].forEach((fileNameSuffix) => {
		test(`Ensure code is normalized (Sample${fileNameSuffix})`, async () => {
			...
			const expectedCode = await fs.readFile(
				path.join(
					TEST_FILES_PATH,
					`sample${fileNameSuffix}_normalized_selection.py`
				),
				"utf8"
			);
	...
	test("Single line: text selection without whitespace ", async () => {
		...
		editor
			.setup((e) => e.selection)
			.returns(() => new Selection(2, 4, 2, 12));
		const textLine = TypeMoq.Mock.ofType<TextLine>();
	...
	test("Single line: partial text selection without whitespace ", async () => {
		...
		editor
			.setup((e) => e.selection)
			.returns(() => new Selection(1, 4, 1, 14));
		const textLine = TypeMoq.Mock.ofType<TextLine>();
		textLine
			.setup((t) => t.text)
			.returns(() => "if (isPrime(x) || isFibonacci(x)):");
		document
			.setup((d) => d.lineAt(TypeMoq.It.isAny()))
			.returns(() => textLine.object);
		document
	...
	test("Multi-line: text selection without whitespace ", async () => {
		...
		document
			.setup((d) => d.lineAt(TypeMoq.It.isAny()))
			.returns((r: number) => textLines[r - 1].object);
		document
	...
	test("Multi-line: text selection without whitespace and partial last line ", async () => {
		...
		document
			.setup((d) =>
				d.getText(
					new Range(
						new Position(selection.start.line, 4),
						new Position(selection.start.line, 16)
					)
				)
			)
			.returns(() => "if (m == 0):");
		document
			.setup((d) =>
				d.getText(
					new Range(
						new Position(selection.start.line, 0),
						new Position(selection.end.line, 20)
					)
				)
			)
			.returns(() => "    if (m == 0):\n        return n + 1");
	...
	test("Multi-line: partial first and last line", async () => {
		...
		document
			.setup((d) =>
				d.getText(
					new Range(
						new Position(selection.start.line, 7),
						new Position(selection.start.line, 13)
					)
				)
			)
			.returns(() => "(m > 0");
		document
			.setup((d) =>
				d.getText(
					new Range(
						new Position(selection.start.line, 0),
						new Position(selection.end.line, 19)
					)
				)
			)
			.returns(() => "    if (m > 0\n        and n == 0)");
	...
	test("File will be saved if file is dirty", async () => {
		...
		workspaceService
			.setup((w) => w.save(TypeMoq.It.isAny()))
			.returns(() => Promise.resolve(expectedSavedUri));
