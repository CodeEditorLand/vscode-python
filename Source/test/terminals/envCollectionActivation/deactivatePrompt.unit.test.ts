...
suite("Terminal Deactivation Limitation Prompt", () => {
	...
	const prompts = [
		Common.editSomething.format("~/.bashrc"),
		Common.doNotShowAgain,
	];
	const expectedMessage =
		Interpreters.terminalDeactivatePrompt.format("~/.bashrc");
	...
	setup(async () => {
		...
		when(documentManager.openTextDocument(initScriptPath)).thenReturn(
			Promise.resolve(document)
		);
		when(documentManager.onDidChangeActiveTextEditor).thenReturn(
			activeEditorEvent.event
		);
		...
		when(
			persistentStateFactory.createGlobalPersistentState(anything(), true)
		).thenReturn(instance(notificationEnabled));
	...
	test('Show notification when "deactivate" command is run when a virtual env is selected', async () => {
		...
		when(shell.showWarningMessage(expectedMessage, ...prompts)).thenResolve(
			undefined
		);
	...
	test("Edit script correctly if `Edit <script>` button is clicked", async () => {
		when(notificationEnabled.value).thenReturn(true);
		when(shell.showWarningMessage(expectedMessage, ...prompts)).thenReturn(
			Promise.resolve(prompts[0])
		);
		let fileCopied = false;
		let createdFile = false;
		sinon
			.stub(fsapi, "copyFile")
			.callsFake(async (_src: string, _dest: string) => {
				fileCopied = true;
				Promise.resolve();
			});
		...
		when(documentManager.showTextDocument(anything())).thenReturn(
	...
	test("Restart terminal if running deactivate command in the same terminal the workaround was applied to", async () => {
		...
		sinon
			.stub(fsapi, "createFile")
			.callsFake(async (_: string) => Promise.resolve());
		when(shell.withProgress(anything(), anything())).thenResolve();
		const editor = mock<TextEditor>();
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		when((editor as any).then).thenReturn(undefined);
