...
suite("Terminal Provider", () => {
	...
	setup(() => {
		...
		experimentService
			.setup((e) =>
				e.inExperimentSync(TerminalEnvVarActivation.experiment)
			)
			.returns(() => false);
		activeResourceService = TypeMoq.Mock.ofType<IActiveResourceService>();
		workspace = TypeMoq.Mock.ofType<IWorkspaceService>();
		serviceContainer
			.setup((c) => c.get(IExperimentService))
		...
		serviceContainer
			.setup((c) => c.get(IActiveResourceService))
	...
	suite("terminal.activateCurrentTerminal setting", () => {
		...
		setup(() => {
			...
			activeResourceService =
				TypeMoq.Mock.ofType<IActiveResourceService>();
		...
		test("If terminal.activateCurrentTerminal setting is set, provided terminal should be activated", async () => {
			...
			terminalActivator.verify(
				(a) =>
					a.activateEnvironmentInTerminal(
						terminal.object,
						TypeMoq.It.isAny()
					),
				TypeMoq.Times.once()
