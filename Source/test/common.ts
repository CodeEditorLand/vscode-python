...
export async function updateSetting(
	...
	const settings = vscode.workspace.getConfiguration(
		"python",
		{ uri: resource, languageId: "python" } || null
	);
...
export const resetGlobalInterpreterPathSetting = async () =>
	retryAsync(restoreGlobalInterpreterPathSetting)();
...
export async function setAutoSaveDelayInWorkspaceRoot(delayinMS: number) {
	const vscode = require("vscode") as typeof import("vscode");
	return retryAsync(setAutoSaveDelay)(
		undefined,
		vscode.ConfigurationTarget.Workspace,
		delayinMS
	);
...
async function setAutoSaveDelay(
...
export interface IExtensionTestApi
	extends PythonExtension,
...
export async function retryIfFail<T>(
	...
	throw new Error(
		"Timeout waiting for function to complete without any errors"
	);
}
 ...
 */
export class TestEventHandler<T extends void | any = any>
	implements IDisposable
{
	...
	public async assertFired(waitPeriod: number = 100): Promise<void> {
		await waitForCondition(
			async () => this.fired,
			waitPeriod,
			`${this.eventNameForErrorMessages} event not fired`
		);
	}
	public async assertFiredExactly(
		numberOfTimesFired: number,
		waitPeriod: number = 2_000
	): Promise<void> {
		await waitForCondition(
	...
	public async assertFiredAtLeast(
		numberOfTimesFired: number,
		waitPeriod: number = 2_000
	): Promise<void> {
		await waitForCondition(
...
): T[K] extends Event<infer TArgs>
	? TestEventHandler<TArgs>
	: TestEventHandler<void> {
	return new TestEventHandler(
		obj[eventName] as any,
		eventName as string,
		dispoables
	) as any;
}
