...
suite("Application Diagnostics - Pylance informational prompt", () => {
	...
	function setupMementos(version?: string, promptShown?: boolean) {
		...
		memento
			.setup((m) => m.get(PYLANCE_PROMPT_MEMENTO))
			.returns(() => promptShown);
	}
	...
	test("Should display a prompt when handling the diagnostic code", async () => {
		const diagnostic = new PylanceDefaultDiagnostic(
		...
		messageHandler
			.setup((f) =>
				f.handle(typemoq.It.isValue(diagnostic), typemoq.It.isAny())
			)
		...
		assert.deepStrictEqual(messagePrompt!.commandPrompts, [
			{ prompt: Common.ok },
		]);
	});
		...
		filterService
			.setup((f) =>
				f.shouldIgnoreDiagnostic(
					typemoq.It.isValue(DiagnosticCodes.PylanceDefaultDiagnostic)
				)
			)
