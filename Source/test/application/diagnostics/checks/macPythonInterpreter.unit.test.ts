...
suite("Application Diagnostics - Checks Mac Python Interpreter", () => {
	...
	suite("Diagnostics", () => {
		...
		test("Should not display a message if No Interpreters diagnostic has been ignored", async () => {
			...
			commandFactory
				.setup((f) =>
					f.createCommand(typemoq.It.isAny(), typemoq.It.isAny())
				)
	...
	suite("Change Handlers.", () => {
		...
		test("Diagnostics are checked with correct interpreter config uri when path changes", async () => {
			...
			const diagnosticSvc =
				new (class extends InvalidMacPythonInterpreterService {
					...
					public onDidChangeConfigurationEx = (
		...
		test("Ensure event Handler is registered correctly", async () => {
			...
			interpreterPathService
				.setup((d) =>
					d.onDidChange(typemoq.It.isAny(), typemoq.It.isAny())
				)
			...
			diagnosticService =
				new (class extends InvalidMacPythonInterpreterService {
					protected async onDidChangeConfiguration(
						_i: InterpreterConfigurationScope
					) {
						invoked = true;
					}
				})(
