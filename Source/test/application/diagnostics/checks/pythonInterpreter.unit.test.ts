...
suite("Application Diagnostics - Checks Python Interpreter", () => {
	...
	function createContainer() {
		...
		fs.setup((f) => f.fileExists(process.env.ComSpec ?? "exists")).returns(
			() => Promise.resolve(true)
		);
		serviceContainer = typemoq.Mock.ofType<IServiceContainer>();
		processService = typemoq.Mock.ofType<IProcessService>();
		const processServiceFactory =
			typemoq.Mock.ofType<IProcessServiceFactory>();
		...
		serviceContainer
			...
			.returns(() => fs.object);
		serviceContainer
			.setup((s) => s.get(typemoq.It.isValue(ICommandManager)))
		...
		interpreterPathService
			.setup((i) => i.get(typemoq.It.isAny()))
			.returns(() => "customPython");
		serviceContainer
	...
	suite("Diagnostics", () => {
		setup(() => {
			diagnosticService =
				new (class extends InvalidPythonInterpreterService {
					public _clear() {
						while (
		...
		test("Should return diagnostics if there are no interpreters and no interpreter has been explicitly set", async () => {
			...
			const diagnostics =
				await diagnosticService._manualDiagnose(undefined);
		...
		test("Handling comspec diagnostic should launch expected browser link", async () => {
			const diagnostic = new DefaultShellDiagnostic(
				DiagnosticCodes.InvalidComspecDiagnostic,
				undefined
			);
		...
		test("Handling no interpreters diagnostic should return select interpreter cmd", async () => {
			...
			expect(messagePrompt!.onClose).to.not.be.equal(
				undefined,
				"onClose handler should be set."
			);
		});
		...
		test("Handling no currently selected interpreter diagnostic should show select interpreter message", async () => {
			...
			expect(messagePrompt!.onClose).be.equal(
				undefined,
				"onClose handler should not be set."
			);
		});
		...
		test("Handling an unsupported diagnostic code should not show a message nor return a command", async () => {
			...
			diagnosticServiceMock
				.setup((f) => f.canHandle(typemoq.It.isAny()))
				.returns(() => Promise.resolve(false));
			messageHandler
				.setup((i) => i.handle(typemoq.It.isAny(), typemoq.It.isAny()))
				.callback((_d, p: MessageCommandPrompt) => p)
		...
		test("Getting command prompts for an unsupported diagnostic code should throw an error", async () => {
			const diagnostic = new InvalidLaunchJsonDebuggerDiagnostic(
