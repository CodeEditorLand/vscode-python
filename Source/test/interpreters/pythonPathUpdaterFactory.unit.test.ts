...
suite("Python Path Settings Updater", () => {
	...
	suite("Global", () => {
		...
		test("Python Path should not be updated when current pythonPath is the same", async () => {
			...
			interpreterPathService
				.setup((i) =>
					i.update(undefined, ConfigurationTarget.Global, pythonPath)
				)
				.returns(() => Promise.resolve())
				.verifiable(TypeMoq.Times.never());

			const updater =
				updaterServiceFactory.getGlobalPythonPathConfigurationService();
	...
	suite("WorkspaceFolder", () => {
		...
		test("Python Path should not be updated when current pythonPath is the same", async () => {
			...
			interpreterPathService
				...
				.verifiable(TypeMoq.Times.never());
			const updater =
				updaterServiceFactory.getWorkspaceFolderPythonPathConfigurationService(
					workspaceFolder
				);
			await updater.updatePythonPath(pythonPath);
			interpreterPathService.verifyAll();
	...
	suite("Workspace (multiroot scenario)", () => {
		...
		test("Python Path should not be updated when current pythonPath is the same", async () => {
			...
			const updater =
				updaterServiceFactory.getWorkspacePythonPathConfigurationService(
					workspaceFolder
				);
			await updater.updatePythonPath(pythonPath);
			interpreterPathService.verifyAll();
