...
suite("Virtual Environment Prompt", () => {
	...
	setup(() => {
		...
		isCreatingEnvironmentStub = sinon.stub(
			createEnvApi,
	...
	test("User is notified if interpreter exists and only python path to global interpreter is specified in settings", async () => {
		...
		when(
			componentAdapter.getWorkspaceVirtualEnvInterpreters(resource)
		).thenResolve([
			interpreter1,
			interpreter2,
		...
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		when(
			helper.getBestInterpreter(
				deepEqual([interpreter1, interpreter2] as any)
			)
		).thenReturn(interpreter2 as any);
		when(
		...
		when(
			appShell.showInformationMessage(anything(), ...prompts)
		).thenResolve();
