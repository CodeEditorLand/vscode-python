...
suite("Interpreters - Display Progress", () => {
	...
	setup(() => {
		...
		componentAdapter = {
			...
			onProgress(
				listener: (e: ProgressNotificationEvent) => any
			): Disposable {
	...
	test("Display discovering message when refreshing interpreters for the first time", async () => {
		...
		const options = capture(
			shell.withProgress as never
		).last()[0] as ProgressOptions;
		expect(options.title).to.be.equal(
			`[${Interpreters.discovering}](command:${Commands.Set_Interpreter})`
		);
	});
	...
	test("Display refreshing message when refreshing interpreters for the second time", async () => {
		...
		expect(options.title).to.be.equal(
			`[${Interpreters.refreshing}](command:${Commands.Set_Interpreter})`
		);
	});
