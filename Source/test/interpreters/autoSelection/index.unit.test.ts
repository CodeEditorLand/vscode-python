...
suite("Interpreters - Auto Selection", () => {
	...
	setup(() => {
		...
		state = mock(PersistentState) as PersistentState<
		...
		const interpreterComparer = new EnvironmentTypeComparer(
			instance(helper)
		);
	...
	suite("Test locator-based auto-selection method", () => {
		...
		setup(() => {
			...
			when(
				stateFactory.createWorkspacePersistentState<
					PythonEnvironment | undefined
				>(anyString(), undefined)
			...
			when(
				workspaceService.getWorkspaceFolderIdentifier(anything(), "")
			).thenReturn("workspaceIdentifier");
		...
		test("getInterpreters is called with ignoreCache at true if there is no value set in the workspace persistent state", async () => {
			...
			const queryState = mock(PersistentState) as PersistentState<
				boolean | undefined
			>;
			...
			when(
				stateFactory.createWorkspacePersistentState<
					boolean | undefined
				>(anyString(), undefined)
			).thenReturn(instance(queryState));
	...
	test("Initialize the store", async () => {
		...
		when(queryState.value).thenReturn(undefined);
		when(
			stateFactory.createWorkspacePersistentState<boolean | undefined>(
				anyString(),
				undefined
			)
		).thenReturn(instance(queryState));
	...
	test("Store interpreter info in state store when resource is undefined", async () => {
		...
		const interpreterInfo = { path: pythonPath } as any;
		when(
			workspaceService.getWorkspaceFolderIdentifier(undefined, anything())
		).thenReturn("");
		when(
			stateFactory.createGlobalPersistentState<
				PythonEnvironment | undefined
		...
		await autoSelectionService.initializeStore(undefined);
		await autoSelectionService.storeAutoSelectedInterpreter(
			undefined,
			interpreterInfo
		);
		const selectedInterpreter =
			autoSelectionService.getAutoSelectedInterpreter(undefined);
	...
	test("Store interpreter info in state store when resource is defined", async () => {
		...
		const selectedInterpreter =
			autoSelectionService.getAutoSelectedInterpreter(resource);
	...
	test("Store workspace interpreter info in state store", async () => {
		...
		when(
			workspaceService.getWorkspaceFolderIdentifier(
				anything(),
				anything()
			)
		).thenReturn("");
		...
		autoSelectionService
			.getAutoSelectedWorkspacePromises()
			.set("", deferred);
	...
	test("Return global value if we do not have a matching value for the resource", async () => {
		...
		when(
			workspaceService.getWorkspaceFolderIdentifier(resource, anything())
		).thenReturn("1");
		const deferred = createDeferred<void>();
		deferred.resolve();
		autoSelectionService
		...
		const selectedInterpreter =
			autoSelectionService.getAutoSelectedInterpreter(
				anotherResourceOfAnotherWorkspace
			);
