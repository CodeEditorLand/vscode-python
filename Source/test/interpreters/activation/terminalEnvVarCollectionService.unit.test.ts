...
suite("Terminal Environment Variable Collection Service", () => {
	...
	setup(() => {
		...
		when(workspaceService.getConfiguration("terminal")).thenReturn(
			instance(workspaceConfig)
		);
		when(
			workspaceConfig.get<boolean>("integrated.shellIntegration.enabled")
		).thenReturn(true);
		...
		when(context.environmentVariableCollection).thenReturn(
			instance(globalCollection)
		);
		when(globalCollection.getScoped(anything())).thenReturn(
			instance(collection)
		);
		experimentService = mock<IExperimentService>();
		when(
			experimentService.inExperimentSync(
				TerminalEnvVarActivation.experiment
			)
		).thenReturn(true);
		...
		when(
			environmentActivationService.getProcessEnvironmentVariables(
				anything(),
				anything()
			)
	...
	test("Apply activated variables to the collection on activation", async () => {
		const applyCollectionStub = sinon.stub(
		...
		when(
			applicationEnvironment.onDidChangeShell(
				anything(),
				anything(),
				anything()
			)
		...
		assert(
			applyCollectionStub.calledOnce,
			"Collection not applied on activation"
		);
	});
	...
	test("When not in experiment, do not apply activated variables to the collection and clear it instead", async () => {
		...
		assert(
			applyCollectionStub.notCalled,
			"Collection should not be applied on activation"
		);
	...
	test("If activated variables are returned for custom shell, apply it correctly to the collection", async () => {
		...
		when(
			collection.replace(anything(), anything(), anything())
		).thenResolve();
		when(collection.delete(anything())).thenResolve();

		await terminalEnvVarCollectionService._applyCollection(
			undefined,
			customShell
		);
		...
		verify(
			collection.replace("CONDA_PREFIX", "prefix/to/conda", anything())
		).once();
	});
	...
	test("If activated variables contain PS1, prefix it using shell integration", async function () {
		...
		when(collection.prepend("PS1", "(envName) ", anything())).thenCall(
			(_, _v, o) => {
				opts = o;
			}
		);
	...
	test('Prepend only "prepend portion of PATH" where applicable', async () => {
		...
		).thenResolve(processEnv);
		const prependedPart = "path/to/activate/dir:";
		const envVars: NodeJS.ProcessEnv = {
			PATH: `${prependedPart}${processEnv.PATH}`,
		};
		when(
			environmentActivationService.getActivatedEnvironmentVariables(
				anything(),
		...
		when(collection.delete(anything())).thenResolve();
		let opts: EnvironmentVariableMutatorOptions | undefined;
		when(collection.prepend("PATH", anything(), anything())).thenCall(
			(_, _v, o) => {
				opts = o;
			}
		);
	...
	test("Prepend full PATH with separator otherwise", async () => {
		...
		verify(collection.clear()).once();
		verify(
			collection.prepend("PATH", `${finalPath}${separator}`, anything())
		).once();
		verify(collection.replace("PATH", anything(), anything())).never();
		assert.deepEqual(opts, {
			applyAtProcessCreation: false,
	...
	test("Correct track that prompt was set for non-Windows bash where PS1 is set", async () => {
		...
		const result =
			terminalEnvVarCollectionService.isTerminalPromptSetCorrectly(
				resource
			);
