...
suite("Terminal Activation Indicator Prompt", () => {
	...
	const expectedMessage = Interpreters.terminalEnvVarCollectionPrompt.format(
		"Python virtual",
		`"(${envName})"`
	);
	...
	setup(async () => {
		...
		terminalEnvVarCollectionService =
			mock<ITerminalEnvVarCollectionService>();
		...
		when(terminalManager.onDidOpenTerminal).thenReturn(
			terminalEventEmitter.event
		);
	...
	test("Show notification when a new terminal is opened for which there is no prompt set", async () => {
		...
		when(
			shell.showInformationMessage(expectedMessage, ...prompts)
