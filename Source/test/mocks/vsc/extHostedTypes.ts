...
export class Range {
	static isRange(thing: unknown): thing is vscode.Range {
		...
		return (
			Position.isPosition((thing as Range).start) &&
...
export class DocumentHighlight {
	...
	toJSON(): { range: Range; kind: DocumentHighlightKind } {
		return {
			...
			kind: DocumentHighlightKind[
				this.kind
			] as unknown as DocumentHighlightKind,
		};
	}
}
	...
	toJSON(): {
		...
		return {
			...
			kind:
				...
				(CompletionItemKind[
					this.kind
				] as unknown as CompletionItemKind),
			detail: this.detail,
...
export class RelativePattern implements IRelativePattern {
	...
	constructor(base: vscode.WorkspaceFolder | string, pattern: string) {
		...
		this.baseUri =
			typeof base === "string" ? vscUri.URI.parse(base) : base.uri;
