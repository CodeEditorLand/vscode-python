...
function resolveExample(
	...
	if (postKeyword !== "") {
		if (resolvedBase.includes(" ")) {
			...
			resolvedBase = `${resolvedBase.substring(
				0,
...
suite("Language Configuration", () => {
	...
	suite('"indentationRules"', () => {
		test("indentationRules is not defined", () => {
			expect(cfg.indentationRules).to.be.equal(
				undefined,
				"missing tests"
			);
		});
	...
	suite('"onEnterRules"', () => {
		const MULTILINE_SEPARATOR_INDENT_REGEX =
			cfg.onEnterRules![0].beforeText;
		...
		test("Multiline separator indent regex should not pick up strings with escaped characters", async () => {
			const result =
				MULTILINE_SEPARATOR_INDENT_REGEX.test("a = 'hello \\n'");
			expect(result).to.be.equal(
				false,
		...
		test("Multiline separator indent regex should pick up strings ending with a multiline separator", async () => {
			const result =
				MULTILINE_SEPARATOR_INDENT_REGEX.test("a = 'multiline \\");
			expect(result).to.be.equal(
				true,
		...
		[
			...
			[
				...
				const [_example, invalid, ignored] = resolveExample(
					base,
					leading,
					postKeyword,
					preColon,
