				...
				[undefined, ...condaEnvs].forEach((condaEnvInfo) => {
					...
					suite(`${InstallerClass.name} (${testSuite})`, () => {
						...
						setup(() => {
							...
							serviceContainer
								.setup((c) =>
									c.get(TypeMoq.It.isValue(IFileSystem))
								)
								.returns(() => fs.object);
							...
							installationChannel =
								TypeMoq.Mock.ofType<IInstallationChannelManager>();
							serviceContainer
								.setup((c) =>
									c.get(
										TypeMoq.It.isValue(
							...
							const condaService =
								TypeMoq.Mock.ofType<ICondaService>();
							condaService
							...
							const condaLocatorService =
								TypeMoq.Mock.ofType<IComponentAdapter>();
							serviceContainer
								.setup((c) =>
									c.get(TypeMoq.It.isValue(IComponentAdapter))
								)
							...
							pythonSettings =
								TypeMoq.Mock.ofType<IPythonSettings>();
							pythonSettings
							...
							terminalService =
								TypeMoq.Mock.ofType<ITerminalService>();
							const terminalServiceFactory =
								TypeMoq.Mock.ofType<ITerminalServiceFactory>();
							terminalServiceFactory
								.setup((f) =>
									f.getTerminalService(TypeMoq.It.isAny())
								)
							...
							interpreterService =
								TypeMoq.Mock.ofType<IInterpreterService>();
							serviceContainer
								.setup((c) =>
									c.get(
						...
						function setActiveInterpreter(
							...
							interpreterService
								.setup((i) =>
									i.getActiveInterpreter(TypeMoq.It.isAny())
								)
								.returns(() =>
						...
						getModuleNamesForTesting()
							...
							.forEach((product) => {
								...
								if (InstallerClass === TestModuleInstaller) {
									suite(
										`If interpreter type is Unknown (${product.name})`,
										async () => {
											test(`If 'python.globalModuleInstallation' is set to true and pythonPath directory is read only, do an elevated install`, async () => {
												const info =
													TypeMoq.Mock.ofType<PythonEnvironment>();
												...
												pythonSettings
													.setup(
														(p) =>
															p.globalModuleInstallation
													)
													.returns(() => true);
												const elevatedInstall =
													sinon.stub(
												...
												assert.ok(
													elevatedInstall.calledOnceWith(
														pythonPath,
														args
													)
												);
												...
												const args =
													...
													Product.pip
														? [
																"-m",
																"executionInfo",
														  ] // Pipe is always installed into the environment.
														: [
																"-m",
																"executionInfo",
																"--user",
