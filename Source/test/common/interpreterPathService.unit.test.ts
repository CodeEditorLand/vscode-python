...
suite("Interpreter Path Service", async () => {
	...
	setup(() => {
		...
		workspaceService
			.setup((w) => w.getWorkspaceFolder(resourceOutsideOfWorkspace))
			.returns(() => undefined);
		persistentStateFactory = TypeMoq.Mock.ofType<IPersistentStateFactory>();
		workspaceService
			.setup((w) => w.onDidChangeConfiguration)
	...
	test("Global settings are not updated if stored value is same as new value", async () => {
		...
		workspaceConfig
			.setup((w) =>
				w.update("defaultInterpreterPath", interpreterPath, true)
			)
	...
	test("Workspace settings are not updated if stored value is same as new value", async () => {
		...
		const persistentState =
			TypeMoq.Mock.ofType<IPersistentState<string | undefined>>();
		workspaceService
		...
		persistentStateFactory
			.setup((p) =>
				p.createGlobalPersistentState<string | undefined>(
					expectedSettingKey,
					undefined
				)
			)
	...
	test("Ensure the correct event is fired if Workspace settings are updated", async () => {
		...
		const _didChangeInterpreterEmitter =
			TypeMoq.Mock.ofType<EventEmitter<InterpreterConfigurationScope>>();
		interpreterPathService._didChangeInterpreterEmitter =
			_didChangeInterpreterEmitter.object;
	...
	test("Workspace settings are correctly updated in case of multiroot folders", async () => {
		...
		const expectedSettingKey = `WORKSPACE_INTERPRETER_PATH_${fs.normCase(
			workspaceFileUri.fsPath
		)}`;
		const persistentState =
			TypeMoq.Mock.ofType<IPersistentState<string | undefined>>();
		workspaceService
			...
			.returns(() => resource.fsPath);
		workspaceService
			.setup((w) => w.workspaceFile)
			.returns(() => workspaceFileUri);
		persistentStateFactory
			.setup((p) =>
				p.createGlobalPersistentState<string | undefined>(
					expectedSettingKey,
	...
	test("Inspecting settings returns as expected if no workspace is opened", async () => {
		...
		const settings = interpreterPathService.inspect(
			resourceOutsideOfWorkspace
		);
	...
	test("Inspecting settings returns as expected if a folder is directly opened", async () => {
		...
		const workspaceFolderPersistentState =
			TypeMoq.Mock.ofType<IPersistentState<string | undefined>>();
		workspaceService
			.setup((w) => w.workspaceFolders)
			.returns(() => undefined);
		...
		persistentStateFactory
			...
			.returns(() => workspaceFolderPersistentState.object);
		workspaceFolderPersistentState
			.setup((p) => p.value)
			.returns(() => "workspaceFolderValue");
	...
	test("Inspecting settings returns as expected in case of multiroot folders", async () => {
		const workspaceFileUri = Uri.parse("path/to/workspaceFile");
		const expectedWorkspaceSettingKey = `WORKSPACE_INTERPRETER_PATH_${fs.normCase(
			workspaceFileUri.fsPath
		)}`;
		...
		const workspaceFolderPersistentState =
			TypeMoq.Mock.ofType<IPersistentState<string | undefined>>();
		const workspacePersistentState =
			TypeMoq.Mock.ofType<IPersistentState<string | undefined>>();
		workspaceService
			.setup((w) => w.workspaceFolders)
			.returns(() => undefined);
	...
	test("Ensure on interpreter change captures the fired event with the correct arguments", async () => {
		...
		const interpreterConfigurationScope = {
			uri: undefined,
			configTarget: ConfigurationTarget.Global,
		};
		...
		interpreterPathService._didChangeInterpreterEmitter.fire(
			interpreterConfigurationScope
		);
		const eventCaptured = await Promise.race([
			deferred.promise,
