...
suite("Process - Python tool execution service", () => {
	...
	setup(() => {
		...
		when(
			pythonService.execModuleObservable(
				anything(),
				anything(),
				anything()
			)
		...
		when(
			pythonService.execModule(anything(), anything(), anything())
		).thenResolve(executionResult);
		...
		when(executionFactory.create(anything())).thenResolve(
			pythonServiceInstance
		);

		processService = mock(ProcessService);
		when(
			processService.execObservable(anything(), anything(), anything())
		).thenReturn(observable);
		when(
			processService.exec(anything(), anything(), anything())
		).thenResolve(executionResult);

		processFactory = mock(ProcessServiceFactory);
		when(processFactory.create(anything())).thenResolve(
		...
		).thenReturn(instance(executionFactory));
		when(
			serviceContainer.get<IProcessServiceFactory>(IProcessServiceFactory)
		).thenReturn(instance(processFactory));

		executionService = new PythonToolExecutionService(
	...
	test("When calling execObservable, throw an error if environment variables are passed to the options parameter", () => {
		...
		const promise = executionService.execObservable(
			executionInfo,
			options,
			resource
		);
	...
	test("When calling execObservable, use a python execution service if a module name is passed to the execution info", async () => {
		...
		const result = await executionService.execObservable(
			executionInfo,
			options,
			resource
		);
