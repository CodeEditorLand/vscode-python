...
function title(resource?: Uri, interpreter?: PythonEnvironment) {
	return `${resource ? "With a resource" : "Without a resource"}${
		interpreter ? " and an interpreter" : ""
	}`;
}
...
async function verifyCreateActivated(
	...
	when(
		activationHelper.getActivatedEnvironmentVariables(
			resource,
			anything(),
			anything()
		)
	...
	[
		...
		suite(title(resource, interpreter), () => {
			...
			setup(() => {
				...
				sinon
					.stub(Conda.prototype, "getInterpreterPathForEnvironment")
					.resolves(pythonPath);
				...
				when(interpreterPathExpHelper.get(anything())).thenReturn(
					"selected interpreter path"
				);

				pyenvs = mock<IComponentAdapter>();
				when(
					pyenvs.isMicrosoftStoreInterpreter(anyString())
				).thenResolve(true);

				executionService =
					typemoq.Mock.ofType<IPythonExecutionService>();
				...
				when(
					serviceContainer.get<IProcessLogger>(IProcessLogger)
				...
				when(
					serviceContainer.get<IActivatedEnvironmentLaunch>(
						IActivatedEnvironmentLaunch
					)
				).thenReturn(instance(activatedEnvironmentLaunch));
				when(
					serviceContainer.get<IComponentAdapter>(IComponentAdapter)
				).thenReturn(instance(pyenvs));
				when(
					serviceContainer.tryGet<IInterpreterService>(
						IInterpreterService
					)
				).thenReturn(instance(interpreterService));
			...
			test("Ensure PythonExecutionService is created", async () => {
				...
				when(processFactory.create(resource)).thenResolve(
			...
			test("If interpreter is explicitly set to `python`, ensure we use it", async () => {
				...
				when(
					autoSelection.autoSelectInterpreter(anything())
				).thenResolve();
				when(configService.getSettings(resource)).thenReturn(
					instance(pythonSettings)
				);
			...
			test("Ensure we use an existing `create` method if there are no environment variables for the activated env", async () => {
				...
				const service = await verifyCreateActivated(
					factory,
					activationHelper,
					resource,
					interpreter
			...
			test("Ensure `createActivatedEnvironment` returns a CondaExecutionService instance if createCondaExecutionService() returns a valid object", async () => {
				...
				if (!interpreter) {
					...
					verify(
						pyenvs.getCondaEnvironment(interpreter!.path)
					).once();
				}
			});
			...
			test("Ensure `createCondaExecutionService` creates a CondaExecutionService instance if there is a conda environment", async () => {
				...
				const result = await factory.createCondaExecutionService(
					pythonPath,
					processService.object
				);
