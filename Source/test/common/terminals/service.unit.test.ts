...
suite("Terminal Service", () => {
	...
	setup(() => {
		...
		mockServiceContainer
			.setup((c) => c.get(ITerminalAutoActivation))
			.returns(() => terminalAutoActivator.object);
	});
	teardown(() => {
		if (service) {
			service.dispose();
	...
	test("Ensure to disable auto activation and right interpreter is activated", async () => {
		...
		terminalAutoActivator.verify(
			(t) => t.disableAutoActivation(terminal.object),
			TypeMoq.Times.once()
		);
		// Ensure the terminal is activated with the interpreter info.
		terminalActivator.verify(
			(t) =>
				t.activateEnvironmentInTerminal(
					terminal.object,
					TypeMoq.It.isObjectWith({ interpreter })
				),
			TypeMoq.Times.once()
		);
	});
