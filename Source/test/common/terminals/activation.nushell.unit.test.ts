...
suite("Terminal Environment Activation (nushell)", () => {
	for (const scriptFileName of ["activate", "activate.sh", "activate.nu"]) {
		suite(`and script file is ${scriptFileName}`, () => {
			...
			for (const { name, value } of getNamesAndValues<TerminalShellType>(
				TerminalShellType
			)) {
				const isNushell = value === TerminalShellType.nushell;
				const isScriptFileSupported =
					isNushell && ["activate.nu"].includes(scriptFileName);
				const expectedReturn = isScriptFileSupported
					? "activation command"
					: "undefined";
				...
				test(`Ensure nushell Activation command returns ${expectedReturn} (Shell: ${name})`, async () => {
					...
					if (isNushell) {
						expect(supported).to.be.equal(
							true,
							`${name} shell not supported (it should be)`
						);
					} else {
						expect(supported).to.be.equal(
							false,
							`${name} incorrectly supported (should not be)`
						);
					...
					const command = await nu.getActivationCommands(
						undefined,
						value
					);
