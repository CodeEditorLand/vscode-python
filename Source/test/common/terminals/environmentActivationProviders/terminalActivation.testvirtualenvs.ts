...
suite("Activation of Environments in Terminal", () => {
	...
	async function openTerminalAndAwaitCommandContent(
		...
		await waitForCondition(
			() => fs.pathExists(logFile),
			logFileCreationWaitMs,
			`${logFile} file not created.`
		);
	...
	async function testActivation(envPath: string) {
		...
		const content = await openTerminalAndAwaitCommandContent(
	...
	test("Should not activate", async () => {
		...
		expect(fileSystem.arePathsSame(content, PYTHON_PATH)).to.equal(
			false,
			"Environment not activated"
		);
	});
	...
	test("Should activate with venv", async function () {
		if (
			process.env.CI_PYTHON_VERSION &&
			process.env.CI_PYTHON_VERSION.startsWith("2.")
		) {
			this.skip();
		}
