...
suite("Persistent State", () => {
	...
	setup(() => {
		...
		persistentStateFactory = new PersistentStateFactory(
			globalMemento,
			workspaceMemento,
			cmdManager.object
		);
	...
	test("Global states created are restored on invoking clean storage command", async () => {
		...
		cmdManager
			.setup((c) =>
				c.registerCommand(Commands.ClearStorage, TypeMoq.It.isAny())
			)
			.callback((_, c) => {
	...
	test("Ensure internal global storage extension uses to track other storages does not contain duplicate entries", async () => {
		...
		await sleep(1);
		persistentStateFactory.createGlobalPersistentState("key2", [
			"defaultValue1",
		]); // Default value type is an array
		await sleep(1);
		persistentStateFactory.createGlobalPersistentState("key2", [
			"defaultValue1",
		]);
	...
	test("Ensure internal workspace storage extension uses to track other storages does not contain duplicate entries", async () => {
		persistentStateFactory.createWorkspacePersistentState(
			"key2",
			"defaultValue1"
		); // Default value type is a string
		await sleep(1);
		persistentStateFactory.createWorkspacePersistentState("key1");
		await sleep(1);
		persistentStateFactory.createWorkspacePersistentState(
	...
	test("Ensure deprecated global storage extension used to track other storages with is reset", async () => {
		const global = persistentStateFactory.createGlobalPersistentState<
			KeysStorage[]
		>(GLOBAL_PERSISTENT_KEYS_DEPRECATED);
	...
	test("Ensure deprecated global storage extension used to track other storages with is reset", async () => {
		const workspace = persistentStateFactory.createWorkspacePersistentState<
			KeysStorage[]
		>(WORKSPACE_PERSISTENT_KEYS_DEPRECATED);
