...
suite("Utils for filesystem - convertFileType function", () => {
	const testsData = [
		{
			info: new KnowsFileTypeDummyImpl(true, false, false),
			kind: "File",
			expected: 1,
		},
		{
			info: new KnowsFileTypeDummyImpl(false, true, false),
			kind: "Directory",
			expected: 2,
		},
		{
			info: new KnowsFileTypeDummyImpl(false, false, true),
			kind: "Symbolic Link",
			expected: 64,
		},
		{
			info: new KnowsFileTypeDummyImpl(false, false, false),
			kind: "Unknown",
			expected: 0,
		},
	];
