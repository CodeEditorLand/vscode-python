...
import {
	...
	parseEnvFile,
} from "../../../client/common/variables/environment";
...
PYTHONPATH=../workspace5
			...
			expect(Object.keys(vars!)).lengthOf(
				2,
	...
	PATHS.map((pathVariable) => {
		suite(`mergeVariables() (path var: ${pathVariable})`, () => {
			...
			test("Ensure variables are merged", async () => {
				...
				expect(Object.keys(vars1)).lengthOf(
					2,
					"Source variables modified"
				);
			...
			test("Ensure path variables in target are overwritten", async () => {
				...
				variablesService.mergeVariables(source, target, {
					overwrite: true,
				});

				expect(Object.keys(source)).lengthOf(
					2,
					"Source variables modified"
				);
