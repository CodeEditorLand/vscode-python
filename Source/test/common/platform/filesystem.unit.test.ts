...
suite("Raw FileSystem", () => {
	...
	setup(() => {
		raw = TypeMoq.Mock.ofType<IRawFS>(
			undefined,
	...
	function createMockLegacyStat(): TypeMoq.IMock<fsextra.Stats> {
		const stat = TypeMoq.Mock.ofType<fsextra.Stats>(
			undefined,
			TypeMoq.MockBehavior.Strict
		);
	...
	function setupStatFileType(
		stat: TypeMoq.IMock<fs.Stats>,
		filetype: FileType
	) {
	...
	suite("move", () => {
		...
		test("move a symlink to a directory (target exists)", async () => {
			...
			raw.setup((r) => r.stat(Uri(tgt))) // It's a symlink.
				.returns(() =>
					Promise.resolve({
						type: FileType.SymbolicLink | FileType.Directory,
					} as unknown as FileStat)
				);
			raw.setup((r) => r.rename(Uri(src), Uri(tgt), { overwrite: true })) // expect the specific filename
				.returns(() => Promise.resolve());
		...
		test("fails if the low-level call fails", async () => {
			...
			raw.setup((r) =>
				r.rename(TypeMoq.It.isAny(), TypeMoq.It.isAny(), {
					overwrite: false,
				})
			) // We don't care about the filename.
				.throws(err);
	...
	suite("writeText", () => {
		...
		test("fails if the low-level call fails", async () => {
			raw.setup((r) =>
				r.writeFile(TypeMoq.It.isAny(), TypeMoq.It.isAny())
			) // We don't care about the filename.
				.throws(new Error("file not found"));
	...
	suite("appendText", () => {
		...
		test("fails if the low-level call fails", async () => {
			raw.setup((r) =>
				r.appendFile(TypeMoq.It.isAny(), TypeMoq.It.isAny())
			) // We don't care about the filename.
				.throws(new Error("file not found"));
	...
	suite("copyFile", () => {
		...
		test("fails if the low-level call fails", async () => {
			...
			raw.setup((r) =>
				r.copy(TypeMoq.It.isAny(), TypeMoq.It.isAny(), {
					overwrite: true,
				})
			) // We don't care about the filename.
				.throws(new Error("file not found"));
	...
	suite("readTextSync", () => {
		...
		test("fails if the low-level call fails", async () => {
			raw.setup((r) =>
				r.readFileSync(TypeMoq.It.isAny(), TypeMoq.It.isAny())
			) // We don't care about the filename.
				.throws(new Error("file not found"));
...
suite("FileSystemUtils", () => {
	...
	setup(() => {
		deps = TypeMoq.Mock.ofType<IUtilsDeps>(
			undefined,
			TypeMoq.MockBehavior.Strict
		);
		...
		utils = new FileSystemUtils(
			...
			(pat: string, options?: { cwd: string }) =>
				deps.object.globFile(pat, options)
		);
	});
	function verifyAll() {
		deps.verifyAll();
	...
	function createMockStat(): TypeMoq.IMock<FileStat> {
		const stat = TypeMoq.Mock.ofType<FileStat>(
			undefined,
			TypeMoq.MockBehavior.Strict
		);
		// This is necessary because passing "mock.object" to
	...
	suite("pathExists", () => {
		...
		test("symlinks are followed", async () => {
			...
			const destIsDir = await utils.pathExists(
				symlink,
				FileType.Directory
			);
	...
	suite("listdir", () => {
		...
		test("fails if not a directory", async () => {
			...
			deps.setup((d) => d.listdir(dirname)) // Fail (async) with not-a-directory.
				.returns(() => Promise.reject(err));
			deps.setup((d) => d.pathExists(dirname)).returns(() =>
