...
suite("FileSystem - raw", () => {
	...
	suite("move", () => {
		...
		test("move directory", async () => {
			const source = await fix.createDirectory(
				"spam/spam/spam/eggs/spam"
			);
			await fix.createFile(
				"spam/spam/spam/eggs/spam/data.json",
				"<text>"
			);
			await fix.createDirectory("spam/spam/spam/hash");
			const target = await fix.resolve("spam/spam/spam/hash/spam");
			const filename = await fix.resolve(
				"spam/spam/spam/hash/spam/data.json",
				false
			);
			await assertDoesNotExist(target);
	...
	suite("listdir", () => {
		test("mixed", async function () {
			...
			const script = await fix.createFile(
				"x/y/z/__main__.py",
				"<script here>"
			);
	...
	suite("raw", () => {
		suite("stat", () => {
			...
			test("for symlinks, gets the info for the linked file", async function () {
				...
				const filename = await fix.createFile("x/y/z/spam.py", "...");
				const symlink = await fix.createSymlink(
					"x/y/z/eggs.py",
					filename
				);
				const old = await fsextra.stat(filename);
		...
		suite("readFile", () => {
			test("wraps the low-level impl", async () => {
				const expected = "<some text>";
				const filename = await fix.createFile(
					"x/y/z/spam.py",
					expected
				);
		...
		suite("getSubDirectories", () => {
			test("mixed types", async () => {
				...
				if (SUPPORTS_SYMLINKS) {
					...
					const symlinkDirSource =
						await fix.createDirectory("x/data");
					const symlink = await fix.createSymlink(
						"x/y/z/scripts/datadir",
						symlinkDirSource
					);
			...
			test("empty if the directory does not exist", async () => {
				const entries =
					await filesystem.getSubDirectories(DOES_NOT_EXIST);
