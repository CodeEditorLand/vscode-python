...
suite("Language server watcher", () => {
	...
	test(`If the interpreter changed, the existing language server should be stopped if there is one`, async () => {
		...
		const extensionManager =
			(await watcher.get()) as ILanguageServerExtensionManager;
		const stopLanguageServerSpy = sandbox.spy(
			extensionManager,
			"stopLanguageServer"
		);
	...
	test(`When starting the language server, if the language server cannot be started, it should call languageServerNotAvailable`, async () => {
		const canStartLanguageServerStub = sandbox.stub(
			NoneLSExtensionManager.prototype,
	...
	test("When the config settings change, but the python.languageServer setting is not affected, the watcher should not restart the language server", async () => {
		...
		const workspaceService = {
			getWorkspaceFolder: (uri: Uri) => ({ uri }),
			onDidChangeConfiguration: (
				listener: (event: ConfigurationChangeEvent) => Promise<void>
			) => {
		...
		watcher.register();
		const startLanguageServerSpy = sandbox.spy(
			watcher,
			"startLanguageServer"
		);
	...
	test("When the config settings change, and the python.languageServer setting is affected, the watcher should restart the language server", async () => {
		...
		getSettingsStub
			.onFirstCall()
			.returns({ languageServer: LanguageServerType.None });
		getSettingsStub
			.onSecondCall()
			.returns({ languageServer: LanguageServerType.Node });
	...
	test("When starting a language server with a Python 2.7 interpreter and the python.languageServer setting is default, use Pylance", async () => {
		const startLanguageServerStub = sandbox.stub(
			PylanceLSExtensionManager.prototype,
			"startLanguageServer"
		);
		startLanguageServerStub.returns(Promise.resolve());

		sandbox
			.stub(PylanceLSExtensionManager.prototype, "canStartLanguageServer")
			.returns(true);
	...
	[
		...
		test(`When starting language servers with different resources, ${
			...
			startLanguageServerStub.returns(Promise.resolve());
			const stopLanguageServerStub = sandbox.stub(
				extensionLSCls.prototype,
				"stopLanguageServer"
			);
			sandbox
				.stub(extensionLSCls.prototype, "canStartLanguageServer")
				.returns(true);
			...
			await watcher.startLanguageServer(
				languageServer,
				Uri.parse("folder1")
			);
			await watcher.startLanguageServer(
		...
		test(`${languageServer} language server(s) should ${
			...
			sandbox
				.stub(extensionLSCls.prototype, "startLanguageServer")
			...
			let onDidChangeWorkspaceFoldersListener: (
			...
			const workspaceService = {
				...
				workspaceFolders: [
					{ uri: Uri.parse("workspace1") },
					{ uri: Uri.parse("workspace2") },
				],
				isTrusted: true,
			} as unknown as IWorkspaceService;
			...
			await watcher.startLanguageServer(
				languageServer,
				Uri.parse("workspace1")
			);
			await watcher.startLanguageServer(
				languageServer,
				Uri.parse("workspace2")
			);
	...
	test("The language server should be restarted if the interpreter info changed", async () => {
		...
		let onDidChangeInfoListener: (
			event: PythonEnvironment
		) => Promise<void> = () => Promise.resolve();
