...
suite("Debugging - commands", () => {
	...
	setup(() => {
		...
		commandManager
			.setup((c) =>
				c.executeCommand(typemoq.It.isAny(), typemoq.It.isAny())
			)
		...
		interpreterService
			...
			.returns(() =>
				Promise.resolve({ path: "ps" } as unknown as PythonEnvironment)
			);
	...
	test("Test running debug file command", async () => {
		let callback: (f: Uri) => Promise<void> = (_f: Uri) =>
			Promise.resolve();
		commandManager
			.setup((c) =>
				c.registerCommand(
		...
		await callback(
			Uri.file(path.join(EXTENSION_ROOT_DIR_FOR_TESTS, "test.py"))
		);
		commandManager.verifyAll();
