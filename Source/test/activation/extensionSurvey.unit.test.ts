...
import {
	...
	extensionSurveyStateKeys,
} from "../../client/activation/extensionSurvey";
import {
...
suite("Extension survey prompt - showSurvey()", () => {
	...
	test("Launch survey if 'Yes' option is clicked", async () => {
		...
		appShell
			...
			.returns(() =>
				Promise.resolve(ExtensionSurveyBanner.bannerLabelYes)
			)
			.verifiable(TypeMoq.Times.once());
		browserService
...
suite("Extension survey prompt - activate()", () => {
	...
	test("If user is not in 'ShowExtensionSurveyPrompt' experiment, return immediately", async () => {
		shouldShowBanner = sinon.stub(
			ExtensionSurveyPrompt.prototype,
			"shouldShowBanner"
		);
		...
		experiments
			.setup((exp) =>
				exp.inExperiment(ShowExtensionSurveyPrompt.experiment)
			)
	...
	test("No survey is shown if shouldShowBanner() returns false and user is in 'ShowExtensionSurveyPrompt' experiment", async () => {
		...
		const doesSurveyShowUp = await Promise.race([
			deferred.promise,
