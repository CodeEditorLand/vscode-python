	...
	suite("Language Server Activation - ActivationManager", () => {
		...
		setup(() => {
			interpreterPathService =
				typemoq.Mock.ofType<IInterpreterPathService>();
			interpreterPathService
				.setup((i) =>
					i.copyOldInterpreterStorageValuesToNew(typemoq.It.isAny())
				)
			...
			autoSelection =
				typemoq.Mock.ofType<IInterpreterAutoSelectionService>();
		...
		test("Initialize will add event handlers and will dispose them when running dispose", async () => {
			...
			when(workspaceService.onDidChangeWorkspaceFolders).thenReturn(
				() => disposable.object
			);
		...
		test("Activate workspace specific to the resource in case of Multiple workspaces when a file is opened", async () => {
			...
			when(
				workspaceService.getWorkspaceFolder(document.object.uri)
			).thenReturn(folder2);
			...
			expect(workspaceFoldersChangedHandler).not.to.be.equal(
				undefined,
				"Handler not set"
			);
			...
			expect(fileOpenedHandler).not.to.be.equal(
				undefined,
				"Handler not set"
			);
		...
		test("List of activated workspaces is updated & Handler docOpenedHandler is disposed in case no. of workspace folders decreases to one", async () => {
			...
			workspaceFoldersChangedHandler.call(managerTest);
			expect(docOpenedHandler).not.to.be.equal(
				undefined,
				"Handler not set"
			);
			...
			assert.deepEqual(
				Array.from(managerTest.activatedWorkspaces.keys()),
				["one"]
			);
		});
	});
