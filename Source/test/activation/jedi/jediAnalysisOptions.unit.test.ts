...
suite("Jedi LSP - analysis Options", () => {
	...
	setup(() => {
		...
		when(envVarsProvider.onDidEnvironmentVariablesChange).thenReturn(
			onDidChangeEnvVariables.event
		);
	...
	test("Validate defaults", async () => {
		...
		expect(result.initializationOptions.markupKindPreferred).to.deep.equal(
			"markdown"
		);
		expect(
			result.initializationOptions.completion.resolveEagerly
		).to.deep.equal(false);
		expect(
			result.initializationOptions.completion.disableSnippets
		).to.deep.equal(true);
		expect(result.initializationOptions.diagnostics.enable).to.deep.equal(
			true
		);
		expect(result.initializationOptions.diagnostics.didOpen).to.deep.equal(
			true
		);
		expect(result.initializationOptions.diagnostics.didSave).to.deep.equal(
			true
		);
		expect(
			result.initializationOptions.diagnostics.didChange
		).to.deep.equal(true);
		expect(
			result.initializationOptions.hover.disable.keyword.all
		).to.deep.equal(true);
		expect(result.initializationOptions.workspace.extraPaths).to.deep.equal(
			[]
		);
		expect(
			result.initializationOptions.workspace.symbols.maxSymbols
		).to.deep.equal(0);
	});
	...
	test("Without extraPaths provided", async () => {
		when(workspaceService.getWorkspaceFolder(anything())).thenReturn(
			new MockWorkspaceFolder(workspacePath)
		);
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		when(configurationService.getSettings(anything())).thenReturn(
			{} as any
	...
	test("With extraPaths provided", async () => {
		...
		when(configurationService.getSettings(anything())).thenReturn({
			...
			autoComplete: {
				extraPaths: [__dirname, "relative/pathB", "relative/pathB"],
			},
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
		} as any);
		analysisOptions.initialize(undefined, undefined);
