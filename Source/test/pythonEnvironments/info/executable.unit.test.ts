...
suite("getExecutablePath()", () => {
	...
	test("should get the value by running python", async () => {
		...
		deps.setup((d) =>
			d.shellExec(
				`${python.command} -c "import sys;print(sys.executable)"`,
				It.isAny()
			)
		)
			// Return the expected value.
			.returns(() => Promise.resolve({ stdout: expected }));
		const exec = async (c: string, a: ShellOptions | undefined) =>
			deps.object.shellExec(c, a);

		const result = await getExecutablePath(python, exec);

		expect(result).to.equal(
			expected,
			"getExecutablePath() should return get the value by running Python"
		);
		deps.verifyAll();
	});
