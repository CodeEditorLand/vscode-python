...
suite("Create Environments Tests", () => {
	...
	setup(() => {
		...
		showQuickPickWithBackStub = sinon.stub(
			windowApis,
	...
	test("Environment creation error", async () => {
		...
		await assert.isRejected(
			handleCreateEnvironmentCommand([provider.object])
		);
	...
	test("User clicked Back", async () => {
		...
		showQuickPickWithBackStub.returns(
			Promise.reject(windowApis.MultiStepAction.Back)
		);

		provider1.setup((p) => (p as any).then).returns(() => undefined);
		provider2.setup((p) => (p as any).then).returns(() => undefined);
		const result = await handleCreateEnvironmentCommand(
			[provider1.object, provider2.object],
			{
				showBackButton: true,
			}
	...
	test("User pressed Escape", async () => {
		...
		showQuickPickWithBackStub.returns(
			Promise.reject(windowApis.MultiStepAction.Cancel)
		);
