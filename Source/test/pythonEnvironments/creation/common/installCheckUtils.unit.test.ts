...
suite("Install check diagnostics tests", () => {
	...
	setup(() => {
		...
		getConfigurationStub.callsFake(
			(section?: string, _scope?: ConfigurationScope | null) => {
				if (section === "python") {
	...
	test("Test parse diagnostics", async () => {
		...
		const result = await getInstalledPackagesDiagnostics(
	...
	[
		...
		test(`Test missing package severity: ${setting}`, async () => {
			...
			plainExecStub.callsFake(
				(_cmd: string, _args: string[], options: SpawnOptions) => {
