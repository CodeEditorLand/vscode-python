...
suite("venv Creation provider tests", () => {
	...
	setup(() => {
		pickExistingVenvActionStub = sinon.stub(
			venvUtils,
		...
		pickPackagesToInstallStub = sinon.stub(
			venvUtils,
			"pickPackagesToInstall"
		);

		showErrorMessageWithLogsStub = sinon.stub(
			commonUtils,
		...
		pickExistingVenvActionStub.resolves(
			venvUtils.ExistingVenvAction.Create
		);
	...
	test("Create venv with python selected by user no packages selected", async () => {
		...
		progressMock
			.setup((p) => p.report({ message: CreateEnv.statusStarting }))
			.verifiable(typemoq.Times.once());
		...
		_next!({
			out: `${VENV_CREATED_MARKER}new_environment`,
			source: "stdout",
		});
		_complete!();
	...
	test("Create venv with pre-existing .venv, user selects re-create", async () => {
		pickExistingVenvActionStub.resolves(
			venvUtils.ExistingVenvAction.Recreate
		);
		pickWorkspaceFolderStub.resolves(workspace1);
	...
	test("Create venv with pre-existing .venv, user selects use existing", async () => {
		pickExistingVenvActionStub.resolves(
			venvUtils.ExistingVenvAction.UseExisting
		);
		pickWorkspaceFolderStub.resolves(workspace1);
	...
	test("Create venv with 1000 requirement files", async () => {
		...
		const expected = JSON.stringify({
			requirements: requirements.map((r) => r.installItem),
		});
	...
	test("Create venv with 5 requirement files", async () => {
		...
		execObservableStub.callsFake(
			(_c, argv: string[], options: SpawnOptions) => {
				stdin = options?.stdinStr;
				actualRequirements = argv
					.filter((arg) => arg.startsWith("requirements"))
					.sort();
				hasStdinArg = argv.includes("--stdin");
				deferred.resolve();
				return {
