...
suite("Create Environment Trigger", () => {
	...
	setup(() => {
		shouldPromptToCreateEnvStub = sinon.stub(
			triggerUtils,
		...
		hasRequirementFilesStub = sinon.stub(
			triggerUtils,
			"hasRequirementFiles"
		);
		hasKnownFilesStub = sinon.stub(triggerUtils, "hasKnownFiles");
		isGlobalPythonSelectedStub = sinon.stub(
			triggerUtils,
		...
		isCreateEnvWorkspaceCheckNotRunStub = sinon.stub(
			triggerUtils,
		...
		disableCreateEnvironmentTriggerStub = sinon.stub(
			triggerUtils,
