...
export class SimpleLocator<I = PythonEnvInfo> extends Locator<I> {
	...
	constructor(
		...
		public callbacks: {
			resolve?:
				| null
				| ((
						env: PythonEnvInfo | string
				  ) => Promise<PythonEnvInfo | undefined>);
			before?(): Promise<void>;
			after?(): Promise<void>;
			onUpdated?: Event<
				PythonEnvUpdatedEvent<I> | ProgressNotificationEvent
			>;
	...
	public async resolveEnv(env: string): Promise<PythonEnvInfo | undefined> {
		...
		return this.callbacks.resolve(
			this.options?.resolveAsString ? env : envInfo
		);
	}
}
...
export function sortedEnvs(envs: PythonEnvInfo[]): PythonEnvInfo[] {
	return envs.sort((env1, env2) => {
		const env1str = `${env1.kind}-${
			env1.executable.filename
		}-${getVersionString(env1.version)}`;
		const env2str = `${env2.kind}-${
			env2.executable.filename
		}-${getVersionString(env2.version)}`;
		return env1str.localeCompare(env2str);
	});
}

export function assertSameEnvs(
	envs: PythonEnvInfo[],
	expected: PythonEnvInfo[]
): void {
