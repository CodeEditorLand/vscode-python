...
suite("Python envs locators - Locators", () => {
	...
	suite("iterEnvs() consolidates", () => {
		...
		test("many", async () => {
			...
			const sub3 = new SimpleLocator([env2, env3, env4], {
				before: () => sub2.done,
			});
		...
		test("with query", async () => {
			...
			async function onQuery(
				q: PythonLocatorQuery | undefined,
				e: PythonEnvInfo[]
			) {
				query = q;
				return e;
		...
		test("iterate out of order", async () => {
			...
			const env6 = createNamedEnv(
				"spam",
				"3.10.0a0",
				PythonEnvKind.Custom
			);
			const env7 = createNamedEnv(
				"eggs",
				"3.9.1a0",
				PythonEnvKind.Custom
			);
			...
			const sub5 = new SimpleLocator([env6, env7], {
				before: () => sub3.done,
			});
