	...
	suite("iterEnvs()", () => {
		test("Iterator only yields unique environments", async () => {
			const env1 = createBasicEnv(
				PythonEnvKind.Venv,
				path.join("path", "to", "exec1")
			);
			const env2 = createBasicEnv(
				PythonEnvKind.Conda,
				path.join("path", "to", "exec2")
			);
			const env3 = createBasicEnv(
				PythonEnvKind.System,
				path.join("path", "to", "exec3")
			);
			const env4 = createBasicEnv(
				PythonEnvKind.Unknown,
				path.join("path", "to", "exec2")
			); // Same as env2
			const env5 = createBasicEnv(
				PythonEnvKind.Venv,
				path.join("path", "to", "exec1")
			); // Same as env1
		...
		test("Updates are applied correctly", async () => {
			...
			const env6 = createBasicEnv(
				PythonEnvKind.VirtualEnv,
				path.join("path", "to", "exec1")
			); // Same as env1
			const environmentsToBeIterated = [
				env1,
				env2,
				env3,
				env4,
				env5,
				env6,
			]; // Contains 3 unique environments
			const parentLocator = new SimpleLocator(environmentsToBeIterated);
			const reducer = new PythonEnvsReducer(parentLocator);
		...
		test("Updates to environments from the incoming iterator replaces the previous info", async () => {
			...
			const iteratorUpdateCallback = () => {
				...
				}); // It is essential for the incoming iterator to fire "null" event signifying it's done
			};
