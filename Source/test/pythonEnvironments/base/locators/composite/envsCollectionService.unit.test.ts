...
import {
	areSameEnv,
	buildEnvInfo,
} from "../../../../../client/pythonEnvironments/base/info/env";
import {
...
suite("Python envs locator - Environments Collection", async () => {
	...
	function applyChangeEventToEnvList(
		envs: PythonEnvInfo[],
		event: PythonEnvCollectionChangedEvent
	) {
		const env = event.old ?? event.new;
		let envIndex = -1;
		if (env) {
			envIndex = envs.findIndex(
	...
	function getLocatorEnvs() {
		...
		); // Path is valid but it's an invalid env
	...
	function getCachedEnvs() {
		const envCached3 = createEnv(
			path.join(TEST_LAYOUT_ROOT, "doesNotExist")
		); // Invalid path, should not be reported.
	...
	test("If `ifNotTriggerredAlready` option is set and a refresh for query is already triggered, triggerRefresh() does not trigger a refresh", async () => {
		...
		expect(refreshTriggerCount).to.equal(
			1,
			"Refresh should not be triggered in case 1"
		);
		...
		expect(refreshTriggerCount).to.equal(
			1,
			"Refresh should not be triggered in case 2"
		);
		await collectionService.triggerRefresh(undefined);
		expect(refreshTriggerCount).to.equal(
			2,
			"Refresh should be triggered in case 3"
		);
	});
	...
	test("Ensure update events are not fired if an environment isn't actually updated", async () => {
		...
		expect(events.length).to.equal(
			0,
			"Do not fire additional events as envs have not updated"
		);
	});
	...
	test("Ensure progress stage updates are emitted correctly and refresh promises correct track promise for each stage", async () => {
		...
		expect(stage).to.equal(
			ProgressReportStage.discoveryStarted,
			"Discovery should already be started"
		);
		...
		refreshPromise = collectionService.getRefreshPromise({
			stage: ProgressReportStage.allPathsDiscovered,
		});
		expect(refreshPromise).to.not.equal(undefined);
		const allPathsDiscoveredPromise = createDeferredFromPromise(
			refreshPromise!
		);
		refreshPromise = collectionService.getRefreshPromise({
			stage: ProgressReportStage.discoveryFinished,
		...
		expect(refreshPromise).to.not.equal(undefined);
		const discoveryFinishedPromise = createDeferredFromPromise(
			refreshPromise!
		);
		...
		collectionService.onProgress((e) => {
			if (e.stage === ProgressReportStage.allPathsDiscovered) {
				assert(
					false,
					"All paths discovered event should not be fired if a query is provided"
				);
			}
		});
		collectionService
			.triggerRefresh({
				searchLocations: { roots: [], doNotIncludeNonRooted: true },
			})
			.ignoreErrors();
		refreshPromise = collectionService.getRefreshPromise({
			stage: ProgressReportStage.allPathsDiscovered,
		});
		expect(refreshPromise).to.equal(
			undefined,
			"All paths discovered stage not applicable if a query is provided"
		);
	});

	test("resolveEnv() uses cache if complete and up to date info is available", async () => {
		const resolvedViaLocator = buildEnvInfo({
			executable: "Resolved via locator",
		});
		const cachedEnvs = getCachedEnvs();
		...
		sinon
			.stub(externalDependencies, "getFileInfo")
			.resolves({ ctime: 100, mtime: 100 });
		...
		collectionService = new EnvsCollectionService(cache, parentLocator);
		const resolved = await collectionService.resolveEnv(
	...
	test("resolveEnv() uses underlying locator once conda envs without python get a python installed", async () => {
		...
		collectionService = new EnvsCollectionService(cache, parentLocator);
		let resolved = await collectionService.resolveEnv(
			condaEnvWithoutPython.location
		);
		...
		resolved = await collectionService.resolveEnv(
			condaEnvWithoutPython.location
		);
		...
		const downstreamEvents = [
			{
				...
				searchLocation: Uri.file("folder1s"),
			},
