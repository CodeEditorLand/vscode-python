	...
	test("iterEnvs()", async () => {
		const expectedEnvs = [
			createBasicEnv(
				...
				path.join(regTestRoot, "py39", "python.exe")
			),
			createBasicEnv(
				PythonEnvKind.OtherGlobal,
				path.join(regTestRoot, "conda3", "python.exe")
			),
			createBasicEnv(
				PythonEnvKind.OtherGlobal,
				path.join(regTestRoot, "python37", "python.exe")
			),
			createBasicEnv(
				PythonEnvKind.OtherGlobal,
				path.join(regTestRoot, "python38", "python.exe")
			),
