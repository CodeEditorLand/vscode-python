...
class Venvs {
	...
	public async create(
		...
		const argv = [
			PYTHON_PATH.fileToCommandArgumentForPythonExt(),
			"-m",
			"virtualenv",
			envName,
		];
		try {
			...
			throw new Error(
				`Failed to create Env ${path.basename(envName)} Error: ${err}`
			);
		}
		...
		if (!filename) {
			throw new Error(
				`No environment to update exists in ${dirToLookInto}`
			);
		}
		return {
			executable: filename,
			envDir: path.dirname(path.dirname(filename)),
		};
	}
	...
	public async createDummyEnv(
		...
		const interpreterPath = path.join(
			this.root,
			envName,
			getOSType() === OSType.Windows ? "python.exe" : "python"
		);
		...
		try {
			...
			throw new Error(
				`Failed to create python executable ${interpreterPath}, Error: ${err}`
			);
		}
		return {
			executable: interpreterPath,
			envDir: path.dirname(interpreterPath),
		};
	}
	...
	public async update(filename: string): Promise<void> {
		try {
			...
			throw new Error(
				`Failed to update Workspace virtualenv executable ${filename}, Error: ${err}`
			);
		}
	}
...
type locatorFactoryFuncType1 = () => Promise<
	ILocator<BasicEnvInfo> & IDisposable
>;

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type locatorFactoryFuncType2 = (
	_: any
) => Promise<ILocator<BasicEnvInfo> & IDisposable>;

export type locatorFactoryFuncType = locatorFactoryFuncType1 &
	locatorFactoryFuncType2;
...
export function testLocatorWatcher(
	...
	async function isLocated(executable: string): Promise<boolean> {
		...
		return items.some((item) =>
			externalDeps.arePathsSame(item.executablePath, executable)
		);
	}
	...
	async function setupLocator(
		onChanged: (e: PythonEnvsChangedEvent) => Promise<void>
	) {
		locator = options?.arg
			? await createLocatorFactoryFunc(options.arg)
			: await createLocatorFactoryFunc();
	...
	test("Detect a new environment", async () => {
		...
		assert.notStrictEqual(
			actualEvent!.searchLocation,
			undefined,
			"Wrong event emitted"
		);
		assert.ok(
			externalDeps.arePathsSame(
				actualEvent!.searchLocation!.fsPath,
				path.dirname(envDir)
			),
			"Wrong event emitted"
		);
	...
	test("Detect when an environment has been updated", async () => {
		...
		const { executable, envDir } = await venvs.createDummyEnv(
			"one",
			options?.kind
		);
		await setupLocator(async (e) => {
		...
		if (options?.kind) {
			assert.strictEqual(
				actualEvent!.kind,
				options.kind,
				"Kind is not as expected"
			);
		}
		assert.notStrictEqual(
			actualEvent!.searchLocation,
			undefined,
			"Search location is not set"
		);
		assert.ok(
			externalDeps.arePathsSame(
				actualEvent!.searchLocation!.fsPath,
